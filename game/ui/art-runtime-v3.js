/* Dungeon Echo art runtime v3.
 * Presentation-only extension for the 2026-08-29 art pass.
 *
 * Adds three visible layers without owning gameplay:
 * - high-detail atlas coverage for eleven legacy monster archetypes;
 * - larger guardian/final-boss staging with floor sigils and depth auras;
 * - deterministic ambient dungeon dressing from the admitted prop atlas.
 *
 * Core canvas, collision, RNG, stats, saves and progression remain authoritative.
 */
(() => {
  'use strict';
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__DE_ART_RUNTIME_V3) return;
  ...
  const artAsset = name => {
    try { return new URL(`../../art/${name}`, scriptSrc || location.href).href; }
    catch (e) { return `art/${name}`; }
  };

  const ASSET_DATA = Object.freeze({
    monster: runtimeAsset('monster-deep-atlas-v2.svg'),
    props: runtimeAsset('dungeon-props-atlas-v1.svg'),
    guardian: artAsset('guardian-atlas-v11.png'),
    finalBoss: artAsset('final-boss-v11.png'),
  });

  // Cells are selected by visual fit, not by gameplay identity. The first v2 layer
  // keeps abomination / seraph / voidspawn / voidlord; this extension covers eleven
  // additional live profile archetypes while early fodder keeps the compact v11 art.
  ...
  const images = {
    ...
    guardian:imageFrom(ASSET_DATA.guardian),
    finalBoss:imageFrom(ASSET_DATA.finalBoss),
  };
  ...
  function boot(attempt = 0) {
    ...
    function occupiedCells() {
      const set = new Set();
      const mark = e => {
        if (e && Number.isInteger(e.x) && Number.isInteger(e.y)) set.add(`${e.x},${e.y}`);
      };
      mark(api.player);
      for (const list of [api.monsters, api.items, api.npcs, api.traps])
        for (const entity of list || []) mark(entity);
      return set;
    }
    ...
    function drawExtendedMonsters(d) {
    ...
    function bossAura(q, depth, finalBoss, now, radius) {
      ...
      glow.addColorStop(0, `hsla(${hue},92%,62%,.23)`);
      glow.addColorStop(.54, `hsla(${hue},88%,54%,.10)`);
      glow.addColorStop(1, `hsla(${hue},82%,46%,0)`);
      ctx.fillStyle = glow;
      ctx.beginPath();
      ctx.arc(q.x, q.y, radius * 1.16, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = `hsla(${hue},92%,66%,${.38 + phase * .18})`;
      ctx.lineWidth = finalBoss ? 2.2 : 1.5;
      ctx.beginPath();
      ctx.ellipse(q.x, q.y + radius * .46, radius * .55, radius * .13, 0, 0, Math.PI * 2);
      ctx.stroke();
      if (!reduceMotion) {
        ctx.globalAlpha = .40;
        ctx.beginPath();
        ctx.ellipse(q.x, q.y + radius * .46, radius * (.66 + phase * .04), radius * .17, 0, 0, Math.PI * 2);
        ctx.stroke();
      }
      ctx.restore();
    }
    ...
    function drawBosses(d, now) {
      ...
      for (const m of api.monsters || []) {
        ...
        if (m.boss && imageReady(images.finalBoss)) {
          const w = 92, h = 101;
          mask(q.x, q.y, 57, 57, .96);
          bossAura(q, depth, true, now, 49);
          cell(images.finalBoss, 0, 1, 1, q.x, q.y - 5, w, h,
            { alpha:1, shadowBlur:12, shadowColor:'rgba(64,18,94,.92)' });
          continue;
        }
        const index = GUARDIAN_CELL[depth];
        if (m.midBoss && index !== undefined && imageReady(images.guardian)) {
          const w = 72, h = 79;
          mask(q.x, q.y, 45, 44, .94);
          bossAura(q, depth, false, now, 39);
          cell(images.guardian, index, 3, 3, q.x, q.y - 3, w, h,
            { alpha:1, shadowBlur:9, shadowColor:'rgba(0,0,0,.9)' });
        }
      }
    }
    ...
    window.__DE_ART_RUNTIME_V3 = {
      version:'v3',
      owner:'presentation',
      gameplayMutation:false,
      extraMonsterArchetypes:Object.keys(MONSTER_ART).length,
