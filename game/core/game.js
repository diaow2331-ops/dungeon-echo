...
function renderTown() {
  ...
  if (shopEl) shopEl.innerHTML = [
    ...
    `<button type="button" data-townbuy="${r.id}"${meta.gold < r.price ? ' disabled' : ''}>购买</button></div>`).join('');
  ...
  if (wheelEl) {
    ...
    wheelEl.innerHTML =
      ...
      ` title="重摇全部八格，需 ${rc} G">重置轮盘 ${rc} G</button></div>`;
  }
}
...
function syncFullscreenUi() {
  ...
  button.innerHTML = active
    ...
    : '<span aria-hidden="true">⛶</span> 全屏 <kbd>F</kbd>';
...
if (seedLabel) seedLabel.textContent = RUN_SEED + '（' + PROFILE_ID + '）';
syncFullscreenUi();
scheduleFrame();
})();
