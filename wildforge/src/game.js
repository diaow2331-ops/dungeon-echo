...
function sellSettlementGood(id){
  ...
  consume(id,1);recordFrontierDelivery(game.frontier,market.site,id,1,price);game.trade.credits+=price;game.trade.sold[key]=used+1;game.trade.volume++;game.trade.turnover+=price;
...
function render(){ctx.save();if(game.fx.shake>0)ctx.translate((game.rng()-.5)*game.fx.shake,(game.rng()-.5)*game.fx.shake);drawBackdrop();drawBiomeAtmosphere();drawWorld();drawSurfaceAccents();drawSettlements();drawSupplyRoutes();drawFrontierCaravans();drawProjectiles();drawEnemyProjectiles();drawEnemies();drawDrops();drawCargoBundles();drawGuard();drawPlayer();drawTarget();drawFx();ctx.restore();drawLighting();updateHud();updateTargetTip();}
