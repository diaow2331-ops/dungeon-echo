...
const SAVE_KEY = 'wildforge.save.v0180';
...
function sellSettlementGood(id){
  ...
  const demand=settlementDailyDemand(market.site,game.trade.day),key=settlementDemandKey(market.site,id),used=settlementSold(market.site,id),priority=id===demand.goodId,cap=priority?demand.quantity:SETTLEMENT_GENERAL_DEMAND_CAP;
  ...
  if(priority&&used+1===demand.quantity){game.trade.credits+=demand.completionBonus;game.trade.turnover+=demand.completionBonus;toast(tr(`聚落订单完成 · 额外奖励 ◆${demand.completionBonus}`,`Settlement order complete · bonus ◆${demand.completionBonus}`));}
  game.saveDirty=true;renderTrade();renderInventory();renderHotbar();
}
...
function renderSettlementTrade(view,market){
  ...
  const rows=Object.entries(TRADE_GOODS).map(([id,good])=>{const priority=id===demand.goodId,local=good.home===site.biome,used=settlementSold(site,id),limit=priority?demand.quantity:SETTLEMENT_GENERAL_DEMAND_CAP,left=Math.max(0,limit-used),base=settlementBaseSellPrice(id,market),price=settlementSellPrice(base,site,id,game.trade.day),owned=count(id);return `<div class="trade-row ${priority?'priority-demand':local?'export':'demand'}"><div class="trade-good"><b>${GLYPH[id]||'◆'} ${itemName(id,lang)}</b><small>${priority?tr(`今日委托 · 溢价 +${Math.round((demand.premium-1)*100)}%`,`TODAY'S ORDER · +${Math.round((demand.premium-1)*100)}% premium`):local?tr('本地产物 · 低价消化','Local surplus · low-value intake'):tr('常规异地需求','Regular imported demand')}</small></div><div class="trade-owned">${tr('随身','Pack')} ${owned}</div><div class="trade-actions"><button data-settlement-sell="${id}" ${owned<=0||left<=0?'disabled':''}>${tr('交付','Deliver')} ◆${price}<small>${left}</small></button></div></div>`;}).join('');
  view.innerHTML=`<div class="trade-head settlement-head"><div><b>⌂ ${name} · ${lang==='zh'?market.biome.zh:market.biome.en}</b><small>${tr('聚落买方 · 周边受保护','Settlement buyer · protected perimeter')}</small></div><strong>◆ ${game.trade.credits}</strong></div><div class="settlement-order"><span>${tr('今日急需','Priority')}</span><b>${itemName(demand.goodId,lang)} ×${remaining}</b><small>${tr(`完成整单另奖 ◆${demand.completionBonus}`,`Complete the order for +◆${demand.completionBonus}`)}</small></div><div class="logistics-strip"><span>${tr('货运','Cargo')} <b>${load}/${cap}</b></span><span>${tr('订单剩余','Order left')} <b>${remaining}</b></span><span>${tr('安全圈','Safe zone')} <b>${Math.round(SETTLEMENT_SAFE_RADIUS)}m</b></span></div><div class="trade-note">${tr('聚落是最终买方，不提供生产或仓储。你的路标负责备货；把货亲自运到这里，才能吃到每日需求溢价。','Settlements are destination buyers, not depots. Produce and load at your beacons, then physically deliver cargo here for the daily demand premium.')}</div><div class="trade-list">${rows}</div>`;
...
function updateHud(){
  ...
  const beacon=nearestBeacon(),route=routeStateAt(p.x,p.y),supply=totalBeaconSupply(),settlement=nearestSettlement(p.x,p.y,90),destination=settlement?` · ⌂${settlementName(settlement,lang)} ${Math.ceil(settlement.d)}m`:'';
