...
function transportOfferForMarket(market=nearbyTradePost()){
  ...
  return makeTransportOffer({day:game.trade.day,originX:market.beacon.x,originBiome:market.biome.id,settlements,cargoByHome,reliabilityBySettlement});
}
...
function sellSettlementGood(id){
  ...
  const base=settlementBaseSellPrice(id,market),price=settlementSellPrice(base,market.site,id,game.trade.day,reliability);consume(id,1);game.trade.credits+=price;game.trade.sold[key]=used+1;game.trade.volume++;game.trade.turnover+=price;
...
function renderSettlementTrade(view,market){
  ...
  const rows=Object.entries(TRADE_GOODS).map(([id,good])=>{const priority=id===demand.goodId,preferred=id===preference,local=good.home===site.biome,contractMatch=matchingTransportContract(site,id),used=settlementSold(site,id),baseLimit=priority?demand.quantity:SETTLEMENT_GENERAL_DEMAND_CAP,contractRemaining=contractMatch?Math.max(0,contractMatch.quantity-contractMatch.delivered):0,limit=contractMatch?Math.max(baseLimit,used+contractRemaining):baseLimit,left=Math.max(0,limit-used),base=settlementBaseSellPrice(id,market),price=settlementSellPrice(base,site,id,game.trade.day,reliability),owned=count(id),detail=contractMatch?tr(`运输委托 · ${contractMatch.delivered}/${contractMatch.quantity}`,`TRANSPORT CONTRACT · ${contractMatch.delivered}/${contractMatch.quantity}`):priority?tr(`今日急需 · 溢价 +${Math.round((demand.premium-1)*100)}%`,`TODAY'S ORDER · +${Math.round((demand.premium-1)*100)}% premium`):preferred?tr(`长期偏好 · 稳定溢价 +${Math.round((SETTLEMENT_PREFERENCE_PREMIUM-1)*100)}%`,`LONG-TERM PREFERENCE · +${Math.round((SETTLEMENT_PREFERENCE_PREMIUM-1)*100)}% stable premium`):local?tr('本地产物 · 低价消化','Local surplus · low-value intake'):tr('常规异地需求','Regular imported demand');return `<div class="trade-row ${contractMatch?'contract-cargo':priority?'priority-demand':preferred?'preferred-demand':local?'export':'demand'}"><div class="trade-good"><b>${GLYPH[id]||'◆'} ${itemName(id,lang)}</b><small>${detail}</small></div><div class="trade-owned">${tr('随身','Pack')} ${owned}</div><div class="trade-actions"><button data-settlement-sell="${id}" ${owned<=0||left<=0?'disabled':''}>${tr('交付','Deliver')} ◆${price}<small>${left}</small></button></div></div>`;}).join('');
  const memory=`<div class="settlement-memory"><div><span>${tr('长期偏好','Long-term preference')}</span><b>${itemName(preference,lang)} · +${Math.round((SETTLEMENT_PREFERENCE_PREMIUM-1)*100)}%</b></div><div><span>${tr('可靠度','Reliability')}</span><b>${settlementReliabilityPips(site)} ${reliability}/${SETTLEMENT_MAX_RELIABILITY}</b></div><small>${tr('完成整单或运输委托会建立可靠度；接下委托后逾期会降低 1 级。更高可靠度会扩大订单并提高奖励。','Complete priority orders or transport contracts to build reliability. An accepted contract that expires costs 1 level. Higher reliability grows orders and rewards.')}</small></div>`;
  view.innerHTML=`<div class="trade-head settlement-head"><div><b>⌂ ${name} · ${lang==='zh'?market.biome.zh:market.biome.en}</b><small>${tr('聚落买方 · 周边受保护','Settlement buyer · protected perimeter')}</small></div><strong>◆ ${game.trade.credits}</strong></div>${memory}${contractCard}<div class="settlement-order"><span>${tr('今日急需','Priority')}</span><b>${itemName(demand.goodId,lang)} ×${remaining}</b><small>${tr(`完成整单另奖 ◆${demand.completionBonus}`,`Complete the order for +◆${demand.completionBonus}`)}</small></div><div class="logistics-strip"><span>${tr('货运','Cargo')} <b>${load}/${cap}</b></span><span>${tr('订单剩余','Order left')} <b>${remaining}</b></span><span>${tr('安全圈','Safe zone')} <b>${Math.round(SETTLEMENT_SAFE_RADIUS)}m</b></span></div><div class="trade-note">${tr('聚落会记住长期贸易关系：稳定偏好便于规划固定商路，而每日急需提供更高短期溢价。你的路标仍负责生产与仓储，货物必须亲自送达。','Settlements remember trade relationships: stable preferences support planned routes while daily priority demand pays a stronger short-term premium. Your beacons still own production and storage, and cargo must be delivered physically.')}</div><div class="trade-list">${rows}</div>`;
...
function render(){ctx.save();if(game.fx.shake>0)ctx.translate((game.rng()-.5)*game.fx.shake,(game.rng()-.5)*game.fx.shake);drawBackdrop();drawBiomeAtmosphere();drawWorld();drawSurfaceAccents();drawSettlements();drawSupplyRoutes();drawProjectiles();drawEnemyProjectiles();drawEnemies();drawDrops();drawCargoBundles();drawGuard();drawPlayer();drawTarget();drawFx();ctx.restore();drawLighting();updateHud();updateTargetTip();}
