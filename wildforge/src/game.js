...
import {createFrontierSimulation,sanitizeFrontierSimulation,stepFrontierSimulation,frontierSettlement,frontierMarketQuote,frontierDemandMap,frontierRelation,frontierRelationName,frontierFactionStatus,recordFrontierDelivery,recordFrontierAid,recordFrontierThreatRemoval,frontierProsperityLevel,frontierThreatFactor,frontierWeatherName} from './frontier-simulation.js';
...
function killEnemy(e) {
  e.dead=true; const type=e.type,frontierSite=nearestSettlement(e.x,e.y,45);recordFrontierThreatRemoval(game.frontier,frontierSite||{biome:game.world.biome(e.x).id},e.boss?4:e.elite?2:1);
...
function drawEnemyBody(e,s){
  ...
  if(t==='greyveil_raider'&&artReady()){const f=Math.floor(performance.now()/180)%4;drawArt('raider_'+f,-s*.58,-s*.86,s*1.16,s*1.74);return;}
  if(t==='moss_crawler'){
