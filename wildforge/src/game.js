...
import {renderInventorySlots,renderHotbarSlots,normalizeHotbar,normalizeInventoryLayout,INVENTORY_SLOT_COUNT} from './inventory-ui.js';
...
import {createFrontierSimulation,sanitizeFrontierSimulation,stepFrontierSimulation,frontierSettlement,frontierMarketQuote,frontierDemandMap,frontierRelation,frontierRelationName,frontierFactionStatus,recordFrontierDelivery,recordFrontierAid,recordFrontierRaidResistance,recordFrontierThreatRemoval,frontierProsperityLevel,frontierThreatFactor,frontierWeatherName} from './frontier-simulation.js';

const $ = s => document.querySelector(s);
const $$ = s => [...document.querySelectorAll(s)];
const canvas = $('#game'), ctx = canvas.getContext('2d', {alpha:false});
const CORE_ART_SRC='./assets/core-art.webp';
const CORE_ART=new Image();
CORE_ART.decoding='async';
CORE_ART.src=CORE_ART_SRC;
const ART=Object.freeze({
  tile_grass:[0,0,16,16],tile_soil:[16,0,16,16],tile_stone:[32,0,16,16],tile_plank:[48,0,16,16],tile_coal:[64,0,16,16],tile_copper:[80,0,16,16],tile_iron:[96,0,16,16],tile_leaf:[112,0,16,16],
  player_0:[0,20,16,24],player_1:[18,20,16,24],player_2:[36,20,16,24],player_3:[54,20,16,24],player_4:[72,20,16,24],player_5:[90,20,16,24],player_6:[108,20,16,24],
  slime_0:[0,48,16,12],slime_1:[18,48,16,12],slime_2:[36,48,16,12],slime_3:[54,48,16,12],slime_4:[72,48,16,12],
  raider_0:[0,64,16,24],raider_1:[18,64,16,24],raider_2:[36,64,16,24],raider_3:[54,64,16,24],
  guard_0:[0,92,16,24],guard_1:[18,92,16,24],guard_2:[36,92,16,24],guard_3:[54,92,16,24],
  boar_0:[0,120,24,16],boar_1:[26,120,24,16],boar_2:[52,120,24,16],boar_3:[78,120,24,16],
  prop_beacon:[144,0,24,40],prop_chest:[172,0,32,24],prop_cargo:[208,0,40,22],prop_workbench:[144,44,40,24],prop_crates:[188,44,40,24],prop_barrels:[232,44,24,24]
});
...
const SAVE_KEY = 'wildforge.save.v0430';
const LEGACY_SAVE_KEY_0420 = 'wildforge.save.v0420';
const LEGACY_SAVE_KEY_0410 = 'wildforge.save.v0410';
const LEGACY_SAVE_KEY_0230 = 'wildforge.save.v0230';
...
const LEGACY_SAVE_KEY_0180 = 'wildforge.save.v0180';
const LEGACY_SAVE_KEY_0170 = 'wildforge.save.v0170';
const LEGACY_SAVE_KEY_0160 = 'wildforge.save.v0160';
...
const PACKBEAST_CAP=10;
const PACKBEAST_MAX_HP=90;
const PACKBEAST_HIRE_COST=52;
const PACKBEAST_MIN_RELIABILITY=2;
const PACKBEAST_FULL_SPEED_PENALTY=.06;
const PACKBEAST_SAFE_RECOVERY_PER_SEC=2.4;
const RAIDER_MIN_COOLDOWN=70;
const DIAGNOSTIC_SAMPLE_CAP=240;
const DIAGNOSTIC_FRONTIER_SAMPLE_EVERY=60;
...
const HOTBAR_DEFAULT = Array.from({length:HOTBAR_SIZE},()=> '');
...
const game = {
  running:false, world:null, seed:'', settlementLayout:'onboarding-v2', time:0.18, last:0, autosave:0, spawnTimer:0, physicsAccumulator:0, wheelAccum:0,
  player:null, inventory:{}, inventoryLayout:Array.from({length:INVENTORY_SLOT_COUNT},()=>''), hotbar:[...HOTBAR_DEFAULT], selected:0, enemies:[], drops:[], projectiles:[], enemyProjectiles:[],
  ...
  fx:{particles:[],shake:0}, smartCursor:false, autoTool:false, moveAxis:{x:0,y:0,touch:false}, mobileAim:{x:1,y:0,active:false}, restFx:0, relicScanCd:0, relicHint:null, toastTimer:0, saveDirty:false, objectiveStage:0, discoveries:[], guardianDefeated:{}, openedChestCount:0, campRespawn:null, campBindTimer:0, infrastructure:{beacons:[]}, worldProgress:{biomesVisited:[],relicBiomes:[],evolution:0}, trade:{credits:12,day:0,bought:{},sold:{},produced:{},volume:0,turnover:0,contractsCompleted:0,contract:null,settlementReliability:{},aid:{}}, frontier:null, cargoBundles:[], guard:null, packbeast:null, raiderCooldown:0, diagnostics:null, rng:Math.random
};

function freshDiagnostics(){return {frameMs:[],frontierMs:[],saveMs:[],frontierSampleCounter:0};}
function resetDiagnostics(){game.diagnostics=freshDiagnostics();}
function pushDiagnostic(key,value){const d=game.diagnostics||(game.diagnostics=freshDiagnostics()),a=d[key];if(!Array.isArray(a)||!Number.isFinite(value))return;a.push(value);if(a.length>DIAGNOSTIC_SAMPLE_CAP)a.splice(0,a.length-DIAGNOSTIC_SAMPLE_CAP);}
function metricSummary(values){const a=(values||[]).filter(Number.isFinite).slice().sort((x,y)=>x-y),n=a.length;if(!n)return {samples:0,median:0,p95:0,max:0};const pick=q=>a[Math.min(n-1,Math.max(0,Math.ceil(n*q)-1))];return {samples:n,median:+pick(.5).toFixed(2),p95:+pick(.95).toFixed(2),max:+a[n-1].toFixed(2)};}
function diagnosticSnapshot(){const d=game.diagnostics||freshDiagnostics(),liveEnemies=game.enemies?.filter(e=>!e.dead)||[];return {version:VERSION,seed:game.seed||'',frameMs:metricSummary(d.frameMs),frontierTickMs:metricSummary(d.frontierMs),saveMs:metricSummary(d.saveMs),entities:{enemies:liveEnemies.length,raidEnemies:liveEnemies.filter(e=>e.raidId).length,drops:game.drops?.length||0,projectiles:(game.projectiles?.length||0)+(game.enemyProjectiles?.length||0),cargoBundles:game.cargoBundles?.length||0,caravans:game.frontier?.caravans?.length||0,raids:game.frontier?.raids?.length||0},world:{settlements:game.frontier?.settlements?Object.keys(game.frontier.settlements).length:0,frontierElapsedDays:+(game.frontier?.elapsedDays||0).toFixed(3)}};}
if(typeof window!=='undefined')window.WildforgeDiagnostics=Object.freeze({snapshot:diagnosticSnapshot,reset:resetDiagnostics});
...
function freshTrade(){return {credits:12,day:0,bought:{},sold:{},produced:{},volume:0,turnover:0,contractsCompleted:0,contract:null,settlementReliability:{},aid:{}};}
function sanitizeTransportContract(raw){
  if(!raw||typeof raw!=='object'||!TRADE_GOODS[raw.goodId]||!raw.destinationId)return null;
  const quantity=Math.max(1,Math.min(8,Math.floor(Number(raw.quantity)||0))),delivered=Math.max(0,Math.min(quantity,Math.floor(Number(raw.delivered)||0))),deadlineDay=Math.max(0,Math.floor(Number(raw.deadlineDay)||0));
  if(!quantity||delivered>=quantity)return null;
  return {offerId:String(raw.offerId||''),goodId:String(raw.goodId),quantity,delivered,reward:Math.max(0,Math.floor(Number(raw.reward)||0)),distance:Math.max(0,Math.floor(Number(raw.distance)||0)),destinationId:String(raw.destinationId),destinationBiome:String(raw.destinationBiome||''),destinationX:Number(raw.destinationX)||0,acceptedDay:Math.max(0,Math.floor(Number(raw.acceptedDay)||0)),deadlineDay};
}
function sanitizeTrade(raw){
  if(!raw||typeof raw!=='object')return freshTrade();const src=raw,base=freshTrade();
  base.credits=Math.max(0,Math.floor(Number(src.credits)||0));base.day=Math.max(0,Math.floor(Number(src.day)||0));base.volume=Math.max(0,Math.floor(Number(src.volume)||0));base.turnover=Math.max(0,Math.floor(Number(src.turnover)||0));base.contractsCompleted=Math.max(0,Math.floor(Number(src.contractsCompleted)||0));
  base.bought=src.bought&&typeof src.bought==='object'?Object.fromEntries(Object.entries(src.bought).map(([k,v])=>[k,Math.max(0,Math.floor(Number(v)||0))])):{};base.sold=src.sold&&typeof src.sold==='object'?Object.fromEntries(Object.entries(src.sold).map(([k,v])=>[k,Math.max(0,Math.floor(Number(v)||0))])):{};base.produced=src.produced&&typeof src.produced==='object'?Object.fromEntries(Object.entries(src.produced).map(([k,v])=>[k,Math.max(0,Math.floor(Number(v)||0))])):{};base.settlementReliability=src.settlementReliability&&typeof src.settlementReliability==='object'?Object.fromEntries(Object.entries(src.settlementReliability).filter(([k])=>k).map(([k,v])=>[k,Math.max(0,Math.min(SETTLEMENT_MAX_RELIABILITY,Math.floor(Number(v)||0)))])):{};base.aid=src.aid&&typeof src.aid==='object'?Object.fromEntries(Object.entries(src.aid).filter(([k])=>k).map(([k,v])=>[k,Math.max(0,Math.floor(Number(v)||0))])):{};base.contract=sanitizeTransportContract(src.contract);return base;
}
...
function sanitizePackbeast(raw){if(!raw||typeof raw!=='object'||Number(raw.hp)<=0)return null;const contents={};let used=0;for(const id of Object.keys(TRADE_GOODS)){const n=Math.max(0,Math.floor(Number(raw?.contents?.[id])||0)),take=Math.min(n,PACKBEAST_CAP-used);if(take>0){contents[id]=take;used+=take;}if(used>=PACKBEAST_CAP)break;}return {x:Number(raw.x)||0,y:Number(raw.y)||0,hp:Math.max(1,Math.min(PACKBEAST_MAX_HP,Number(raw.hp)||PACKBEAST_MAX_HP)),maxHp:PACKBEAST_MAX_HP,contents,stranded:!!raw.stranded,facing:Number(raw.facing)<0?-1:1};}
...
function activePackbeast(){return game.packbeast&&game.packbeast.hp>0?game.packbeast:null;}
function joinedPackbeast(){const beast=activePackbeast();return beast&&!beast.stranded?beast:null;}
function packbeastUnits(){return bundleUnits(activePackbeast());}
function joinedPackbeastUnits(){return bundleUnits(joinedPackbeast());}
function packbeastCount(id){return Math.max(0,Number(joinedPackbeast()?.contents?.[id])||0);}
function packbeastLoadRatio(){return joinedPackbeast()?Math.max(0,Math.min(1,joinedPackbeastUnits()/PACKBEAST_CAP)):0;}
function packbeastValue(){return bundleValue(joinedPackbeast());}
function convoyCargoUnits(){return cargoLoad()+joinedPackbeastUnits();}
function cargoDeclaredValue(){return Object.entries(TRADE_GOODS).reduce((sum,[id,g])=>sum+count(id)*g.base,0)+packbeastValue();}
...
function sanitizeInventory(raw){
  ...
  for(const [id,value] of Object.entries(raw)){const def=ITEMS[id],n=Math.max(0,Math.floor(Number(value)||0));if(def&&n>0)out[id]=Math.min(def.stack||99,n);}
  return out;
}
function ensureInventorySlot(id){
  if(!id||count(id)>0&&game.inventoryLayout.includes(id))return true;
  if(game.inventoryLayout.includes(id))return true;const slot=game.inventoryLayout.indexOf('');if(slot<0)return false;game.inventoryLayout[slot]=id;return true;
}
function autoAssignHotbar(id){
  const def=ITEMS[id];if(!def||def.kind==='cargo'||game.hotbar.includes(id))return;const slot=game.hotbar.indexOf('');if(slot>=0)game.hotbar[slot]=id;
}
function canStoreItem(id,n=1){
  const def=ITEMS[id],amount=Math.max(0,Math.floor(Number(n)||0));if(!def||amount<=0)return false;const before=count(id);if(before+amount>(def.stack||99))return false;return before>0||game.inventoryLayout.includes(id)||game.inventoryLayout.includes('');
}
function addItem(id,n=1) {
  if(!canStoreItem(id,n))return false;const before=count(id),amount=Math.floor(Number(n)||0);if(before<=0&&!ensureInventorySlot(id))return false;
  game.inventory[id]=before+amount;if(before<=0)autoAssignHotbar(id);game.saveDirty=true;updateObjective();return true;
}
function consume(id,n=1) {
  if(count(id)<n)return false;game.inventory[id]-=n;if(game.inventory[id]<=0){delete game.inventory[id];game.inventoryLayout=game.inventoryLayout.map(x=>x===id?'':x);game.hotbar=game.hotbar.map(x=>x===id?'':x);}game.saveDirty=true;return true;
}
function moveInventoryItem(id,target){
  if(!ITEMS[id]||count(id)<=0)return false;target=Math.max(0,Math.min(INVENTORY_SLOT_COUNT-1,Number(target)||0));const from=game.inventoryLayout.indexOf(id);if(from<0||from===target)return false;const swap=game.inventoryLayout[target]||'';game.inventoryLayout[target]=id;game.inventoryLayout[from]=swap;game.saveDirty=true;renderInventory();return true;
}
function ownedHotbar(raw){return normalizeHotbar(raw,HOTBAR_DEFAULT).map(id=>id&&count(id)>0?id:'');}
...
function applySave(raw) {
  ...
  if(![VERSION,'0.43.0'].includes(raw.v)&&game.world.relaxLegacySurfaceTrees)game.world.relaxLegacySurfaceTrees();
  game.selected=Math.max(0,Math.min(HOTBAR_SIZE-1,Number(raw.selected)||0)); game.time=Number(raw.time)||.18; game.objectiveStage=Number(raw.objectiveStage)||0; game.trade=sanitizeTrade(raw.trade); game.cargoBundles=sanitizeCargoBundles(raw.cargoBundles); game.guard=sanitizeGuard(raw.guard); game.packbeast=sanitizePackbeast(raw.packbeast); game.raiderCooldown=0; game.worldProgress=raw.worldProgress&&typeof raw.worldProgress==='object'?{biomesVisited:Array.isArray(raw.worldProgress.biomesVisited)?raw.worldProgress.biomesVisited:[],relicBiomes:Array.isArray(raw.worldProgress.relicBiomes)?raw.worldProgress.relicBiomes:[],evolution:Math.max(0,Number(raw.worldProgress.evolution)||0)}:{biomesVisited:[],relicBiomes:[],evolution:0}; game.infrastructure=raw.infrastructure&&typeof raw.infrastructure==='object'?{beacons:Array.isArray(raw.infrastructure.beacons)?raw.infrastructure.beacons.filter(b=>Number.isFinite(b.x)&&Number.isFinite(b.y)).slice(0,8).map(b=>({x:b.x,y:b.y,createdAt:Number(b.createdAt)||0,supply:Math.min(MAX_BEACON_SUPPLY,Math.max(0,Number(b.supply)||0)),warehouse:sanitizeWarehouse(b.warehouse)})):[]}:{beacons:[]}; game.nightsSurvived=Math.max(0,Number(raw.nightsSurvived)||0); game.outpostReady=!!raw.outpostReady; game.nightState='init'; game.nightSurge=0; game.discoveries=Array.isArray(raw.discoveries)?raw.discoveries:[]; seedOnboardingFromSave(raw); game.guardianDefeated=raw.guardianDefeated&&typeof raw.guardianDefeated==='object'?raw.guardianDefeated:{}; game.openedChestCount=Math.max(0,Number(raw.openedChestCount)||0); game.campRespawn=raw.campRespawn&&Number.isFinite(raw.campRespawn.x)&&Number.isFinite(raw.campRespawn.y)?raw.campRespawn:null; game.forgePlaced=!!raw.forgePlaced; game.forgeActive=!!raw.forgeActive; game.forgeProgress=0; game.bossActive=!!raw.bossActive; game.bossDefeated=!!raw.bossDefeated; game.completed=!!raw.completed; game.campBindTimer=0;
  game.frontier=sanitizeFrontierSimulation(raw.frontier,{seed:game.seed,settlements:game.world.settlements||[],day:game.nightsSurvived});resetDiagnostics();
...
function updateWorldRhythm(dt){
  ...
  if(phase==='night'){
    ...
    game.nightsSurvived++;game.saveDirty=true;syncTradeDay();
...
function milestoneKey(id){return `milestone:${id}`;}
function hasMilestone(id){return game.discoveries.includes(milestoneKey(id));}
function markMilestone(id){const key=milestoneKey(id);if(game.discoveries.includes(key))return false;game.discoveries.push(key);game.saveDirty=true;updateObjective();return true;}
function seedOnboardingMilestone(id){const key=milestoneKey(id);if(!game.discoveries.includes(key))game.discoveries.push(key);}
function seedOnboardingFromSave(raw){const oldStage=Math.max(0,Number(raw?.objectiveStage)||0),trade=raw?.trade||{},beacons=raw?.infrastructure?.beacons||[];if(oldStage>=2||Number(raw?.inventory?.workbench)>0)seedOnboardingMilestone('crafted-workbench');if(oldStage>=3||['wood_pick','stone_pick','copper_pick','iron_pick','delver_pick'].some(id=>Number(raw?.inventory?.[id])>0))seedOnboardingMilestone('crafted-pick');if(beacons.length)seedOnboardingMilestone('first-beacon');const settlementSale=Object.keys(trade.sold||{}).some(key=>String(key).startsWith('settlement:'));if(settlementSale||Number(trade.contractsCompleted)>0){seedOnboardingMilestone('first-settlement');seedOnboardingMilestone('first-market');}if(trade.contract||Number(trade.contractsCompleted)>0)seedOnboardingMilestone('first-contract');if(Number(trade.contractsCompleted)>0)seedOnboardingMilestone('cross-region-delivery');}
function onboardingCargoReady(){if(Object.keys(TRADE_GOODS).some(id=>availableCargoCount(id)>0))return true;return (game.infrastructure?.beacons||[]).some(b=>Object.values(b.warehouse||{}).some(n=>Number(n)>0));}
function onboardingSettlement(){return nearestSettlement(game.player?.x||game.world?.spawn?.x||0,game.player?.y||game.world?.spawn?.y||0);}
function onboardingDirection(site){if(!site||!game.player)return '';const dx=site.x-game.player.x,arrow=Math.abs(dx)<3?'•':dx>0?'→':'←';return `${arrow} ${Math.ceil(Math.abs(dx))}m`;}

function updateObjective() {
  const hasWorkbench=hasMilestone('crafted-workbench')||count('workbench')>0,hasPick=hasMilestone('crafted-pick')||['wood_pick','stone_pick','copper_pick','iron_pick','delver_pick'].some(id=>count(id)>0),hasMeal=hasMilestone('first-meal'),settlementSeen=hasMilestone('first-settlement'),marketSeen=hasMilestone('first-market'),beaconReady=(game.infrastructure?.beacons||[]).length>0,cargoReady=onboardingCargoReady(),contract=activeTransportContract(),routeDone=(game.trade?.contractsCompleted||0)>0||hasMilestone('cross-region-delivery');
  let stage=0,zh='',en='';
  if(!hasWorkbench){stage=0;zh=count('wood')>=4?'把青芯木加工成木板 → 制作第一张工匠台':'收集青芯木与纤维 → 准备第一张工匠台';en=count('wood')>=4?'Turn Greenheart Wood into planks → craft a Craft Table':'Gather Greenheart Wood and fiber → prepare a Craft Table';}
  else if(!hasPick){stage=1;zh='制作第一把镐 → 石料、火堆与食物都从这里开始';en='Craft your first pick → stone, a fire pit, and food come next';}
  else if(!hasMeal){const meat=count('raw_meat')+count('trail_ration');stage=2;zh=meat>0?'饥饿时进食 → 鲜肉可应急，熔火堆能烤成旅行口粮':'制作木刃并猎取棘鬃兽 → 获取鲜肉；饥饿会影响远行';en=meat>0?'Eat when hungry → raw meat works, an Ember Pit makes better trail rations':'Craft a wooden blade and hunt a Bramble Boar → secure meat for travel';}
  else if(!settlementSeen){const site=onboardingSettlement(),where=site?`${settlementName(site,'zh')} ${onboardingDirection(site)}`:'附近聚落';stage=3;zh=`食物已解决 → 前往 ${where}，看看这个世界需要什么`;en=`Food secured → reach ${site?settlementName(site,'en')+' '+onboardingDirection(site):'a nearby settlement'} and see what the world needs`;}
  else if(!marketSeen){stage=4;zh='靠近聚落按 E / 行囊 → 打开「贸易」，查看实时短缺';en='At the settlement press E / Pack → Trade and inspect its live shortage';}
  else if(!beaconReady){stage=5;zh='制作并放置边境路标 → 建立你的第一处货站';en='Craft and place a Frontier Beacon → establish your first depot';}
  else if(!cargoReady){stage=6;zh='在路标旁放工匠台 → 贸易页生产或采购第一批本地标准货';en='Put a Craft Table by the beacon → produce or buy your first local cargo';}
  else if(!contract&&!routeDone){stage=7;zh='打开路标贸易页 → 接取一份指向真实短缺的运输委托';en='Open beacon Trade → accept a transport contract driven by a real shortage';}
  else if(contract){const site=transportContractSite(contract),where=site?onboardingDirection(site):`→ ${Math.ceil(contract.distance||0)}m`,days=contractDaysLeft(contract,game.trade.day);stage=8;zh=`运输 ${itemName(contract.goodId,'zh')} ${contract.delivered}/${contract.quantity} → ${site?settlementName(site,'zh'):'目标聚落'} · ${where} · 剩 ${days} 日`;en=`Haul ${itemName(contract.goodId,'en')} ${contract.delivered}/${contract.quantity} → ${site?settlementName(site,'en'):'destination'} · ${where} · ${days}d left`; }
  else if(!routeDone){stage=8;zh='把标准货送进聚落真实库存 → 完成第一笔跨区交付';en='Deliver standardized cargo into real settlement stock → finish your first route';}
  else if(count('copper_ore')<2&&count('copper_bar')<1&&count('copper_pick')<1){stage=9;zh='第一条商路已跑通 → 下到浅层洞带寻找赤铜';en='First trade route established → descend into the shallow caves for copper';}
  else if(count('copper_pick')<1){stage=10;zh='熔炼赤铜 → 打造赤铜镐，扩大探索半径';en='Smelt copper → forge a Copper Pick and expand your reach';}
  else if(count('iron_pick')<1){stage=11;zh='进入冷铁深层 → 升级冷铁装备';en='Reach the Cold-Iron Deep → upgrade to cold-iron gear';}
  else if(count('crystal')<1){stage=12;zh='寻找星晶裂隙 → 带回第一块星晶';en='Seek the Starshard Rift → return with your first Star Crystal';}
  else if(count('star_forge')<1&&!game.forgePlaced){stage=13;zh='打造星核炉 → 带它深入星晶裂隙';en='Build a Starcore Forge → carry it into the Starshard Rift';}
  else if(!game.forgeActive){stage=14;zh='在裂隙点燃星核炉 → 做好迎战准备';en='Ignite the Starcore Forge in the rift → brace for impact';}
  else if(!game.bossDefeated){stage=15;zh='击败裂隙巨兽 → 改变荒境的终局';en='Defeat the Rift Behemoth → change the frontier’s endgame';}
  else {stage=16;zh='荒境熔炉已点亮 → 继续经营商路、介入战争与改变势力格局';en='Wildforge is lit → keep trading, intervening in wars, and reshaping the frontier';}
  const objective=$('#objective'),stageChanged=stage!==game.objectiveStage;if(stageChanged){game.objectiveStage=stage;game.saveDirty=true;if(objective&&game.running){objective.classList.remove('pulse');void objective.offsetWidth;objective.classList.add('pulse');haptic(8);}}
  if(objective)objective.classList.toggle('contract-active',!!contract);const el=$('#objectiveText');if(el)el.textContent=lang==='zh'?zh:en;
}
...
function syncTradeDay(){
  if(!game.trade)game.trade=freshTrade();const day=Math.max(0,Math.floor(game.nightsSurvived||0));
  if(game.trade.day!==day){game.trade.day=day;game.trade.bought={};game.trade.sold={};game.trade.produced={};game.saveDirty=true;}
  if(game.trade.contract&&day>game.trade.contract.deadlineDay){const failed=game.trade.contract,site=(game.world?.settlements||[]).find(x=>x.id===failed.destinationId)||null,trust=site?adjustSettlementReliability(site,-1):null;game.trade.contract=null;game.saveDirty=true;const suffix=trust&&trust.after<trust.before?tr(` · ${settlementName(site,'zh')} 可靠度 ${trust.after}/${SETTLEMENT_MAX_RELIABILITY}`,` · ${settlementName(site,'en')} reliability ${trust.after}/${SETTLEMENT_MAX_RELIABILITY}`):'';toast(tr(`运输委托逾期 · ${itemName(failed.goodId,'zh')}`,`Transport contract expired · ${itemName(failed.goodId,'en')}`)+suffix);}
}
...
function nearestSettlement(x=game.player?.x||0,y=game.player?.y||0,range=Infinity){
  let best=null,bd=range;for(const site of game.world?.settlements||[]){const d=Math.hypot(site.x-x,site.y-y);if(d<bd){bd=d;best={...site,d};}}return best;
}
function nearbySettlementMarket(){
  const site=nearestSettlement();if(!site||site.d>SETTLEMENT_TRADE_RADIUS)return null;return {site,biome:game.world.biome(site.x),d:site.d};
}
function settlementBaseSellPrice(id,market){
  const good=TRADE_GOODS[id];if(!good||!market)return 0;const local=good.home===market.site.biome,reliability=settlementReliability(market.site),quote=frontierMarketQuote(game.frontier,market.site,id,{reliability});
...
function settlementAtWar(site){return (game.world?.settlements||[]).some(other=>other.id!==site?.id&&frontierFactionStatus(game.frontier,other)?.status!=='annexed'&&frontierRelation(game.frontier,site,other).stance==='war');}
...
function settlementReliabilityPips(site){const value=settlementReliability(site);return '◆'.repeat(value)+'◇'.repeat(SETTLEMENT_MAX_RELIABILITY-value);}
function activeTransportContract(){const c=game.trade?.contract;return c&&c.delivered<c.quantity&&game.trade.day<=c.deadlineDay?c:null;}
function transportContractSite(contract=activeTransportContract()){return contract?(game.world?.settlements||[]).find(site=>site.id===contract.destinationId)||null:null;}
function transportOfferForMarket(market=nearbyTradePost()){
  if(!market||activeTransportContract())return null;const cargoByHome=Object.fromEntries(Object.entries(TRADE_GOODS).map(([id,good])=>[good.home,id])),settlements=game.world?.settlements||[],reliabilityBySettlement=Object.fromEntries(settlements.map(site=>[site.id,settlementReliability(site)]));
  return makeTransportOffer({day:game.trade.day,originX:market.beacon.x,originBiome:market.biome.id,settlements,cargoByHome,reliabilityBySettlement,demandBySettlement:frontierDemandMap(game.frontier,settlements),completedContracts:game.trade.contractsCompleted||0});
}
function acceptTransportContract(){
  syncTradeDay();const market=nearbyTradePost(),offer=transportOfferForMarket(market);if(!market||!offer)return;
  game.trade.contract={...offer,acceptedDay:game.trade.day,deadlineDay:game.trade.day+TRANSPORT_CONTRACT_DAYS,delivered:0};game.saveDirty=true;markMilestone('first-contract');updateObjective();renderTrade();updateHud();
  const site=transportContractSite(game.trade.contract),name=site?settlementName(site,lang):tr('目标聚落','destination'),where=site?onboardingDirection(site):`→ ${offer.distance}m`;toast(tr(`运输委托已接 · ${itemName(offer.goodId,'zh')} ×${offer.quantity} → ${name} · ${where}`,`Contract accepted · ${itemName(offer.goodId,'en')} ×${offer.quantity} → ${name} · ${where}`));
}
function matchingTransportContract(site,id){const c=activeTransportContract();return c&&site?.id===c.destinationId&&id===c.goodId?c:null;}
function deliverTransportContract(site,id){
  const c=matchingTransportContract(site,id);if(!c)return 0;c.delivered=Math.min(c.quantity,c.delivered+1);game.saveDirty=true;
  if(c.delivered>=c.quantity){const reward=c.reward;game.trade.credits+=reward;game.trade.turnover+=reward;game.trade.contractsCompleted=(game.trade.contractsCompleted||0)+1;adjustSettlementReliability(site,1);game.trade.contract=null;return reward;}return 0;
}
...
function cargoMoveMultiplier(routeActive=false){const effective=Math.max(0,(cargoLoadRatio()-.2)/.8),routeMitigation=routeActive?.35:1,beast=activePackbeast(),beastDrag=beast&&!beast.stranded?PACKBEAST_FULL_SPEED_PENALTY*packbeastLoadRatio()*(routeActive?.45:1):0;return Math.max(.76,1-FREIGHT_FULL_SPEED_PENALTY*effective*routeMitigation-beastDrag);}
function cargoNightSpawnFactor(routeActive=false){if(routeActive||game.nightState!=='night')return 1;const convoyPressure=Math.max(cargoLoadRatio(),packbeastLoadRatio()*.82);return 1-FREIGHT_NIGHT_THREAT*convoyPressure;}
...
function packbeastRoom(n=1){const beast=joinedPackbeast();return !!beast&&joinedPackbeastUnits()+Math.max(1,Math.floor(Number(n)||1))<=PACKBEAST_CAP;}
function packbeastAdd(id,n=1){const beast=activePackbeast(),amount=Math.max(1,Math.floor(Number(n)||1));if(!TRADE_GOODS[id]||!beast||beast.stranded||!packbeastRoom(amount))return false;beast.contents=beast.contents&&typeof beast.contents==='object'?beast.contents:{};beast.contents[id]=packbeastCount(id)+amount;game.saveDirty=true;return true;}
function packbeastTake(id,n=1){const beast=joinedPackbeast(),amount=Math.max(1,Math.floor(Number(n)||1));if(!beast||packbeastCount(id)<amount)return false;beast.contents[id]-=amount;if(beast.contents[id]<=0)delete beast.contents[id];game.saveDirty=true;return true;}
function availableCargoCount(id){return count(id)+packbeastCount(id);}
function consumeCargoUnit(id){if(count(id)>0)return consume(id,1);return packbeastTake(id,1);}
...
function nearbyStrandedPackbeast(range=2.2){const beast=activePackbeast();if(!beast?.stranded)return null;return Math.hypot(beast.x-(game.player?.x||0),beast.y-(game.player?.y||0))<=range?beast:null;}
function rejoinPackbeast(){const beast=nearbyStrandedPackbeast();if(!beast)return false;beast.stranded=false;beast.facing=game.player?.facing||1;game.saveDirty=true;toast(tr('苔背驮兽重新跟队','Mossback packbeast rejoined the caravan'));return true;}
function toggleCargoBundle(){if(rejoinPackbeast())return true;return cargoLoad()>0?deployCargoBundle():recoverCargoBundle();}
...
function hirePackbeast(){
  const market=nearbySettlementMarket();if(!market)return;const reliability=settlementReliability(market.site);if(activePackbeast())return toast(tr('你已经有一头苔背驮兽','You already have a Mossback packbeast'));if(reliability<PACKBEAST_MIN_RELIABILITY)return toast(tr(`需要与该聚落建立至少 ${PACKBEAST_MIN_RELIABILITY}/5 可靠度`,`Requires at least ${PACKBEAST_MIN_RELIABILITY}/5 reliability with this settlement`));if(game.trade.credits<PACKBEAST_HIRE_COST)return toast(tr(`铸印不足 · 签领驮兽需要 ◆${PACKBEAST_HIRE_COST}`,`Not enough Forge Marks · packbeast costs ◆${PACKBEAST_HIRE_COST}`));game.trade.credits-=PACKBEAST_HIRE_COST;const p=game.player;game.packbeast={x:p.x-p.facing*1.8,y:p.y,hp:PACKBEAST_MAX_HP,maxHp:PACKBEAST_MAX_HP,contents:{},stranded:false,facing:p.facing};game.saveDirty=true;renderTrade();updateHud();toast(tr(`苔背驮兽加入商队 · 额外驮运 ${PACKBEAST_CAP} 件`,`Mossback packbeast joined · +${PACKBEAST_CAP} cargo hold`));
}
function loadWarehousePackbeast(id){const market=nearbyTradePost();if(!market||warehouseCount(market.beacon,id)<=0)return;if(!activePackbeast())return toast(tr('还没有苔背驮兽','No Mossback packbeast yet'));if(!packbeastRoom(1))return toast(tr(`驮兽货架已满 ${packbeastUnits()}/${PACKBEAST_CAP}`,`Packbeast hold full ${packbeastUnits()}/${PACKBEAST_CAP}`));if(!warehouseTake(market.beacon,id,1))return;packbeastAdd(id,1);renderTrade();updateHud();}
function storePackbeastCargo(id){const market=nearbyTradePost();if(!market||packbeastCount(id)<=0)return;if(warehouseUnits(market.beacon)>=WAREHOUSE_CAP)return toast(tr('据点仓库已满','Depot warehouse is full'));if(!packbeastTake(id,1))return;warehouseAdd(market.beacon,id,1);renderTrade();updateHud();}
function dropPackbeastCargo(beast){const units=bundleUnits(beast);if(units<=0)return;const maxHp=CARGO_BUNDLE_BASE_HP+units*CARGO_BUNDLE_HP_PER_UNIT;game.cargoBundles.push({x:beast.x,y:beast.y+.2,hp:maxHp,maxHp,contents:{...beast.contents}});game.cargoBundles=game.cargoBundles.slice(-4);}
function hurtPackbeast(amount,source='raider'){const beast=activePackbeast();if(!beast||amount<=0)return false;beast.hp=Math.max(0,beast.hp-amount);spawnDebris(beast.x,beast.y,source==='projectile'?'#a8c6a0':'#9b6f4c',5,.55);if(beast.hp<=0){dropPackbeastCargo(beast);game.packbeast=null;toast(tr('苔背驮兽倒下 · 货物散落在原地','Mossback packbeast fell · cargo was stranded on the trail'));}game.saveDirty=true;return true;}
function updatePackbeast(dt){const beast=activePackbeast();if(!beast||beast.stranded)return;const p=game.player,desired=p.x-p.facing*2.15,dx=desired-beast.x;beast.facing=Math.sign(dx)||beast.facing;if(Math.abs(dx)>6.5){beast.x=p.x-p.facing*2.1;beast.y=p.y;}else{const speed=Math.abs(dx)>2.4?4.4:2.35,nx=beast.x+Math.sign(dx)*speed*dt;if(!enemyCollides(nx,beast.y,.92,1.18))beast.x=nx;if(Math.abs(beast.y-p.y)>2.6)beast.y=p.y;else beast.y+=(p.y-beast.y)*Math.min(1,dt*4.2);}const safe=nearestSettlement(beast.x,beast.y,SETTLEMENT_SAFE_RADIUS)||campfireDistance(beast.x,beast.y)<4.1;if(safe&&beast.hp<beast.maxHp){beast.hp=Math.min(beast.maxHp,beast.hp+PACKBEAST_SAFE_RECOVERY_PER_SEC*dt);game.saveDirty=true;}}
function guardHireCost(){return GUARD_BASE_COST+Math.ceil(cargoDeclaredValue()*.08);}
function hireGuard(){
  const market=nearbyTradePost();if(!market)return;if(activeGuard())return toast(tr('护卫契约仍在生效','Escort contract is still active'));if(convoyCargoUnits()<=0)return toast(tr('先装好货，再决定是否雇佣护卫','Load cargo first, then decide whether to hire a guard'));const cost=guardHireCost();if(game.trade.credits<cost)return toast(tr(`铸印不足 · 雇佣护卫需要 ◆${cost}`,`Not enough Forge Marks · guard costs ◆${cost}`));game.trade.credits-=cost;const p=game.player;game.guard={x:p.x-p.facing*1.2,y:p.y,hp:GUARD_MAX_HP,maxHp:GUARD_MAX_HP,remaining:GUARD_CONTRACT_DAYS*180,attackCd:0,facing:p.facing};game.saveDirty=true;renderTrade();toast(tr(`已雇佣边境护卫 · ◆${cost} · 约 ${Math.round(GUARD_CONTRACT_DAYS*24)} 小时`,`Frontier guard hired · ◆${cost} · about ${Math.round(GUARD_CONTRACT_DAYS*24)} hours`));
}
...
function sellTradeGood(id){
  syncTradeDay();const market=nearbyTradePost(),good=TRADE_GOODS[id];if(!market||!good||availableCargoCount(id)<=0)return;const key=market.biome.id+':'+id,used=Number(game.trade.sold[key]||0),price=tradePrice(id,market,'sell');if(used>=TRADE_DAILY_SELL_CAP)return toast(tr('这里今日已收满这种货','This market has met today’s demand for that cargo'));if(!consumeCargoUnit(id))return;game.trade.credits+=price;game.trade.sold[key]=used+1;game.trade.volume++;game.trade.turnover+=price;game.saveDirty=true;renderTrade();renderInventory();renderHotbar();updateHud();
}
function sellSettlementGood(id){
  syncTradeDay();const market=nearbySettlementMarket(),good=TRADE_GOODS[id];if(!market||!good||availableCargoCount(id)<=0)return;
  const reliability=settlementReliability(market.site),demand=settlementDailyDemand(market.site,game.trade.day,reliability),key=settlementDemandKey(market.site,id),used=settlementSold(market.site,id),priority=id===demand.goodId,contract=matchingTransportContract(market.site,id),baseCap=priority?demand.quantity:SETTLEMENT_GENERAL_DEMAND_CAP,contractRemaining=contract?Math.max(0,contract.quantity-contract.delivered):0,cap=contract?Math.max(baseCap,used+contractRemaining):baseCap;
  if(used>=cap)return toast(tr('这个聚落今日已经收够这种货','This settlement has enough of that cargo for today'));
  const base=settlementBaseSellPrice(id,market),price=settlementSellPrice(base,market.site,id,game.trade.day,reliability),quote=settlementFrontierQuote(market.site,id);
  if(quote.affordable<price)return toast(tr('聚落金库暂时不足 · 等待商路回款或换个市场','Settlement treasury is short · wait for trade income or use another market'));
  if(!consumeCargoUnit(id))return;recordFrontierDelivery(game.frontier,market.site,id,1,price);game.trade.credits+=price;game.trade.sold[key]=used+1;game.trade.volume++;game.trade.turnover+=price;if(good.home!==market.site.biome)markMilestone('cross-region-delivery');
  const dailyBonus=priority&&used+1===demand.quantity?demand.completionBonus:0;if(dailyBonus){game.trade.credits+=dailyBonus;game.trade.turnover+=dailyBonus;adjustSettlementReliability(market.site,1);}const contractReward=deliverTransportContract(market.site,id);
  if(contractReward||dailyBonus){const total=contractReward+dailyBonus,label=contractReward&&dailyBonus?tr('运输委托 + 聚落订单完成','Transport contract + settlement order complete'):contractReward?tr('运输委托完成','Transport contract complete'):tr('聚落订单完成','Settlement order complete');toast(`${label} · ${tr('额外奖励','bonus')} ◆${total} · ${tr('可靠度','Reliability')} ${settlementReliability(market.site)}/${SETTLEMENT_MAX_RELIABILITY}`);}
  game.saveDirty=true;renderTrade();renderInventory();renderHotbar();updateHud();
}
function aidSettlement(site,id){
  const market=nearbySettlementMarket();if(!market||market.site.id!==site?.id||!TRADE_GOODS[id])return false;const quote=settlementFrontierQuote(site,id),needRatio=quote.shortage/Math.max(1,quote.target);if(needRatio<.08)return toast(tr('这里暂时没有明显短缺','This settlement has no meaningful shortage right now'));if(availableCargoCount(id)<=0)return toast(tr(`缺少可援助的 ${itemName(id,'zh')}`,`No ${itemName(id,'en')} available to donate`));if(!consumeCargoUnit(id))return false;const result=recordFrontierAid(game.frontier,site,id,1);if(!result)return false;game.trade.aid=game.trade.aid&&typeof game.trade.aid==='object'?game.trade.aid:{};const before=Math.max(0,Number(game.trade.aid[site.id])||0),after=before+1;game.trade.aid[site.id]=after;if(Math.floor(before/3)<Math.floor(after/3))adjustSettlementReliability(site,1);game.saveDirty=true;renderTrade();renderInventory();renderHotbar();updateHud();toast(tr(`无偿援助 · ${itemName(id,'zh')} ×1${result.atWar?' · 战时支援':''}`,`Aid delivered · ${itemName(id,'en')} ×1${result.atWar?' · wartime support':''}`));return true;
}
...
function registerBeacon(x,y){
  ...
  list.push({x:x+.5,y:y+.5,createdAt:Math.floor(Date.now()/1000),supply:0,warehouse:{}});game.infrastructure={beacons:list};game.saveDirty=true;markMilestone('first-beacon');updateObjective();return true;
}
...
function updateProgression(dt) {
  ...
  const firstTown=nearestSettlement(game.player.x,game.player.y,18);if(firstTown&&markMilestone('first-settlement'))toast(tr(`抵达 ${settlementName(firstTown,'zh')} · 这里的库存与需求会真实变化`,`Reached ${settlementName(firstTown,'en')} · its stock and demand change with the world`));
...
function resize() {
  ...
  ctx.setTransform(game.dpr,0,0,game.dpr,0,0);ctx.imageSmoothingEnabled=false;
...
function killEnemy(e) {
  e.dead=true; const type=e.type,frontierSite=nearestSettlement(e.x,e.y,45);if(!e.raidId)recordFrontierThreatRemoval(game.frontier,frontierSite||{biome:game.world.biome(e.x).id},e.boss?4:e.elite?2:1);if(e.raidId){const result=recordFrontierRaidResistance(game.frontier,e.raidId,1),site=result?(game.world?.settlements||[]).find(x=>x.id===result.defenderId):null;if(result?.repelled){if(site)adjustSettlementReliability(site,1);toast(tr(`你击溃了袭击队 · ${site?settlementName(site,'zh'):'聚落'}守住了`,`Raid repelled · ${site?settlementName(site,'en'):'the settlement'} holds`));game.fx.shake=Math.max(game.fx.shake,5);}else if(result)toast(tr('袭击队受损 · 继续拦截','Raid force weakened · keep intercepting'));game.saveDirty=true;}
...
function eatSelectedFood(){const id=selectedId(),item=ITEMS[id];if(item?.kind!=='food'||count(id)<1||game.attackCd>0)return false;const p=game.player;if(p.hunger>=HUNGER_MAX-1)return toast(tr('现在还不饿','You are not hungry enough yet'));consume(id,1);p.hunger=Math.min(HUNGER_MAX,p.hunger+Math.max(1,Number(item.nourish)||1));p.starveTick=0;game.attackCd=.35;game.saveDirty=true;markMilestone('first-meal');renderHotbar();renderInventory();toast(tr(`进食 · ${itemName(id,'zh')} · 饱食 ${Math.round(p.hunger)}/${HUNGER_MAX}`,`Ate ${itemName(id,'en')} · Hunger ${Math.round(p.hunger)}/${HUNGER_MAX}`));return true;}
...
function updateDrops(dt){
  ...
  for(const d of game.drops){
    ...
    if(dist<.72&&d.age>.1){if(!addItem(d.id,d.n)){if(game.uiOpen)toast(tr('物品栏已满','Inventory full'));continue;}d.dead=true;sfx('pickup',ITEMS[d.id]?.rare?1.25:.55);spawnDebris(d.x,d.y,ITEMS[d.id]?.rare?'#e9c16b':'#f0d9a2',ITEMS[d.id]?.rare?8:3,ITEMS[d.id]?.rare?.75:.35);haptic(ITEMS[d.id]?.rare?18:4);if(ITEMS[d.id]?.rare)toast(tr(`发现稀有装备 · ${itemName(d.id,'zh')}`,`Rare gear found · ${itemName(d.id,'en')}`));else if(d.id==='ancient_core')toast(tr('获得古代机芯','Ancient Core acquired'));renderHotbar();renderInventory();}
  }
...
function spawnEnemy() {
  ...
  if(!underground&&nearestSettlement(p.x,p.y,SETTLEMENT_SAFE_RADIUS))return;
...
function updateEnemyProjectiles(dt){
  for(const p of game.enemyProjectiles){p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;const ward=campfireDistance(p.x,p.y)<4.1;if(ward){p.dead=true;spawnDebris(p.x,p.y,'#72d7b8',2,.25);continue;}if(p.life<=0||game.world.solid(Math.floor(p.x),Math.floor(p.y))){p.dead=true;continue;}const cargo=nearestCargoBundle(p.x,p.y,.62);if(cargo){damageCargoBundle(cargo,p.damage,'enemy');p.dead=true;continue;}const beast=activePackbeast();if(beast&&Math.hypot(p.x-beast.x,p.y-beast.y)<.72){hurtPackbeast(p.damage*.9,'projectile');p.dead=true;continue;}if(playerHitboxContains(p.x,p.y)){hurtPlayer(p.damage,'wisp_bolt');p.dead=true;game.fx.shake=Math.max(game.fx.shake,3.2);spawnDebris(game.player.x,game.player.y,'#66c7ae',7,.7);}}
  game.enemyProjectiles=game.enemyProjectiles.filter(p=>!p.dead&&p.life>0);
}
function syncLocalRaidActors(){
  const raids=game.frontier?.raids||[],liveIds=new Set(raids.map(r=>r.id));for(const e of game.enemies)if(e.raidId&&!liveIds.has(e.raidId))e.dead=true;if(!raids.length||currentDepth()>6)return;const raid=raids[0],sites=game.world?.settlements||[],a=sites.find(x=>x.id===raid.from),b=sites.find(x=>x.id===raid.to);if(!a||!b)return;const t=Math.max(0,Math.min(1,Number(raid.progress)||0)),wx=a.x+(b.x-a.x)*t,tx=Math.max(2,Math.min(WORLD_W-3,Math.floor(wx))),wy=game.world.surface[tx]-.35;if(Math.hypot(game.player.x-wx,game.player.y-wy)>16)return;const active=game.enemies.filter(e=>!e.dead&&e.raidId===raid.id);const desired=Math.min(3,Math.max(1,Math.ceil(Number(raid.strength)||1)));if(active.length>=desired)return;const base=ENEMY_TYPES.greyveil_raider,palette=a.biome==='verdant'?'#789b69':a.biome==='ember'?'#b2664b':'#6f9db2';for(let i=active.length;i<desired;i++){const side=(i%2?1:-1),x=Math.max(2,Math.min(WORLD_W-3,wx+side*(1.2+i*.65))),sx=Math.floor(x),y=game.world.surface[sx]-.3,scale=.92+Math.min(.28,(Number(raid.strength)||1)*.07),def={...base,zh:`${settlementName(a,'zh')}袭击者`,en:`${settlementName(a,'en')} Raider`,hp:Math.round(base.hp*scale),damage:Math.max(4,Math.round(base.damage*(.9+scale*.12))),speed:base.speed*.94,color:palette};game.enemies.push({type:'faction_raider',def,x,y,vx:0,vy:0,hp:def.hp,maxHp:def.hp,dead:false,hit:0,attack:.25,flying:false,dir:b.x>a.x?1:-1,raidId:raid.id,raidFrom:raid.from,raidTo:raid.to});}}
function updateEnemies(dt) {
  game.raiderCooldown=Math.max(0,(game.raiderCooldown||0)-dt);updateGuard(dt);updatePackbeast(dt);syncLocalRaidActors();const p=game.player,night=game.nightState==='night',fire=nearbyStationTile('campfire');game.spawnTimer-=dt;if(game.spawnTimer<=0){const route=routeStateAt(p.x,p.y),relief=night&&route.active?ROUTE_NIGHT_SPAWN_RELIEF:1,freightThreat=cargoNightSpawnFactor(route.active),phase=night?'night':dayLight()<.58?'dusk':'day',ecologyThreat=frontierThreatFactor(game.frontier,game.world.biome(p.x).id);game.spawnTimer=nextEncounterDelay({phase,surge:game.nightSurge>0,rng:game.rng,routeRelief:relief,freightFactor:freightThreat})/Math.max(.72,ecologyThreat);spawnEnemy();}
  for(const e of game.enemies){if(e.dead)continue;e.hit=Math.max(0,e.hit-dt);e.attack=Math.max(0,e.attack-dt);const g=!e.elite?activeGuard():null,cargoTarget=e.type==='greyveil_raider'?nearestCargoBundle(e.x,e.y,18):null,beastTarget=e.type==='greyveil_raider'&&joinedPackbeastUnits()>0?joinedPackbeast():null,playerDist=Math.hypot(p.x-e.x,p.y-e.y),guardDist=g?Math.hypot(g.x-e.x,g.y-e.y):Infinity,beastDist=beastTarget?Math.hypot(beastTarget.x-e.x,beastTarget.y-e.y):Infinity,target=cargoTarget||((beastTarget&&beastDist<playerDist*1.15)?beastTarget:((g&&guardDist<playerDist*.92)?g:p)),dx=target.x-e.x,dy=target.y-e.y,dist=Math.hypot(dx,dy);if(Math.min(playerDist,dist)>30)continue;
    ...
    if(dist<1.05&&e.attack<=0){if(cargoTarget)damageCargoBundle(cargoTarget,e.def.damage*1.15,'raider');else if(beastTarget&&target===beastTarget)hurtPackbeast(e.def.damage*1.05,'raider');else if(target===g)hurtGuard(e.def.damage);else hurtPlayer(e.def.damage,e.type);e.attack=e.elite?1.05:.85;}
  }
...
function die(source) {
  const strandedCargo=cargoLoad();if(strandedCargo>0)deployCargoBundle();const beast=activePackbeast();if(beast&&!beast.stranded){beast.x=game.player.x-game.player.facing*.9;beast.y=game.player.y;beast.stranded=true;}game.input.left=game.input.right=game.input.jump=game.input.mine=game.input.place=false;game.running=false;
  ...
  $('#deathText').textContent=(strandedCargo>0?tr(`货物留在倒下的位置（${strandedCargo} 件）。`,`Your cargo remains where you fell (${strandedCargo} units). `):'')+(beast?tr(`苔背驮兽也留在原地（驮载 ${packbeastUnits()}/${PACKBEAST_CAP}）。`,`Your Mossback also remains there (${packbeastUnits()}/${PACKBEAST_CAP} cargo). `):'')+(lost.length?tr('撤回营地时另遗失：','Other supplies lost while retreating: ')+lost.slice(0,4).join(' · '):tr('其余随身物资已保住。','Other carried supplies were kept.'));$('#deathScreen').classList.remove('hidden');
}
...
function craft(r) {
  ...
  if(!canStoreItem(r.out.id,r.out.n)){toast(tr('物品栏没有足够空间','Not enough inventory space'));return;}
  for(const [id,n] of Object.entries(r.need))consume(id,n);addItem(r.out.id,r.out.n);if(r.out.id==='workbench')markMilestone('crafted-workbench');if(ITEMS[r.out.id]?.kind==='pick')markMilestone('crafted-pick');if(r.out.id==='campfire')markMilestone('crafted-campfire');if(r.out.id==='beacon')markMilestone('crafted-beacon');if(['pick','weapon'].includes(ITEMS[r.out.id]?.kind))game.hotbar[game.selected]=r.out.id;renderInventory();renderCraft();renderHotbar();toast(r.out.id==='beacon'?tr('边境路标完成 · 建站后可采购本地货，再运往异地出售','Frontier Beacon ready · establish a post, load local cargo, then sell it abroad'):`${tr('制造','Crafted')} · ${itemName(r.out.id,lang)} ×${r.out.n}`);
}
function renderInventory() {
  renderInventorySlots({view:$('#inventoryView'),inventory:game.inventory,layout:game.inventoryLayout,items:ITEMS,itemName,lang,glyph:GLYPH,selectedId,onEquip:equipToHotbar,onMove:moveInventoryItem});
}
...
function renderSettlementTrade(view,market){
  const site=market.site,reliability=settlementReliability(site),preference=settlementLongTermPreference(site),demand=settlementDailyDemand(site,game.trade.day,reliability),name=settlementName(site,lang),soldPriority=settlementSold(site,demand.goodId),remaining=settlementDemandRemaining(site,game.trade.day,soldPriority,reliability),load=cargoLoad(),cap=cargoCapacity(),beast=activePackbeast(),beastLoad=packbeastUnits(),contract=activeTransportContract(),contractSite=transportContractSite(contract),contractHere=!!contract&&contract.destinationId===site.id,simTown=frontierSettlement(game.frontier,site);
  const liveNeeds=Object.keys(TRADE_GOODS).map(id=>{const q=settlementFrontierQuote(site,id);return {id,q,ratio:q.shortage/Math.max(1,q.target)};}).sort((a,b)=>b.ratio-a.ratio),liveNeed=liveNeeds[0],liveNeedCard=liveNeed?`<div class="settlement-order live-shortage"><span>${tr('实时短缺','LIVE SHORTAGE')}</span><b>${itemName(liveNeed.id,lang)} · ${tr('缺口','Shortage')} ${Math.ceil(liveNeed.q.shortage)}</b><small>${tr(`真实库存 ${Math.round(liveNeed.q.stock)}/${Math.round(liveNeed.q.target)} · 市场 ×${liveNeed.q.multiplier.toFixed(2)}`,`Live stock ${Math.round(liveNeed.q.stock)}/${Math.round(liveNeed.q.target)} · market ×${liveNeed.q.multiplier.toFixed(2)}`)}</small></div>`:'';
  const contractCard=contract?`<div class="transport-contract ${contractHere?'active-destination':''}"><div><span>${tr('运输委托','Transport contract')}</span><b>${contractSite?settlementName(contractSite,lang):tr('目标聚落','Destination')} · ${itemName(contract.goodId,lang)} ×${contract.quantity}</b><small>${contractHere?tr(`已交付 ${contract.delivered}/${contract.quantity} · 剩 ${contractDaysLeft(contract,game.trade.day)} 日`,`Delivered ${contract.delivered}/${contract.quantity} · ${contractDaysLeft(contract,game.trade.day)} days left`):tr(`目标距此 ${Math.ceil(Math.abs((contractSite?.x||contract.destinationX)-site.x))}m · 剩 ${contractDaysLeft(contract,game.trade.day)} 日`,`Destination ${Math.ceil(Math.abs((contractSite?.x||contract.destinationX)-site.x))}m away · ${contractDaysLeft(contract,game.trade.day)} days left`)}</small></div><strong>+◆${contract.reward}</strong></div>`:'';
  const rows=Object.entries(TRADE_GOODS).map(([id,good])=>{const priority=id===demand.goodId,preferred=id===preference,local=good.home===site.biome,contractMatch=matchingTransportContract(site,id),used=settlementSold(site,id),baseLimit=priority?demand.quantity:SETTLEMENT_GENERAL_DEMAND_CAP,contractRemaining=contractMatch?Math.max(0,contractMatch.quantity-contractMatch.delivered):0,limit=contractMatch?Math.max(baseLimit,used+contractRemaining):baseLimit,left=Math.max(0,limit-used),quote=settlementFrontierQuote(site,id),base=settlementBaseSellPrice(id,market),price=settlementSellPrice(base,site,id,game.trade.day,reliability),owned=availableCargoCount(id),beastHeld=packbeastCount(id),detail=contractMatch?tr(`运输委托 · ${contractMatch.delivered}/${contractMatch.quantity}`,`TRANSPORT CONTRACT · ${contractMatch.delivered}/${contractMatch.quantity}`):priority?tr(`今日采购订单 · 溢价 +${Math.round((demand.premium-1)*100)}%`,`TODAY'S ORDER · +${Math.round((demand.premium-1)*100)}% premium`):preferred?tr(`长期偏好 · 稳定溢价 +${Math.round((SETTLEMENT_PREFERENCE_PREMIUM-1)*100)}%`,`LONG-TERM PREFERENCE · +${Math.round((SETTLEMENT_PREFERENCE_PREMIUM-1)*100)}% stable premium`):local?tr('本地产物 · 低价消化','Local surplus · low-value intake'):tr('常规异地需求','Regular imported demand'),marketState=tr(`真实库存 ${Math.round(quote.stock)}/${Math.round(quote.target)} · 市场 ×${quote.multiplier.toFixed(2)}`,`Live stock ${Math.round(quote.stock)}/${Math.round(quote.target)} · market ×${quote.multiplier.toFixed(2)}`),cashOk=quote.affordable>=price;return `<div class="trade-row ${contractMatch?'contract-cargo':priority?'priority-demand':preferred?'preferred-demand':local?'export':'demand'}"><div class="trade-good"><b>${GLYPH[id]||'◆'} ${itemName(id,lang)}</b><small>${detail}</small><small class="market-live">${marketState}</small></div><div class="trade-owned">${tr('可交付','Deliverable')} ${owned}${beast?` · ${tr('驮','Beast')} ${beastHeld}`:''}</div><div class="trade-actions"><button data-settlement-sell="${id}" ${owned<=0||left<=0||!cashOk?'disabled':''}>${cashOk?tr('交付','Deliver'):tr('金库不足','Treasury low')} ◆${price}<small>${left}</small></button></div></div>`;}).join('');
  const memory=`<div class="settlement-memory"><div><span>${tr('长期偏好','Long-term preference')}</span><b>${itemName(preference,lang)} · +${Math.round((SETTLEMENT_PREFERENCE_PREMIUM-1)*100)}%</b></div><div><span>${tr('可靠度','Reliability')}</span><b>${settlementReliabilityPips(site)} ${reliability}/${SETTLEMENT_MAX_RELIABILITY}</b></div><small>${tr('商誉仍记录你与聚落的长期关系；实时库存、商队与生态则决定当前市场压力。','Standing remains your long-term relationship; live stock, caravans and ecology now shape current market pressure.')}</small></div>`;
  const simStrip=simTown?`<div class="logistics-strip frontier-live"><span>${tr('人口','Population')} <b>${simTown.population}</b></span><span>${tr('活力','Vitality')} <b>${Math.round(simTown.wellbeing)}</b></span><span>${tr('安全','Safety')} <b>${Math.round(simTown.safety)}</b></span><span>${tr('外交','Diplomacy')} <b>${settlementDiplomacySummary(site)}</b></span></div>`:'';
  const aidOptions=Object.keys(TRADE_GOODS).map(id=>{const q=settlementFrontierQuote(site,id);return {id,q,ratio:q.shortage/Math.max(1,q.target),owned:availableCargoCount(id)};}).sort((a,b)=>b.ratio-a.ratio),aid=aidOptions.find(x=>x.owned>0&&x.ratio>=.08)||aidOptions[0],aidVisible=!!aid&&(aid.ratio>=.22||settlementAtWar(site)||['weakened','collapsing'].includes(simTown?.status)),aidCard=aidVisible?`<div class="settlement-aid"><div><span>${settlementAtWar(site)?tr('战时支援','Wartime aid'):tr('紧急援助','Emergency aid')}</span><b>${itemName(aid.id,lang)} · ${tr('缺口','Shortage')} ${Math.ceil(aid.q.shortage)}</b><small>${tr('无偿交付会直接补入真实库存；每 3 次有效援助提升一次可靠度。','Donation enters real stock with no payment; every 3 effective aid deliveries improves reliability.')}</small></div><button data-settlement-aid="${aid.id}" ${aid.owned<=0||aid.ratio<.08?'disabled':''}>${tr('援助 1','Aid 1')} · ${tr('持有','Own')} ${aid.owned}</button></div>`:'';
  const packbeastPanel=beast?`<div class="packbeast-hire active"><div><span>${tr('苔背驮兽','Mossback Packbeast')}</span><b>${tr('驮载','Hold')} ${beastLoad}/${PACKBEAST_CAP} · HP ${Math.ceil(beast.hp)}/${beast.maxHp}</b><small>${beast.stranded?tr('驮兽已离队：回到它身边按 R / 货包键重新编队。','Packbeast is stranded: return to it and press R / Cargo to regroup.'):tr('驮兽货物不占用双手；整支商队货值仍会吸引劫徒。','Pack cargo leaves your hands free; total convoy value still attracts raiders.')}</small></div></div>`:`<div class="packbeast-hire"><button data-hire-packbeast ${reliability<PACKBEAST_MIN_RELIABILITY?'disabled':''}>${reliability<PACKBEAST_MIN_RELIABILITY?tr(`可靠度 ${PACKBEAST_MIN_RELIABILITY}/5 解锁驮兽`,`Unlock packbeast at ${PACKBEAST_MIN_RELIABILITY}/5 reliability`):tr(`签领苔背驮兽 ◆${PACKBEAST_HIRE_COST}`,`Acquire Mossback Packbeast ◆${PACKBEAST_HIRE_COST}`)}</button><small>${tr(`独立驮载 ${PACKBEAST_CAP} 件；双手可继续战斗，但驮兽会成为劫掠目标。`,`Separate ${PACKBEAST_CAP}-unit hold; your hands stay free, but the beast becomes a raid target.`)}</small></div>`;
  view.innerHTML=`<div class="trade-head settlement-head"><div><b>⌂ ${name} · ${lang==='zh'?market.biome.zh:market.biome.en}</b><small>${tr('活体聚落市场 · 周边受保护','Living settlement market · protected perimeter')}</small></div><strong>◆ ${game.trade.credits}</strong></div>${simStrip}${liveNeedCard}${memory}${aidCard}${packbeastPanel}${contractCard}<div class="settlement-order"><span>${tr('今日采购订单','Daily procurement')}</span><b>${itemName(demand.goodId,lang)} ×${remaining}</b><small>${tr(`完成整单另奖 ◆${demand.completionBonus}`,`Complete the order for +◆${demand.completionBonus}`)}</small></div><div class="logistics-strip"><span>${tr('随身货运','Carried')} <b>${load}/${cap}</b></span><span>${tr('驮兽','Packbeast')} <b>${beast?beastLoad+'/'+PACKBEAST_CAP:tr('无','None')}</b></span><span>${tr('订单剩余','Order left')} <b>${remaining}</b></span><span>${tr('天气','Weather')} <b>${frontierWeatherName(game.frontier,lang)}</b></span></div><div class="trade-note">${tr('价格现在同时受库存、聚落生态与旧有商誉/急需影响。你交付的货物会真实进入这里的库存，AI 商队也会搬运同一批货。','Prices now combine live stock pressure with standing and daily orders. Your deliveries enter the same inventory used by autonomous caravans.')}</div><div class="trade-list">${rows}</div>`;
  view.querySelector('[data-settlement-aid]')?.addEventListener('click',b=>aidSettlement(site,b.currentTarget.dataset.settlementAid));view.querySelector('[data-hire-packbeast]')?.addEventListener('click',hirePackbeast);view.querySelectorAll('[data-settlement-sell]').forEach(b=>b.onclick=()=>sellSettlementGood(b.dataset.settlementSell));
}
function renderTrade(){
  const view=$('#tradeView');if(!view)return;if(!game.world||!game.player){view.innerHTML='';return;}syncTradeDay();const settlement=nearbySettlementMarket();if(settlement){renderSettlementTrade(view,settlement);return;}const market=nearbyTradePost();if(!market){view.innerHTML=`<div class="trade-empty"><b>${tr('附近没有贸易节点','No trade node nearby')}</b><span>${tr('靠近你的边境路标可生产、入库和装货；抵达聚落则可查看每日需求并交付。','Use your Frontier Beacons to produce, store and load cargo; reach a settlement to see daily demand and deliver it.')}</span></div>`;return;}
  const route=market.linked?tr(`路网 ${Math.max(1,market.biomeCount)} 地`,`Route · ${Math.max(1,market.biomeCount)} biomes`):tr('孤立路标','Isolated beacon'),load=cargoLoad(),cap=cargoCapacity(),warehouse=warehouseUnits(market.beacon),bench=nearStation('workbench'),guard=activeGuard(),guardCost=guardHireCost(),beast=activePackbeast(),beastLoad=packbeastUnits(),contract=activeTransportContract(),contractSite=transportContractSite(contract),offer=transportOfferForMarket(market),offerSite=offer?(game.world.settlements||[]).find(site=>site.id===offer.destinationId):null;
  const contractPanel=contract?`<div class="transport-contract active"><div><span>${tr('执行中','ACTIVE CONTRACT')}</span><b>${contractSite?settlementName(contractSite,lang):tr('目标聚落','Destination')} · ${itemName(contract.goodId,lang)} ×${contract.quantity}</b><small>${tr(`已交付 ${contract.delivered}/${contract.quantity} · 剩 ${contractDaysLeft(contract,game.trade.day)} 日`,`Delivered ${contract.delivered}/${contract.quantity} · ${contractDaysLeft(contract,game.trade.day)} days left`)}</small></div><strong>+◆${contract.reward}</strong></div>`:offer?`<div class="transport-contract offer"><div><span>${tr('今日运输委托','TODAY’S TRANSPORT')}</span><b>${itemName(offer.goodId,lang)} ×${offer.quantity} → ${offerSite?settlementName(offerSite,lang):tr('异地聚落','Distant settlement')}</b><small>${tr(`${offer.distance}m · ${TRANSPORT_CONTRACT_DAYS} 日期限 · 目标可靠度 ${offer.destinationReliability||0}/${SETTLEMENT_MAX_RELIABILITY}`,`${offer.distance}m · ${TRANSPORT_CONTRACT_DAYS}-day deadline · target reliability ${offer.destinationReliability||0}/${SETTLEMENT_MAX_RELIABILITY}`)}</small></div><strong>+◆${offer.reward}</strong><button data-accept-contract>${tr('接取委托','Accept')}</button></div>`:'';
  const rows=Object.entries(TRADE_GOODS).map(([id,good])=>{const local=good.home===market.biome.id,buy=tradePrice(id,market,'buy'),sell=tradePrice(id,market,'sell'),bought=Number(game.trade.bought[id]||0),sold=Number(game.trade.sold[market.biome.id+':'+id]||0),owned=count(id),beastHeld=packbeastCount(id),stored=warehouseCount(market.beacon,id),recipe=TRADE_PRODUCTION[id],produced=Number(game.trade.produced[productionKey(market,id)]||0),need=recipe?Object.entries(recipe.need).map(([mat,n])=>`${itemName(mat,lang)} ${count(mat)}/${n}`).join(' · '):'',canProduce=local&&bench&&canProduceTradeGood(id,market);return `<div class="trade-row ${local?'export':'demand'}"><div class="trade-good"><b>${GLYPH[id]||'◆'} ${itemName(id,lang)}</b><small>${local?tr('本地货物 · 可生产/采购','Local cargo · produce or buy'):tr('异地需求 · 高价收购','Imported demand · premium buyback')}</small>${local?`<small class="produce-need">${tr('生产','Make')}: ${need}</small>`:''}</div><div class="trade-owned">${tr('随身','Pack')} ${owned}${beast?` · ${tr('驮','Beast')} ${beastHeld}`:''} · ${tr('仓','Depot')} ${stored}</div><div class="trade-actions">${local?`<button data-trade-produce="${id}" ${canProduce?'':'disabled'}>${tr('生产','Make')}<small>${TRADE_DAILY_PRODUCTION_CAP-produced}</small></button><button data-trade-buy="${id}" ${bought>=TRADE_DAILY_BUY_CAP||!cargoRoom(id,1)?'disabled':''}>${tr('采购','Buy')} ◆${buy}<small>${TRADE_DAILY_BUY_CAP-bought}</small></button>`:''}<button data-trade-load="${id}" ${stored<=0||!cargoRoom(id,1)?'disabled':''}>${tr('装货','Load')}</button>${beast?`<button data-trade-load-beast="${id}" ${stored<=0||!packbeastRoom(1)?'disabled':''}>${tr('驮装','Beast')}</button><button data-trade-store-beast="${id}" ${beastHeld<=0||warehouse>=WAREHOUSE_CAP?'disabled':''}>${tr('驮回','Return')}</button>`:''}<button data-trade-store="${id}" ${owned<=0||warehouse>=WAREHOUSE_CAP?'disabled':''}>${tr('入库','Store')}</button><button data-trade-sell="${id}" ${owned+beastHeld<=0||sold>=TRADE_DAILY_SELL_CAP?'disabled':''}>${tr('出售','Sell')} ◆${sell}<small>${TRADE_DAILY_SELL_CAP-sold}</small></button></div></div>`;}).join('');
  view.innerHTML=`<div class="trade-head"><div><b>${lang==='zh'?market.biome.zh:market.biome.en} · ${tr('边境货站','Frontier Depot')}</b><small>${route}${market.linked?tr(' · 联网成交 +6%',' · linked demand +6%'):''} · ${tr('委托完成','Contracts')} ${game.trade.contractsCompleted||0}</small></div><strong>◆ ${game.trade.credits}</strong></div>${contractPanel}<div class="logistics-strip"><span>${tr('随身货运','Carried')} <b>${load}/${cap}</b>${count('freight_frame')>0?` · ${tr('背架','Frame')}`:''}</span><span>${tr('驮兽','Packbeast')} <b>${beast?beastLoad+'/'+PACKBEAST_CAP:tr('无','None')}</b></span><span>${tr('仓库','Depot')} <b>${warehouse}/${WAREHOUSE_CAP}</b></span><span>${tr('工匠台','Craft Table')} <b>${bench?tr('在线','Ready'):tr('缺失','Missing')}</b></span><span>${tr('护卫','Guard')} <b>${guard?Math.ceil(guard.hp)+'/'+guard.maxHp:tr('无','None')}</b></span></div><div class="escort-hire"><button data-hire-guard ${guard||convoyCargoUnits()<=0?'disabled':''}>${guard?tr('护卫契约生效中','Guard on contract'):convoyCargoUnits()<=0?tr('先装货再雇护卫','Load cargo before hiring'):tr(`雇佣护卫 ◆${guardCost}`,`Hire guard ◆${guardCost}`)}</button><small>${tr('护卫会跟随并近战拦截普通敌人；契约持续约一昼夜半。','Guard follows and intercepts ordinary hostiles for about 1.5 days.')}</small></div><div class="trade-note">${tr('本地原料可在工匠台打包成标准货物并直接入库；运输委托不会瞬移货物，仍必须装货后亲自穿过荒野抵达指定聚落。','Pack local materials at a Craft Table. Transport contracts never teleport cargo: load it, cross the frontier, and deliver it to the named settlement.')}</div><div class="trade-list">${rows}</div>`;
  view.querySelector('[data-accept-contract]')?.addEventListener('click',acceptTransportContract);view.querySelector('[data-hire-guard]')?.addEventListener('click',hireGuard);view.querySelectorAll('[data-trade-produce]').forEach(b=>b.onclick=()=>produceTradeGood(b.dataset.tradeProduce));view.querySelectorAll('[data-trade-buy]').forEach(b=>b.onclick=()=>buyTradeGood(b.dataset.tradeBuy));view.querySelectorAll('[data-trade-load]').forEach(b=>b.onclick=()=>loadWarehouseCargo(b.dataset.tradeLoad));view.querySelectorAll('[data-trade-load-beast]').forEach(b=>b.onclick=()=>loadWarehousePackbeast(b.dataset.tradeLoadBeast));view.querySelectorAll('[data-trade-store-beast]').forEach(b=>b.onclick=()=>storePackbeastCargo(b.dataset.tradeStoreBeast));view.querySelectorAll('[data-trade-store]').forEach(b=>b.onclick=()=>storeWarehouseCargo(b.dataset.tradeStore));view.querySelectorAll('[data-trade-sell]').forEach(b=>b.onclick=()=>sellTradeGood(b.dataset.tradeSell));
}
...
function updateHud(){
  ...
  const beacon=nearestBeacon(),route=routeStateAt(p.x,p.y),supply=totalBeaconSupply(),contract=activeTransportContract(),contractSite=transportContractSite(contract),settlement=contractSite?null:nearestSettlement(p.x,p.y,90),destination=contractSite?` · ${onboardingDirection(contractSite)} ${settlementName(contractSite,lang)} ${contract.delivered}/${contract.quantity}`:settlement?` · ⌂${settlementName(settlement,lang)} ${Math.ceil(settlement.d)}m`:'';
  const routeText=(route.active?tr(`补给路 +8% · ${route.biomeCount}地 · ${supply}`,`SUPPLY ROUTE +8% · ${route.biomeCount} biomes · ${supply}`):beacon?tr(`路标 ${Math.ceil(beacon.d)}m · 线${route.links.length}`,`Beacon ${Math.ceil(beacon.d)}m · L${route.links.length}`):tr('无路标','No beacon'))+destination+` · ◆${Math.max(0,Math.floor(game.trade?.credits||0))} · ▣${cargoLoad()}/${cargoCapacity()}${cargoLoadRatio()>.75?tr(' 重载',' HEAVY'):''}${activeGuard()?tr(' · ⚔护卫',' · ⚔GUARD'):''}${activePackbeast()?tr(` · 驮${packbeastUnits()}/${PACKBEAST_CAP}${activePackbeast().stranded?'离队':''}`,` · BEAST ${packbeastUnits()}/${PACKBEAST_CAP}${activePackbeast().stranded?' STRANDED':''}`):''}`;
  const beaconEl=$('#beaconText');if(beaconEl){beaconEl.textContent=routeText;beaconEl.classList.toggle('route-active',route.active);beaconEl.classList.toggle('has-supply',supply>0);}const cargoBtn=$('#mobileCargo');if(cargoBtn)cargoBtn.classList.toggle('relevant',cargoLoad()>0||(game.cargoBundles?.length||0)>0||!!activePackbeast()?.stranded);const placeBtn=$('#mobilePlace'),held=ITEMS[selectedId()];if(placeBtn)placeBtn.classList.toggle('relevant',held?.tile!==undefined);
  const danger=$('#dangerText'),settlementSafe=nearestSettlement(p.x,p.y,SETTLEMENT_SAFE_RADIUS);if(danger){danger.textContent=settlementSafe?tr('聚落守望','SETTLEMENT WARD'):game.nightState==='night'?(game.nightSurge>0?tr('夜袭高压','NIGHT SURGE'):tr('夜袭','NIGHT WATCH')):game.nightState==='dusk'?tr('守夜准备','PREPARE'):tr('安全','SAFE');danger.classList.toggle('active',!settlementSafe&&game.nightState==='night');danger.classList.toggle('warning',!settlementSafe&&game.nightState==='dusk');danger.title=[settlementSafe?tr('聚落安全圈内','Inside settlement safe perimeter'):fireNotice(),routeText].filter(Boolean).join(' · ');}
}
...
function visualHash(n,salt=0){let x=(Math.imul((Math.floor(n)||0)^salt,0x45d9f3b)^0x9e3779b9)>>>0;x^=x>>>16;x=Math.imul(x,0x45d9f3b)>>>0;x^=x>>>16;return x/4294967295;}
function drawBiomeAtmosphere(){
  if(!game.world||!game.player||currentDepth()>8)return;const s=game.camera.tile,b=game.world.biome(game.player.x),w=game.cssW,h=game.cssH,now=performance.now()*.001,light=dayLight();
  ctx.save();
  if(b.id==='verdant'){
    for(let i=0;i<28;i++){const seed=visualHash(i,0x51a7),speed=10+visualHash(i,0x91b3)*18,x=wrapScreen(seed*w*1.45+now*speed-game.camera.x*s*.045,w+80)-40,y=h*(.16+visualHash(i,0x711d)*.66)+Math.sin(now*.7+i)*7;ctx.globalAlpha=.10+.18*visualHash(i,0x2211);ctx.fillStyle=i%4===0?'#c6df8b':'#8fc69a';const q=2+(i%3);ctx.save();ctx.translate(x,y);ctx.rotate(now*.45+i);ctx.fillRect(-q*.65,-1,q*1.3,2);ctx.restore();}
    ctx.globalAlpha=.012+.018*light;ctx.fillStyle='#d9f0b5';for(let i=0;i<4;i++){const x=wrapScreen(i*397-game.camera.x*s*.10,w+280)-140,shaft=16+i*3;ctx.beginPath();ctx.moveTo(x-shaft*.28,0);ctx.lineTo(x+shaft*.72,0);ctx.lineTo(x+shaft*1.45,h*.68);ctx.lineTo(x+shaft*.18,h*.68);ctx.closePath();ctx.fill();}
  }else if(b.id==='ember'){
    for(let i=0;i<34;i++){const seed=visualHash(i,0x9e21),x=wrapScreen(seed*w*1.6-now*(8+visualHash(i,0x117)*13)-game.camera.x*s*.03,w+70)-35,y=h*(.20+visualHash(i,0x771)*.66)-Math.sin(now*.8+i)*5,q=1+(i%3);ctx.globalAlpha=.08+.18*visualHash(i,0x553);ctx.fillStyle=i%7===0?'#dc8b5c':'#80645a';ctx.fillRect(Math.round(x),Math.round(y),q,q);}
    ctx.globalAlpha=.055;ctx.strokeStyle='#d38b68';ctx.lineWidth=1;for(let i=0;i<6;i++){const x=wrapScreen(i*239-game.camera.x*s*.08,w+180)-90;ctx.beginPath();for(let y=h*.48;y<h*.92;y+=18){const wobble=Math.sin(now*1.7+y*.025+i)*5;ctx.lineTo(x+wobble,y);}ctx.stroke();}
  }else{
    for(let i=0;i<38;i++){const seed=visualHash(i,0xf051),speed=12+visualHash(i,0x337)*20,x=wrapScreen(seed*w*1.35+now*speed-game.camera.x*s*.025,w+60)-30,y=wrapScreen(h*(visualHash(i,0x991)*.82)+now*(8+visualHash(i,0x228)*12),h*.92);const q=1+(i%3);ctx.globalAlpha=.12+.28*visualHash(i,0x552);ctx.fillStyle='#e5f2f5';ctx.fillRect(Math.round(x),Math.round(y),q,q);}
  }
  const weather=game.frontier?.weather||'clear';
  if(weather==='rain'||weather==='storm'){const drops=weather==='storm'?58:38;ctx.strokeStyle=weather==='storm'?'rgba(194,219,225,.32)':'rgba(205,229,232,.24)';ctx.lineWidth=weather==='storm'?1.5:1;ctx.globalAlpha=1;for(let i=0;i<drops;i++){const seed=visualHash(i,0x71aa),x=wrapScreen(seed*w*1.6+now*(weather==='storm'?190:120),w+80)-40,y=wrapScreen(visualHash(i,0x19ad)*h+now*(weather==='storm'?330:220),h+45)-20;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x-(weather==='storm'?7:3),y+(weather==='storm'?18:12));ctx.stroke();}}
  else if(weather==='mist'){ctx.globalAlpha=.10;ctx.fillStyle='#d7e1db';for(let i=0;i<5;i++){const x=wrapScreen(i*311+now*11-game.camera.x*s*.018,w+260)-130,y=h*(.32+i*.11);ctx.beginPath();ctx.ellipse(x,y,120+i*20,18+i*3,0,0,Math.PI*2);ctx.fill();}}
  else if(weather==='dry'){ctx.globalAlpha=.12;ctx.fillStyle='#c6ad82';for(let i=0;i<18;i++){const x=wrapScreen(i*137-now*28-game.camera.x*s*.025,w+40)-20,y=h*(.55+visualHash(i,0x401)*.30);ctx.fillRect(x,y,2,1);}}
  ctx.globalAlpha=1;ctx.restore();
}
function drawSurfaceAccents(){
  if(!game.world||currentDepth()>5)return;const s=game.camera.tile,minX=Math.max(1,Math.floor(game.camera.x)-2),maxX=Math.min(WORLD_W-2,Math.ceil(game.camera.x+game.cssW/s)+2);
  ctx.save();
  for(let tx=minX;tx<=maxX;tx+=2){const sy=game.world.surface[tx],x=(tx-game.camera.x)*s,y=(sy-game.camera.y)*s,b=game.world.biome(tx),r=visualHash(tx,0x48d1);if(r<.42)continue;
    if(b.id==='verdant'){ctx.strokeStyle=`rgba(133,178,101,${.18+r*.18})`;ctx.lineWidth=Math.max(1,s*.035);for(let k=0;k<2+(r>.78?1:0);k++){const dx=(k-.5)*s*.18;ctx.beginPath();ctx.moveTo(x+s*.5+dx,y);ctx.quadraticCurveTo(x+s*(.46+k*.08),y-s*(.14+r*.16),x+s*(.38+k*.14),y-s*(.22+r*.18));ctx.stroke();}}
    else if(b.id==='ember'){ctx.fillStyle=`rgba(84,61,56,${.22+r*.18})`;const q=s*(.10+r*.12);ctx.fillRect(x+s*.22,y-q,q*1.2,q);ctx.fillStyle='rgba(201,119,79,.10)';ctx.fillRect(x+s*.62,y-s*.08,s*.10,s*.08);}
    else {ctx.fillStyle=`rgba(218,236,241,${.16+r*.22})`;ctx.beginPath();ctx.moveTo(x+s*.28,y);ctx.lineTo(x+s*.40,y-s*(.14+r*.22));ctx.lineTo(x+s*.52,y);ctx.closePath();ctx.fill();if(r>.78){ctx.fillStyle='rgba(170,215,232,.28)';ctx.fillRect(x+s*.65,y-s*.14,s*.05,s*.14);}}
  }
  ctx.restore();
}
...
function drawTile(tx,ty,id) {
  ...
  if(id===TILE.TREE_TRUNK||id===TILE.LEAF||id===TILE.GLOW_MOSS||id===TILE.TORCH||id===TILE.CAMPFIRE||id===TILE.RUIN_SPIKE||id===TILE.RUIN_URN||id===TILE.ROPE||id===TILE.PLATFORM){drawNonSolidTile(x,y,s,id,def);return;}
  ...
  const artTile={
    [TILE.GRASS]:'tile_grass',[TILE.SOIL]:'tile_soil',[TILE.STONE]:'tile_stone',[TILE.PLANK]:'tile_plank',[TILE.COAL]:'tile_coal',[TILE.COPPER]:'tile_copper',[TILE.IRON]:'tile_iron'
  }[id];
  if(artTile&&drawArt(artTile,x,y,s+1,s+1))return;
  if(id===TILE.WORKBENCH&&drawArt('prop_workbench',x-s*.34,y+s*.12,s*1.68,s*.88))return;
  if(id===TILE.RELIC_CHEST&&drawArt('prop_chest',x-s*.34,y+s*.20,s*1.68,s*.76))return;
  if(id===TILE.BEACON&&artReady()){
    const active=beaconAt(tx,ty),linked=active&&linkedBeaconKeys().has(beaconKey(active.x,active.y)),supply=Math.min(MAX_BEACON_SUPPLY,Math.max(0,Number(active?.supply)||0)),stored=warehouseUnits(active),pulse=.55+.45*Math.sin(performance.now()*.005+tx);
    if(stored>0)drawArt('prop_crates',x-s*(stored>=16?.95:.62),y+s*.28,s*(stored>=16?1.12:.82),s*(stored>=16?.66:.48));
    if(stored>=8)drawArt('prop_barrels',x+s*.62,y+s*.38,s*(stored>=20?.68:.52),s*(stored>=20?.68:.52));
    if(stored>=20){ctx.fillStyle='rgba(37,29,24,.72)';ctx.fillRect(x-s*.88,y+s*.76,s*2.35,s*.12);ctx.fillStyle='#c8a15f';ctx.fillRect(x-s*.72,y+s*.69,s*2.02,s*.07);}
    drawArt('prop_beacon',x+s*.10,y-s*.88,s*.80,s*1.80);
    ctx.strokeStyle=linked?`rgba(114,215,184,${.45+pulse*.4})`:`rgba(238,207,132,${.28+pulse*.28})`;ctx.lineWidth=Math.max(1,s*.045);ctx.beginPath();ctx.arc(x+s*.5,y-s*.20,s*(linked?.38:.30),0,Math.PI*2);ctx.stroke();
    if(stored>0){ctx.fillStyle='rgba(7,13,14,.72)';ctx.fillRect(x-s*.34,y-s*.98,s*.68,s*.24);ctx.fillStyle=stored>=WAREHOUSE_CAP?'#ef9b72':'#d8c78e';ctx.font=`800 ${Math.max(8,Math.round(s*.24))}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(`▣ ${stored}`,x,y-s*.86);}
    for(let i=0;i<supply;i++){ctx.fillStyle='#85d9bd';ctx.fillRect(x+s*(.28+i*.20),y+s*.80,s*.12,s*.10);}return;
  }
...
function drawNonSolidTile(x,y,s,id,def){
  if(id===TILE.TREE_TRUNK){ctx.fillStyle=def.color;ctx.fillRect(x+s*.28,y-s*.03,s*.44,s*1.08);ctx.fillStyle='rgba(236,185,109,.15)';ctx.fillRect(x+s*.33,y+s*.12,s*.10,s*.72);ctx.fillStyle='rgba(48,27,17,.28)';ctx.fillRect(x+s*.60,y,s*.07,s);return;}
  if(id===TILE.LEAF&&drawArt('tile_leaf',x,y,s,s))return;
  if(id===TILE.LEAF){
...
function drawCargoBundles(){
  const s=game.camera.tile;for(const b of game.cargoBundles||[]){const x=(b.x-game.camera.x)*s,y=(b.y-game.camera.y)*s,ratio=Math.max(0,b.hp/b.maxHp),w=s*1.28,h=s*.72;
    if(!drawArt('prop_cargo',x-w/2,y-h/2,w,h)){ctx.fillStyle='#3d2c22';ctx.fillRect(x-w/2,y-h/2,w,h);ctx.strokeStyle='#d0a15e';ctx.strokeRect(x-w/2,y-h/2,w,h);}
    ctx.fillStyle='rgba(0,0,0,.60)';ctx.fillRect(x-w*.42,y-h*.78,w*.84,h*.12);ctx.fillStyle=ratio>.35?'#7bd5b8':'#e48166';ctx.fillRect(x-w*.42,y-h*.78,w*.84*ratio,h*.12);
    ctx.fillStyle='#f0d9a2';ctx.font=`700 ${Math.max(8,s*.26)}px system-ui`;ctx.textAlign='center';ctx.fillText('▣'+bundleUnits(b),x,y+h*.76);
  }
}
function drawPackbeast(){
  const b=activePackbeast();if(!b)return;const s=game.camera.tile,x=(b.x-game.camera.x)*s,y=(b.y-game.camera.y)*s;if(x<-s*3||x>game.cssW+s*3||y<-s*3||y>game.cssH+s*3)return;
  const load=packbeastUnits(),ratio=packbeastLoadRatio(),frame=Math.floor(performance.now()/180)%4;ctx.save();ctx.translate(x,y);if(b.facing<0)ctx.scale(-1,1);ctx.globalAlpha=b.stranded?.72:1;
  ctx.fillStyle='rgba(0,0,0,.26)';ctx.beginPath();ctx.ellipse(0,s*.48,s*.72,s*.10,0,0,Math.PI*2);ctx.fill();
  if(!drawArt('boar_'+frame,-s*.70,-s*.48,s*1.40,s*.94)){ctx.fillStyle='#617858';ctx.fillRect(-s*.54,-s*.28,s*1.08,s*.58);ctx.fillStyle='#3e5141';ctx.fillRect(s*.34,-s*.38,s*.38,s*.36);ctx.fillRect(-s*.42,s*.22,s*.14,s*.38);ctx.fillRect(s*.28,s*.22,s*.14,s*.38);}
  ctx.fillStyle='#58422f';ctx.fillRect(-s*.44,-s*.48,s*.88,s*.22);ctx.fillStyle='#aa8757';const crates=Math.min(3,Math.ceil(load/4));for(let i=0;i<crates;i++)ctx.fillRect((-s*.39+i*s*.29),-s*(.72+.05*(i%2)),s*.25,s*.26);
  if(ratio>.7){ctx.strokeStyle='#e2c679';ctx.lineWidth=Math.max(1,s*.045);ctx.strokeRect(-s*.48,-s*.78,s*.96,s*.38);}
  ctx.globalAlpha=1;if(b.stranded){ctx.fillStyle='#f0c878';ctx.font=`800 ${Math.max(9,Math.round(s*.28))}px system-ui`;ctx.textAlign='center';ctx.fillText(tr('离队','STRANDED'),0,-s*.92);}
  if(b.hp<b.maxHp||load>0){ctx.fillStyle='rgba(0,0,0,.68)';ctx.fillRect(-s*.58,-s*1.02,s*1.16,Math.max(4,s*.045));ctx.fillStyle=b.hp/b.maxHp<.35?'#d46d5c':'#8fb56f';ctx.fillRect(-s*.58,-s*1.02,s*1.16*(b.hp/b.maxHp),Math.max(4,s*.045));ctx.fillStyle='#ead9ab';ctx.font=`700 ${Math.max(8,Math.round(s*.22))}px system-ui`;ctx.textAlign='center';ctx.fillText(`▣ ${load}/${PACKBEAST_CAP}`,0,-s*1.12);}
  ctx.restore();
}
function drawGuard(){
  const g=activeGuard();if(!g)return;const s=game.camera.tile,x=(g.x-game.camera.x)*s,y=(g.y-game.camera.y)*s,frame=Math.floor(performance.now()/180)%4;
  ctx.save();ctx.translate(x,y);if(g.facing<0)ctx.scale(-1,1);const sw=s*1.16,sh=s*1.74;
  if(!drawArt('guard_'+frame,-sw/2,-sh*.69,sw,sh)){ctx.fillStyle='#27383d';ctx.fillRect(-s*.25,-s*.72,s*.5,s*.82);ctx.fillStyle='#a9b9ae';ctx.fillRect(-s*.19,-s*.96,s*.38,s*.28);}
  ctx.restore();ctx.fillStyle='rgba(0,0,0,.60)';ctx.fillRect(x-s*.34,y-s*1.13,s*.68,s*.08);ctx.fillStyle='#78c59c';ctx.fillRect(x-s*.34,y-s*1.13,s*.68*(g.hp/g.maxHp),s*.08);
}
function drawDrops(){
...
function drawPlayer(){
  ...
  const moving=Math.min(1,Math.abs(p.vx)/3.8),walkFrame=Math.floor(performance.now()/135)%3,carrying=cargoLoad()>0;
  let frame=0;if(carrying)frame=4+(Math.floor(performance.now()/190)%2);else if(p.attackFlash>0)frame=6;else if(p.grounded&&moving>.16)frame=1+walkFrame;
  ctx.fillStyle='rgba(0,0,0,.28)';ctx.beginPath();ctx.ellipse(0,h*.48,w*.62,h*.08,0,0,Math.PI*2);ctx.fill();
  const sw=s*1.16,sh=s*1.74;ctx.globalAlpha=game.hurtCd>0?.60:1;
  const usedArt=drawArt('player_'+frame,-sw/2,h*.50-sh,sw,sh);ctx.globalAlpha=1;
  if(!usedArt){ctx.fillStyle=game.hurtCd>0?'#ef9b8e':'#d9c5a7';ctx.fillRect(-w*.23,-h*.49,w*.46,h*.29);ctx.fillStyle='#334c4f';ctx.fillRect(-w*.33,-h*.18,w*.66,h*.58);ctx.fillStyle='#151f22';ctx.fillRect(-w*.31,h*.39,w*.24,h*.44);ctx.fillRect(w*.07,h*.39,w*.24,h*.44);}
  if(!carrying&&frame!==6)drawHeldItem(s,w,h);
  if(p.attackFlash>0){ctx.strokeStyle='rgba(247,216,144,.82)';ctx.lineWidth=Math.max(2,s*.065);ctx.beginPath();ctx.arc(w*.16,0,w*1.45,-.85,.55);ctx.stroke();}
  ctx.restore();
}
function drawEnemyBody(e,s){
  const c=e.def.color,t=e.type;ctx.fillStyle=c;
  if(t==='moss_crawler'&&artReady()){const f=Math.floor(performance.now()/170)%5;drawArt('slime_'+f,-s*.50,-s*.38,s,s*.75);return;}
  if(t==='bramble_boar'&&artReady()){const f=Math.floor(performance.now()/155)%4;drawArt('boar_'+f,-s*.72,-s*.48,s*1.44,s*.96);return;}
  if((t==='greyveil_raider'||t==='faction_raider')&&artReady()){const f=Math.floor(performance.now()/180)%4;drawArt('raider_'+f,-s*.58,-s*.86,s*1.16,s*1.74);return;}
  if(t==='moss_crawler'){
...
function drawSettlements(){
  ...
  for(const site of game.world.settlements){
    ...
    const distance=Math.hypot(site.x-p.x,site.y-p.y),near=distance<22,reliability=settlementReliability(site),simTown=frontierSettlement(game.frontier,site),simProsperity=frontierProsperityLevel(game.frontier,site),prosperity=Math.max(0,Math.min(5,Math.round(reliability*.45+simProsperity*.55))),demand=near?settlementDailyDemand(site,game.trade?.day||0,reliability):null;
    ...
    const now=performance.now()*.001,night=1-dayLight(),crowdCount=Math.min(3,1+Math.floor(prosperity/2)),tradeGlow=.025+prosperity*.008;
    ...
    ctx.fillStyle='rgba(232,203,136,.18)';ctx.fillRect(-s*8.3,-s*.20,s*16.6,s*.10);
    const houses=[[-7.1,2.45,3.3],[-4.0,2.2,2.85],[3.65,2.35,3.05],[6.65,2.5,3.45]];
    for(const [q,w,h] of houses){
      ctx.fillStyle=palette[0];ctx.fillRect((q-w*.5)*s,-h*s,w*s,h*s);
      ctx.fillStyle=palette[1];ctx.beginPath();ctx.moveTo((q-w*.62)*s,-h*s);ctx.lineTo(q*s,-(h+.9)*s);ctx.lineTo((q+w*.62)*s,-h*s);ctx.fill();
      ctx.fillStyle='#e0bd74';ctx.fillRect((q-.18)*s,-s*.9,s*.36,s*.9);
      ctx.fillStyle='rgba(238,217,165,.52)';ctx.fillRect((q-w*.25)*s,-(h-.72)*s,s*.34,s*.34);
    }
    ctx.fillStyle=palette[3];ctx.fillRect(-s*2.25,-s*2.25,s*4.5,s*.22);
    ctx.fillStyle=palette[2];for(let i=0;i<5;i++)ctx.fillRect((-2.18+i*.9)*s,-s*2.23,s*.48,s*.55);
    ctx.fillStyle=palette[0];ctx.fillRect(-s*2.05,-s*1.72,s*4.1,s*1.4);
...
function drawFrontierCaravans(){
  const caravans=game.frontier?.caravans||[],raids=game.frontier?.raids||[];if((!caravans.length&&!raids.length)||!game.world?.settlements?.length)return;const byId=new Map(game.world.settlements.map(site=>[site.id,site])),s=game.camera.tile;
  for(const c of caravans){const a=byId.get(c.from),b=byId.get(c.to);if(!a||!b)continue;const t=Math.max(0,Math.min(1,Number(c.progress)||0)),wx=a.x+(b.x-a.x)*t,tx=Math.max(1,Math.min(WORLD_W-2,Math.floor(wx))),wy=game.world.surface[tx]-.48,x=(wx-game.camera.x)*s,y=(wy-game.camera.y)*s;if(x<-s*3||x>game.cssW+s*3||y<-s*3||y>game.cssH+s*3)continue;ctx.save();ctx.translate(Math.round(x),Math.round(y));if(b.x<a.x)ctx.scale(-1,1);ctx.fillStyle='rgba(5,8,9,.30)';ctx.beginPath();ctx.ellipse(0,s*.22,s*.72,s*.12,0,0,Math.PI*2);ctx.fill();if(!drawArt('prop_cargo',-s*.62,-s*.54,s*1.18,s*.64)){ctx.fillStyle='#8a6644';ctx.fillRect(-s*.38,-s*.34,s*.76,s*.42);ctx.strokeStyle='#d6b56f';ctx.strokeRect(-s*.38,-s*.34,s*.76,s*.42);}ctx.strokeStyle='#403327';ctx.lineWidth=Math.max(2,s*.07);for(const q of [-.36,.34]){ctx.beginPath();ctx.arc(q*s,s*.08,s*.18,0,Math.PI*2);ctx.stroke();}ctx.fillStyle='#d8c18c';ctx.fillRect(-s*.68,-s*.43,s*.16,s*.50);ctx.fillStyle='#2c3e3b';ctx.fillRect(-s*.72,-s*.56,s*.24,s*.18);ctx.restore();}
  for(const r of raids){const a=byId.get(r.from),b=byId.get(r.to);if(!a||!b)continue;const t=Math.max(0,Math.min(1,Number(r.progress)||0)),wx=a.x+(b.x-a.x)*t,tx=Math.max(1,Math.min(WORLD_W-2,Math.floor(wx))),wy=game.world.surface[tx]-.55,x=(wx-game.camera.x)*s,y=(wy-game.camera.y)*s;if(x<-s*3||x>game.cssW+s*3||y<-s*3||y>game.cssH+s*3)continue;ctx.save();ctx.translate(Math.round(x),Math.round(y));if(b.x<a.x)ctx.scale(-1,1);ctx.fillStyle='rgba(109,41,35,.82)';ctx.beginPath();ctx.moveTo(-s*.34,-s*.44);ctx.lineTo(s*.36,-s*.18);ctx.lineTo(-s*.34,s*.04);ctx.closePath();ctx.fill();ctx.strokeStyle='#e29a72';ctx.lineWidth=Math.max(1,s*.045);ctx.beginPath();ctx.moveTo(-s*.38,-s*.5);ctx.lineTo(-s*.38,s*.18);ctx.stroke();ctx.fillStyle='#342722';for(let i=0;i<Math.min(3,Math.ceil(r.strength||1));i++)ctx.fillRect((-s*.22+i*s*.20),s*.04,s*.12,s*.28);ctx.restore();}
}
...
function drawSupplyRoutes(){
  ...
  for(const link of links){const ax=(link.a.x-game.camera.x)*s,ay=(link.a.y-game.camera.y)*s,bx=(link.b.x-game.camera.x)*s,by=(link.b.y-game.camera.y)*s;if(Math.max(ax,bx)<-40||Math.min(ax,bx)>game.cssW+40||Math.max(ay,by)<-40||Math.min(ay,by)>game.cssH+40)continue;const biomes=componentBiomeIds(beaconComponentFor(link.a)),diverse=biomes.length>1,active=pointSegmentDistance(p.x,p.y,link.a,link.b)<=ROUTE_RADIUS;ctx.globalAlpha=active?.62:diverse?.34:.22;ctx.strokeStyle=diverse?'#82d7bc':'#d3b66d';ctx.lineWidth=Math.max(1.4,s*(active?.075:.045));ctx.beginPath();ctx.moveTo(ax,ay);ctx.lineTo(bx,by);ctx.stroke();ctx.setLineDash([]);for(const t of [.25,.5,.75]){const nx=ax+(bx-ax)*t,ny=ay+(by-ay)*t,r=Math.max(2.5,s*.1);ctx.fillStyle=diverse?'#82d7bc':'#d3b66d';ctx.fillRect(Math.round(nx-r/2),Math.round(ny-r*1.9),Math.round(r),Math.round(r*2.1));ctx.fillStyle='#f2dfb5';ctx.fillRect(Math.round(nx-r*.8),Math.round(ny-r*2.05),Math.round(r*1.6),Math.max(1,Math.round(r*.35)));}ctx.setLineDash([Math.max(5,s*.34),Math.max(4,s*.22)]);}
  ctx.restore();
}
...
function render(){ctx.save();if(game.fx.shake>0)ctx.translate((game.rng()-.5)*game.fx.shake,(game.rng()-.5)*game.fx.shake);drawBackdrop();drawBiomeAtmosphere();drawWorld();drawSurfaceAccents();drawSettlements();drawSupplyRoutes();drawFrontierCaravans();drawProjectiles();drawEnemyProjectiles();drawEnemies();drawDrops();drawCargoBundles();drawPackbeast();drawGuard();drawPlayer();drawTarget();drawFx();ctx.restore();drawLighting();updateHud();updateTargetTip();}
...
function simulationStep(dt){updateSurvival(dt);updatePlayer(dt);updateHazards();updateActions(dt);updateProjectiles(dt);updateEnemyProjectiles(dt);updateEnemies(dt);updateDrops(dt);updateCampfireRest(dt);updateProgression(dt);game.time=(game.time+dt/180)%1;if(game.frontier){const before=game.frontier.elapsedDays||0,d=game.diagnostics||(game.diagnostics=freshDiagnostics()),sample=(d.frontierSampleCounter++%DIAGNOSTIC_FRONTIER_SAMPLE_EVERY)===0,started=sample?performance.now():0;stepFrontierSimulation(game.frontier,{dtDays:dt/180,day:(game.nightsSurvived||0)+game.time,settlements:game.world.settlements||[]});if(sample)pushDiagnostic('frontierMs',performance.now()-started);if(Math.floor(before*20)!==Math.floor((game.frontier.elapsedDays||0)*20))game.saveDirty=true;}}
function loop(now){
  if(!game.running)return;const frameDt=Math.min(MAX_FRAME_DT,Math.max(0,(now-(game.last||now))/1000));game.last=now;pushDiagnostic('frameMs',frameDt*1000);syncMobileAim();
...
$('#mobilePack').onclick=()=>{const tradeNode=nearbySettlementMarket()||nearbyTradePost();togglePanel('inventoryPanel');if(tradeNode&&!$('#inventoryPanel').classList.contains('hidden'))selectPanelTab('trade');};
...
function selectPanelTab(tab){$$('[data-tab]').forEach(x=>x.classList.toggle('active',x.dataset.tab===tab));for(const [name,id] of [['inventory','inventoryView'],['craft','craftView'],['trade','tradeView']])$('#'+id).classList.toggle('hidden',name!==tab);if(tab==='craft')renderCraft();if(tab==='trade'){renderTrade();if(nearbySettlementMarket())markMilestone('first-market');}}
...
$('#newWorldBtn').onclick=()=>{if(confirm(tr('这会替换当前本地世界。继续？','This replaces the current local world. Continue?'))){for(const key of [SAVE_KEY,LEGACY_SAVE_KEY_0420,LEGACY_SAVE_KEY_0410,LEGACY_SAVE_KEY_0230,LEGACY_SAVE_KEY_0220,LEGACY_SAVE_KEY_0210,LEGACY_SAVE_KEY_0200,LEGACY_SAVE_KEY_0190,LEGACY_SAVE_KEY_0180,LEGACY_SAVE_KEY_0170,LEGACY_SAVE_KEY_0160,LEGACY_SAVE_KEY_0150,LEGACY_SAVE_KEY_0140,LEGACY_SAVE_KEY_0131,LEGACY_SAVE_KEY_0130,LEGACY_SAVE_KEY_0120,LEGACY_SAVE_KEY_0110,LEGACY_SAVE_KEY_0100,LEGACY_SAVE_KEY,LEGACY_SAVE_KEY_091,LEGACY_SAVE_KEY_090,LEGACY_SAVE_KEY_080,LEGACY_SAVE_KEY_070,LEGACY_SAVE_KEY_060,LEGACY_SAVE_KEY_OLD,LEGACY_SAVE_KEY_OLDER,LEGACY_SAVE_KEY_OLDEST])localStorage.removeItem(key);location.reload();}};
