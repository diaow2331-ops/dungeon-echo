...
const scoreEl=$('score'), speedEl=$('speed'), comboEl=$('combo'), mistakeChip=$('mistakeChip'), mistakesLeftEl=$('mistakesLeft'), bestEl=$('best'), bestBelow=$('bestBelow'), bestComboEl=$('bestCombo'), runsEl=$('runs'), onTimeEndsEl=$('onTimeEnds'), overtimeEndsEl=$('overtimeEnds');
const clockEl=$('clock'), stageEl=$('stage'), sceneHud=$('sceneHud'), sceneToast=$('sceneToast'), tickerEl=$('ticker'), dayProgress=$('dayProgress'), fullscreenBtn=$('fullscreenBtn'), fullscreenLabel=$('fullscreenLabel'), bgmChip=$('bgmChip'), bgmHud=$('bgmHud'), eventChip=$('eventChip'), eventHud=$('eventHud'), buffChip=$('buffChip'), buffHud=$('buffHud'), tutorialToast=$('tutorialToast'), discoveryCountEl=$('discoveryCount'), discoveryListEl=$('discoveryList');
...
Object.assign(UI_I18N,{
  ...
  factScenes:{zh:'办公室场景',en:'office scenes'},factJump:{zh:'跳跃层级',en:'jump depth'},factBuffer:{zh:'容错次数',en:'mistake buffer'},factEnds:{zh:'不同结局',en:'endings'},mistakes:{zh:'容错',en:'Buffer'},buff:{zh:'状态',en:'Buff'},
...
function applyLanguage(persist=true){
  ...
  syncAudioControls();updateMusicHud();renderDiscoveries();renderRunLedger();updateDailyUi();updateRouteStrip();syncMistakeHud();syncBuffHud();if(tutorialActive&&tutorialStep)setTutorial(tutorialStep);
...
const PLAYER_HIT={left:10,right:10,top:7,bottom:6},JUMP_BUFFER_WINDOW=.12,MAX_MISTAKES=3,HIT_IFRAME=1.15,JUMP_GRAVITY=1750;
const BOSS_RUSH_WARNING=.48,BUG_MUTATION_WARNING=.56,LEAVE_SLIP_DURATION=12;
...
let feelFlash=0,feelKind='none',comboBurstTimer=0,comboBurstValue=0,airBurstTimer=0,landingPulse=0,bossWarningTimer=0;
...
const directorPatterns=[
  // v1.19: each room has its own readable phrase vocabulary instead of drawing from one generic late-game pool.
  // gapPx is clear space between this hazard and the next one; pair-gap guards still remain authoritative.
  {scenes:[0],minStage:0,name:'工位巡桌',seq:[{label:'BUG',gapPx:720},{label:'老板',gapPx:null}]},
  {scenes:[0],minStage:0,name:'需求插队',seq:[{label:'BUG',gapPx:760},{label:'临时需求',gapPx:null}]},
  {scenes:[1],minStage:1,name:'会议余波',seq:[{label:'会议',gapPx:680},{label:'邮件',gapPx:null}]},
  {scenes:[1],minStage:1,name:'镜头追问',seq:[{label:'邮件',gapPx:650},{label:'会议',gapPx:null}]},
  {scenes:[2],minStage:2,name:'茶水间惊魂',seq:[{label:'咖啡渍',gapPx:660},{label:'老板',gapPx:null}]},
  {scenes:[2],minStage:2,name:'补丁洒了',seq:[{label:'BUG',gapPx:610},{label:'咖啡渍',gapPx:null}]},
  {scenes:[3],minStage:3,name:'健身训练',seq:[{label:'哑铃',gapPx:590},{label:'邮件',gapPx:null}]},
  {scenes:[3],minStage:3,name:'需求追击',seq:[{label:'临时需求',gapPx:690},{label:'BUG',gapPx:null}]},
  {scenes:[3],minStage:4,name:'下班连环',seq:[{label:'BUG',gapPx:540},{label:'邮件',gapPx:580},{label:'老板',gapPx:null}]}
];
...
function syncMistakeHud(){const left=Math.max(0,MAX_MISTAKES-mistakes);if(mistakesLeftEl)mistakesLeftEl.textContent=String(left);if(mistakeChip){mistakeChip.dataset.level=left<=1?'danger':(left===2?'warn':'ok');mistakeChip.title=currentLang==='en'?`${left} mistake buffer remaining`:`还剩 ${left} 次容错`}}
function syncStats(){bestEl.textContent=best;bestBelow.textContent=best+'m';bestComboEl.textContent=bestCombo;runsEl.textContent=runs;onTimeEndsEl.textContent=onTimeEndings;overtimeEndsEl.textContent=overtimeEndings;syncMistakeHud()}
...
function cleanRunRecord(value){
  ...
  return {distance:Math.max(0,Math.floor(Number(value.distance)||0)),combo:Math.max(0,Math.floor(Number(value.combo)||0)),near:Math.max(0,Math.floor(Number(value.near)||0)),perfect:Math.max(0,Math.floor(Number(value.perfect)||0)),mistakes:Math.max(0,Math.min(MAX_MISTAKES,Math.floor(Number(value.mistakes)||0))),discoveries:Math.max(0,Math.floor(Number(value.discoveries)||0)),outcome,cause:String(value.cause||'').slice(0,20),scene:Math.max(0,Math.min(4,Math.floor(Number(value.scene)||0))),clock:String(value.clock||'14:00').slice(0,12),daily:Boolean(value.daily),dailyDate:String(value.dailyDate||'').slice(0,10),ts:Math.max(0,Math.floor(Number(value.ts)||0))}
}
...
function runMetricLine(){const d=dailyMode?(currentLang==='en'?`DAILY ${dailyShortLabel()} · `:`今日挑战 ${dailyShortLabel()} · `):'';return currentLang==='en'?`${d}Peak combo ${runPeakCombo} · Near misses ${runNearMisses} (Perfect ${runPerfectNearMisses}) · Mistakes ${mistakes}/${MAX_MISTAKES}`:`${d}峰值连摸 ${runPeakCombo} · 擦边 ${runNearMisses}（极限 ${runPerfectNearMisses}） · 失误 ${mistakes}/${MAX_MISTAKES}`}
function renderRunLedger(){
  ...
  else{const r=lastRunRecord,d=runDailyLabel(r),prefix=d?`${d} · `:'';lastRunSummaryEl.textContent=currentLang==='en'?`${prefix}${runOutcomeLabel(r)} · ${r.clock} · ${runSceneLabel(r)} · ${r.distance}m · Peak combo ${r.combo} · Near ${r.near} (Perfect ${r.perfect}) · Mistakes ${r.mistakes}/${MAX_MISTAKES} · New finds ${r.discoveries}`:`${prefix}${runOutcomeLabel(r)} · ${r.clock} · ${runSceneLabel(r)} · ${r.distance}m · 峰值连摸 ${r.combo} · 擦边 ${r.near}（极限 ${r.perfect}） · 失误 ${r.mistakes}/${MAX_MISTAKES} · 新发现 ${r.discoveries}`}
  ...
  topRuns.forEach((r,i)=>{const row=document.createElement('div'),rank=document.createElement('span'),main=document.createElement('span'),score=document.createElement('span');row.className='run-history-row';rank.className='run-rank';main.className='run-history-main';score.className='run-history-score';rank.textContent=`#${i+1}`;main.textContent=`${runDailyLabel(r)?runDailyLabel(r)+' · ':''}${runOutcomeLabel(r)} · ${r.clock} · ${runSceneLabel(r)} · N${r.near}/P${r.perfect} · M${r.mistakes}`;score.textContent=`${r.distance}m · C${r.combo}`;if(r.ts)row.title=new Date(r.ts).toLocaleString(currentLang==='en'?'en-US':'zh-CN');row.append(rank,main,score);topRunHistoryEl.append(row)})
}
function recordFinishedRun(outcome,cause=''){
  if(runRecordSaved)return lastRunRecord;runRecordSaved=true;
  const rec=cleanRunRecord({distance,combo:runPeakCombo,near:runNearMisses,perfect:runPerfectNearMisses,mistakes,discoveries:Math.max(0,discoveries.size-runDiscoveryStart),outcome,cause,scene:outcome==='caught'?Math.max(0,sceneIndex):4,clock:clockEl.textContent||'14:00',daily:dailyMode,dailyDate:dailyMode?dailySeedDate:'',ts:Date.now()});
...
function setTutorial(step){
  ...
  else{tutorialToast.textContent=tr('教学完成 · 普通碰撞有 3 次容错；看预警、留第二段。');tutorialToast.classList.add('good')}
}
...
function updateScene(force=false){
  ...
  if(!force){screenShake=Math.max(screenShake,2.5);tickerEl.textContent=currentLang==='en'?`Entering ${tr(s.name)}: ${tr(s.tag)}`:`进入 ${s.name}：${s.tag}`;tickerTimer=4.5;spawnTimer=Math.max(spawnTimer,1.35);directorCooldown=Math.max(directorCooldown,2.1);encounterClusterCount=0;encounterClusterTarget=3+Math.floor(gameRandom()*3);alignMusicPhrase(.12)}
...
function performJump(){
  ...
  const before=player.jumps;player.vy=before===0?-740:-590;player.jumps++;player.squash=.12;jumpBufferTimer=0;beep(player.jumps===1?520:680,.045);
  ...
  if(groundTakeoff){for(let i=0;i<8;i++)particles.push({x:player.x+7+Math.random()*31,y:GROUND-4,vx:-35-Math.random()*115,vy:-12-Math.random()*72,a:.78,r:1.5+Math.random()*2.5,c:'#8e877d'});landingPulse=Math.max(landingPulse,.08)}
  else if(before===1){airBurstTimer=.20;emitPlayerBurst('double',10);feelFlash=Math.max(feelFlash,.055);feelKind='double';beep(840,.04,'triangle',.020)}
  return true
}
function jump(){
...
function chooseObstacle(){
  ...
  if(worldTime<4.5)label='BUG';else if(worldTime<11&&(label==='会议'||label==='临时需求'||label==='邮件'))label=gameRandom()<.62?'BUG':'老板';
  return label;
}
...
function sampleNormalGapPx(spawned){
  ...
  let bands;
  // v1.19: the route lasts about four minutes, so encounters need deliberate air between them.
  // Earlier builds stretched route time without proportionally reducing obstacle density, which made good runs feel like nonstop input spam.
  if(runDistance<360){bands=[{w:.10,a:560,b:680},{w:.50,a:720,b:900},{w:.40,a:980,b:1240}]}
  else if(runDistance<1050){bands=[{w:.22,a:500,b:620},{w:.52,a:650,b:820},{w:.26,a:880,b:1080}]}
  else if(runDistance<1800){bands=[{w:.28,a:450,b:570},{w:.50,a:610,b:770},{w:.22,a:820,b:1000}]}
  else{bands=[{w:.34,a:410,b:520},{w:.48,a:560,b:700},{w:.18,a:750,b:900}]}
  ...
  gap+=sceneHalf?-18:32;
  if(gymRushTimer>0)gap+=125;if(worldTime<10)gap=Math.max(gap,650);gap=Math.max(315,gap);
...
function applyEncounterCadence(gapPx,plan){
  // Obstacles arrive in short readable phrases, followed by a real breathing beat. Curated director sequences count as one phrase.
  const finishedPattern=!!plan&&directorQueue.length===0;
  if(plan){
    if(!finishedPattern)return gapPx;
    encounterClusterCount=0;encounterClusterTarget=3+Math.floor(gameRandom()*3);encounterBreathers++;
    return gapPx+rand(stageIndex>=4?330:430,stageIndex>=4?520:680);
  }
  encounterClusterCount++;
  if(encounterClusterCount<encounterClusterTarget)return gapPx;
  encounterClusterCount=0;encounterClusterTarget=3+Math.floor(gameRandom()*3);encounterBreathers++;
  return gapPx+rand(stageIndex>=4?300:400,stageIndex>=4?500:650);
}
function maybeQueueDirectorPattern(){
  ...
  if(directorCooldown>0||runDistance<(sceneIndex===0?260:700))return false;
  let chance=stageIndex>=4?.22:(stageIndex>=3?.18:(stageIndex>=1?.14:.10));if(stageIndex<4)chance*=sceneHalf?1.18:.78;if(gameRandom()>chance)return false;
  let available=directorPatterns.filter(p=>p.minStage<=stageIndex&&(!p.scenes||p.scenes.includes(sceneIndex)));
...
function spawnObstacle(forcedLabel=null){
  ...
  if(label==='老板'){obstacle.rush=sceneIndex===0&&worldTime>11&&gameRandom()<0.46;obstacle.rushTriggered=false;obstacle.rushWarnTimer=0;obstacle.rushTimer=0;obstacle.extraSpeed=210+gameRandom()*90}
  if(label==='BUG'){obstacle.mutation=(bugPatchTimer>0||worldTime<8)?'none':(gameRandom()<.70?(gameRandom()<.5?'tall':'long'):'none');obstacle.mutationState='idle';obstacle.mutationTimer=0;obstacle.baseW=k.w;obstacle.baseH=k.h;obstacle.targetW=k.w;obstacle.targetH=k.h}
  if(label==='临时需求'){obstacle.dropState='warning';obstacle.warning=stageIndex>=4?.66:.80;obstacle.vy=0;obstacle.targetY=GROUND-k.h;obstacle.warningPulse=Math.random()*6.28}
  if(label==='会议'){
...
function pickupGrabBox(p){return {x:p.x-8,y:p.y-8,w:p.w+16,h:p.h+16}}

function addFloater(x,y,text,color='#171717',size=14){floaters.push({x,y,text,a:1,vy:-34,color,size})}
...
function updateComboHud(){
  const chip=comboEl.parentElement;if(!chip)return;const life=combo>0?Math.max(0,1-comboAge/COMBO_WINDOW):0;chip.style.setProperty('--combo-life',`${(life*100).toFixed(1)}%`);chip.dataset.flow=combo>=5?'true':'false'
}
function triggerFeel(kind='near',strength=1){
  feelKind=kind;feelFlash=Math.max(feelFlash,.12+.08*strength);screenShake=Math.max(screenShake,kind==='perfect'?5.5:(kind==='boss'?4.2:2.8));
}
function emitPlayerBurst(kind='jump',count=8){
  const cx=player.x+player.w*.5,cy=player.y+player.h*.72;
  for(let i=0;i<count;i++){const a=Math.random()*Math.PI*2,sp=(kind==='perfect'?120:70)+Math.random()*(kind==='perfect'?180:110);particles.push({x:cx,y:cy,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-(kind==='land'?35:0),a:kind==='perfect'?1.0:.72,r:1.3+Math.random()*2.3,c:kind==='perfect'?(i%2?'#d8ef9f':'#fffdf8'):(kind==='double'?'#d7ecfb':'#8e877d')})}
}
function triggerComboMilestone(value){
  comboBurstValue=value;comboBurstTimer=.62;feelFlash=Math.max(feelFlash,.10);feelKind='combo';screenShake=Math.max(screenShake,3.2);emitPlayerBurst('perfect',Math.min(18,7+Math.floor(value/2)));beep(Math.min(1200,620+value*18),.055,'square',.030);setTimeout(()=>beep(Math.min(1450,760+value*20),.06,'triangle',.024),45);
  // A clean ten-obstacle streak can repair one earlier mistake once per scene. This turns Combo into survival skill, not only score.
  if(value>0&&value%10===0&&mistakes>0&&sceneIndex>=0&&sceneIndex<4&&!sceneComboRecoveryUsed[sceneIndex]){
    mistakes--;sceneComboRecoveryUsed[sceneIndex]=true;syncMistakeHud();feelKind='recover';feelFlash=Math.max(feelFlash,.22);screenShake=Math.max(screenShake,3.6);
    addFloater(player.x+58,player.y-30,currentLang==='en'?'FLOW RECOVERY · +1 BUFFER':'稳住了 · 恢复 1 次容错','#506b2c');tickerEl.textContent=currentLang==='en'?'Ten clean clears repaired one mistake buffer.':`连续躲过 10 个障碍，恢复 1 次容错。`;tickerTimer=Math.max(tickerTimer,4.2);beep(880,.07,'triangle',.032);setTimeout(()=>beep(1174,.08,'square',.026),55)
  }
}
function passObstacle(o){
  o.passed=true;combo++;runPeakCombo=Math.max(runPeakCombo,combo);comboAge=0;comboEl.textContent=combo;updateComboHud();
  ...
  if(nearTier===2){triggerFeel('perfect',1);emitPlayerBurst('perfect',14);beep(1046,.055,'triangle',.035);setTimeout(()=>beep(1318,.045,'square',.026),35)}
  else if(nearTier===1){triggerFeel('near',.65);beep(920,.045,'triangle',.025)}
  else if(mechanicBonus){triggerFeel('mechanic',.55);beep(860,.045,'triangle',.024)}
  if(combo>0&&combo%5===0)triggerComboMilestone(combo);
...
function collectPickup(p){
  p.got=true;
  if(p.kind==='leave'){
    leaveSlipHits=1;leaveSlipTimer=LEAVE_SLIP_DURATION;syncBuffHud();distance+=10;addFloater(p.x-10,p.y-10,currentLang==='en'?'Leave slip · 1 save':'请假条 · 挡一次','#506b2c');beep(620,.08,'triangle',.035);screenShake=1.5;return
  }
  if(p.kind==='risk'){
    riskBoostTimer=Math.max(riskBoostTimer,7);syncBuffHud();distance+=15;addFloater(p.x-16,p.y-10,currentLang==='en'?'Risk bonus · near-miss ×2':'绩效单 · 擦边 ×2','#8a6a16');beep(988,.08,'triangle',.035);screenShake=2;return
  }
...
function absorbWithLeaveSlip(o){
  ...
  leaveSlipHits=0;leaveSlipTimer=0;syncBuffHud();o.passed=true;o.x=-999;combo=0;comboAge=0;comboEl.textContent='0';updateComboHud();addFloater(player.x+55,player.y-16,currentLang==='en'?`Leave slip saved you from ${tr(o.label)}`:`请假条挡住了「${o.label}」`,'#506b2c');screenShake=5;beep(540,.07,'triangle',.04);return true
}
function takeObstacleHit(o){
  if(hitInvulnTimer>0)return false;mistakes=Math.min(MAX_MISTAKES,mistakes+1);lastHitCause=o.label;syncMistakeHud();
  if(mistakes>=MAX_MISTAKES){gameOver(o.label);return true}
  hitInvulnTimer=HIT_IFRAME;hitFlashTimer=.30;o.passed=true;o.x=-999;combo=0;comboAge=0;comboEl.textContent='0';updateComboHud();feelKind='hurt';feelFlash=Math.max(feelFlash,.20);screenShake=Math.max(screenShake,8);
  const left=MAX_MISTAKES-mistakes;addFloater(player.x+52,player.y-30,currentLang==='en'?`BUFFER ${left}/${MAX_MISTAKES}`:`容错 ${left}/${MAX_MISTAKES}`,'#9c3f2f',12);tickerEl.textContent=currentLang==='en'?`${tr(o.label)} clipped you. ${left} buffer${left===1?'':'s'} left.`:`撞上「${o.label}」，但本局还剩 ${left} 次容错。`;tickerTimer=Math.max(tickerTimer,3.4);if(navigator.vibrate)navigator.vibrate(24);beep(170,.08,'sawtooth',.04);setTimeout(()=>beep(260,.045,'square',.024),55);return true
}
...
function syncBuffHud(){if(!buffChip||!buffHud)return;const parts=[];if(leaveSlipHits>0&&leaveSlipTimer>0)parts.push(currentLang==='en'?`Leave ${leaveSlipTimer.toFixed(1)}s`:`请假条 ${leaveSlipTimer.toFixed(1)}s`);if(riskBoostTimer>0)parts.push(currentLang==='en'?`Risk ×2 ${riskBoostTimer.toFixed(1)}s`:`擦边×2 ${riskBoostTimer.toFixed(1)}s`);buffHud.textContent=parts.join(' · ')||'—';buffChip.classList.toggle('hidden',parts.length===0)}
...
function update(dt){
  ...
  speed=Math.min(820,350+runDistance*.11+(gymRushTimer>0?62:0));visualScrollPx+=speed*dt;const step=speed*dt*RUN_PROGRESS_SCALE;runDistance+=step;distance+=step;
  ...
  leaveSlipTimer=Math.max(0,leaveSlipTimer-dt);if(leaveSlipTimer===0)leaveSlipHits=0;riskBoostTimer=Math.max(0,riskBoostTimer-dt);syncBuffHud();hitInvulnTimer=Math.max(0,hitInvulnTimer-dt);hitFlashTimer=Math.max(0,hitFlashTimer-dt);
  jumpBufferTimer=Math.max(0,jumpBufferTimer-dt);lastGrounded=player.y>=GROUND-player.h-1;
  player.vy+=JUMP_GRAVITY*dt;player.y+=player.vy*dt;
  if(player.y>=GROUND-player.h){const landed=!lastGrounded,impact=player.vy;player.y=GROUND-player.h;player.vy=0;player.jumps=0;if(landed){player.squash=Math.max(player.squash,impact>760?.105:.082);landingPulse=Math.max(landingPulse,impact>760?.22:.13);if(impact>520){emitPlayerBurst('land',Math.min(12,6+Math.floor((impact-500)/120)));screenShake=Math.max(screenShake,impact>820?2.4:1.2)}}if(jumpBufferTimer>0)performJump()}
  ...
  spawnTimer-=dt;if(spawnTimer<=0){
    ...
    gapPx=applyEncounterCadence(gapPx,plan);
    if(gapPx>lastSpawnGapPx){lastSpawnGapPx=gapPx;spacingHistory.push(Math.round(gapPx));if(spacingHistory.length>14)spacingHistory.shift()}
  ...
  for(const o of obstacles){
    let vx=speed;
    if(o.label==='老板'&&o.rush&&!o.rushTriggered&&o.x<610){o.rushTriggered=true;o.rushWarnTimer=BOSS_RUSH_WARNING;o.rushTimer=0;bossWarningTimer=.72;triggerFeel('boss',.8);addFloater(o.x-30,o.y-18,currentLang==='en'?'BOSS SPOT CHECK!':'老板突击检查！','#9c3f2f');duckMusic(.62,.22);beep(210,.065,'sawtooth',.038);setTimeout(()=>beep(280,.045,'square',.026),65)}
    ...
    if(o.label==='BUG'&&o.mutation!=='none'&&o.mutationState==='idle'&&o.x<720){
      o.mutationState='warn';o.mutationTimer=BUG_MUTATION_WARNING;addFloater(o.x-12,o.y-14,currentLang==='en'?(o.mutation==='tall'?'BUG growing TALL!':'BUG growing LONG!'):(o.mutation==='tall'?'BUG 要长高了！':'BUG 要拉长了！'),'#9c3f2f');beep(330,.045,'square',.025)
    }
  ...
  for(const o of obstacles){
    ...
    if(collided){if(hitInvulnTimer>0)continue;if(absorbWithLeaveSlip(o))continue;takeObstacleHit(o);if(state==='gameover')return}
  }
  for(const p of pickups)if(!p.got&&hit(pbox,pickupGrabBox(p),0))collectPickup(p);
  ...
  for(const d of particles){d.x+=d.vx*dt;d.y+=d.vy*dt;d.vy+=240*dt;d.a-=dt*1.9}particles=particles.filter(d=>d.a>0);
  for(const f of floaters){f.y+=f.vy*dt;f.a-=dt*.9}floaters=floaters.filter(f=>f.a>0);
  feelFlash=Math.max(0,feelFlash-dt);if(feelFlash===0)feelKind='none';comboBurstTimer=Math.max(0,comboBurstTimer-dt);airBurstTimer=Math.max(0,airBurstTimer-dt);landingPulse=Math.max(0,landingPulse-dt);bossWarningTimer=Math.max(0,bossWarningTimer-dt);screenShake=Math.max(0,screenShake-dt*24);overtimeFlash=Math.max(0,overtimeFlash-dt);
}
...
function drawRunnerBackdropClearance(i){
  ...
  const cx=player.x+player.w*.5,cy=GROUND-102,tones=[[244,242,233],[243,243,239],[245,240,232],[245,222,187]],tone=tones[Math.max(0,Math.min(3,i))];
  const glow=ctx.createRadialGradient(cx,cy,18,cx,cy,154);glow.addColorStop(0,`rgba(${tone[0]},${tone[1]},${tone[2]},.46)`);glow.addColorStop(.43,`rgba(${tone[0]},${tone[1]},${tone[2]},.27)`);glow.addColorStop(.76,`rgba(${tone[0]},${tone[1]},${tone[2]},.10)`);glow.addColorStop(1,`rgba(${tone[0]},${tone[1]},${tone[2]},0)`);
  ctx.fillStyle=glow;ctx.fillRect(cx-170,cy-170,340,325);
}
...
function drawAssetRunway(){
  ...
  const laneTop=GROUND-34,fasciaTop=GROUND+45,pal=[
    ['#f3f0e8','#ddd9d1','#c9c6bf','#35424a','#76a7bb'],['#f1f0eb','#d9d7d1','#c7c6c2','#303a44','#8795b8'],
    ['#f4eee6','#dfd5c9','#c9beb2','#4a4239','#b49b70'],['#efefeb','#d4d4d1','#c1c1bf','#39383f','#9b8eb3']
  ][Math.max(0,Math.min(3,sceneIndex))];ctx.save();
  const lane=ctx.createLinearGradient(0,laneTop,0,fasciaTop);lane.addColorStop(0,pal[0]);lane.addColorStop(.42,pal[1]);lane.addColorStop(1,pal[2]);ctx.fillStyle=lane;ctx.fillRect(0,laneTop,W,fasciaTop-laneTop);
  ctx.fillStyle='rgba(255,255,255,.72)';ctx.fillRect(0,laneTop,W,3);ctx.fillStyle='rgba(42,48,53,.18)';ctx.fillRect(0,laneTop+4,W,2);
  // Tile joints begin below the character's feet so no vertical seam can visually cage the runner.
  ctx.strokeStyle='rgba(92,91,88,.22)';ctx.lineWidth=1;const seam=-((visualScrollPx*.82)%180)-180;for(let x=seam;x<W;x+=180){ctx.beginPath();ctx.moveTo(x,GROUND+11);ctx.lineTo(x,fasciaTop);ctx.stroke()}
  ctx.strokeStyle='rgba(255,255,255,.20)';ctx.beginPath();ctx.moveTo(0,GROUND+28.5);ctx.lineTo(W,GROUND+28.5);ctx.stroke();
  // Strong, simple contact edge: this is the only line the eye needs to read as the collision floor.
  ctx.fillStyle='rgba(255,255,255,.88)';ctx.fillRect(0,GROUND-2,W,2);ctx.fillStyle='rgba(25,31,36,.82)';ctx.fillRect(0,GROUND+2,W,4);ctx.fillStyle='rgba(25,31,36,.20)';ctx.fillRect(0,GROUND+8,W,1);
  const marker=-((visualScrollPx*1.02)%150)-150;ctx.fillStyle='rgba(37,43,48,.26)';for(let x=marker;x<W;x+=150)ctx.fillRect(x,GROUND+19,46,3);
  // Dark foreground fascia occupies the previously empty lower third and provides parallax speed.
  const fascia=ctx.createLinearGradient(0,fasciaTop,0,H);fascia.addColorStop(0,pal[3]);fascia.addColorStop(.22,'#30373c');fascia.addColorStop(1,'#20272c');ctx.fillStyle=fascia;ctx.fillRect(0,fasciaTop,W,H-fasciaTop);
  ctx.fillStyle='rgba(17,23,27,.92)';ctx.fillRect(0,fasciaTop,W,5);ctx.fillStyle='rgba(255,255,255,.09)';ctx.fillRect(0,fasciaTop+6,W,2);
  // Continuous foreground speed belt: no card-shaped panels, so the lower edge reads as world geometry rather than webpage UI.
  const accent=pal[4],belt=-((visualScrollPx*1.18)%210)-210;
  ctx.fillStyle=accent;ctx.globalAlpha=.24;for(let x=belt;x<W;x+=210){ctx.beginPath();ctx.moveTo(x,fasciaTop+23);ctx.lineTo(x+72,fasciaTop+23);ctx.lineTo(x+94,fasciaTop+31);ctx.lineTo(x+22,fasciaTop+31);ctx.closePath();ctx.fill()}
  ctx.globalAlpha=.12;ctx.fillStyle='#fff';const rail=-((visualScrollPx*1.42)%175)-175;for(let x=rail;x<W;x+=175)ctx.fillRect(x,fasciaTop+46,58,2);
  ctx.globalAlpha=.22;ctx.strokeStyle=accent;ctx.lineWidth=2;const slash=-((visualScrollPx*1.28)%280)-280;for(let x=slash;x<W;x+=280){ctx.beginPath();ctx.moveTo(x,fasciaTop+66);ctx.lineTo(x+88,fasciaTop+66);ctx.lineTo(x+116,fasciaTop+74);ctx.stroke()}
  ctx.globalAlpha=1;
  const shade=ctx.createLinearGradient(0,H-42,0,H);shade.addColorStop(0,'rgba(8,12,15,0)');shade.addColorStop(1,'rgba(8,12,15,.34)');ctx.fillStyle=shade;ctx.fillRect(0,H-42,W,42);ctx.restore()
}
...
function drawEndingCinematicFx(){
  ...
  ctx.save();
  if(endingCinematicType==='ontime'){
    const a=Math.max(0,Math.min(1,1-endingCinematicTimer/1.55));ctx.fillStyle=`rgba(240,212,135,${.08+.12*a})`;ctx.fillRect(0,0,W,GROUND);
    ...
    const a=Math.max(0,Math.min(1,1-endingCinematicTimer/1.75));ctx.fillStyle=`rgba(55,48,74,${.10+.24*a})`;ctx.fillRect(0,0,W,GROUND);
...
function drawCeilingDetails(offset=0,tone='#d9d2c6'){
  ...
  const o=-((visualScrollPx*.12+offset)%330)-330;
  for(let i=-1;i<6;i++){
    const x=i*330+o;
...
function drawWorkstationScene(){
  ...
  const back=-((visualScrollPx*.16)%430)-430;
  for(let i=-1;i<5;i++){
  ...
  const pod=-((visualScrollPx*.46)%380)-80;for(let i=-1;i<5;i++)drawWorkPod(i*380+pod,335);drawWorkstationLife(pod);
}
...
function drawMeetingRoomScene(){
  ...
  const module=-((visualScrollPx*.44)%500)-120;for(let i=-1;i<4;i++)drawMeetingPod(i*500+module,408);drawMeetingLife(module);
}
...
function drawPantryScene(){
  ...
  for(let x=-((visualScrollPx*.15)%110)-110;x<W;x+=110){ctx.beginPath();ctx.moveTo(x,116);ctx.lineTo(x,258);ctx.stroke()}
  const back=-((visualScrollPx*.16)%520)-520;
  for(let i=-1;i<5;i++){
    const x=i*520+back;
  ...
  const pod=-((visualScrollPx*.44)%540)-120;for(let i=-1;i<4;i++)drawPantryPod(i*540+pod,370);drawPantryLife(pod);
}
...
function drawGymScene(){
  ...
  const pod=-((visualScrollPx*.46)%520)-100;for(let i=-1;i<4;i++)drawGymPod(i*520+pod,418);drawGymLife(pod);
}
...
function drawMeetingLife(module){
  ...
  ctx.globalAlpha=.26+.12*pulse;ctx.fillStyle='#6d96aa';for(let i=-1;i<5;i++){const x=i*460-((visualScrollPx*.16)%460)-460;ctx.fillRect(x+72,146,48+((i&1)*18),3)}
  ctx.restore()
}
...
function drawGymLife(pod){
  ...
  ctx.globalAlpha=.10;ctx.fillStyle='#6f7f92';const ghost=-((visualScrollPx*.08)%420);for(let x=-420+ghost;x<W+420;x+=420){ctx.beginPath();ctx.ellipse(x+128,234,14,21,0,0,Math.PI*2);ctx.fill();ctx.fillRect(x+116,254,24,54)}ctx.restore()
}
...
function drawSharedFloor(){
  ...
  const tileX=-((visualScrollPx*.82)%150)-150;
  ...
  const markerX=-((visualScrollPx)%140)-140;
...
function drawAmbientOfficeLife(){
  ...
  const drift=-((visualScrollPx*.18)%520);
  if(sceneIndex===0){
...
function drawRunAtmosphere(){
  ...
  ctx.save();
  // Three subtle motion planes keep the office from reading as a static backdrop.
  const farShift=(visualScrollPx*.11)%360,nearShift=(visualScrollPx*.84)%230,motion=Math.max(0,Math.min(1,(speed-330)/360));
  ctx.globalAlpha=.045+.025*motion;ctx.fillStyle='#7691a0';for(let x=-360-farShift;x<W+360;x+=360)ctx.fillRect(x,118,88,3);
  ctx.globalAlpha=.055+.055*motion;ctx.fillStyle='#171717';for(let x=-230-nearShift;x<W+230;x+=230){ctx.save();ctx.translate(x,GROUND+25);ctx.transform(1,0,-.55,1,0,0);ctx.fillRect(0,0,74,3);ctx.restore()}
  ctx.globalAlpha=.04+.05*motion;const edge=ctx.createLinearGradient(0,0,W,0);edge.addColorStop(0,'rgba(23,23,23,.72)');edge.addColorStop(.08,'rgba(23,23,23,0)');edge.addColorStop(.92,'rgba(23,23,23,0)');edge.addColorStop(1,'rgba(23,23,23,.72)');ctx.fillStyle=edge;ctx.fillRect(0,0,W,GROUND);ctx.globalAlpha=1;
  // Shift the office from cool afternoon into warmer late-day pressure without changing collision readability.
  if(day>.18){
    const a=(day-.18)*.075;const g=ctx.createLinearGradient(0,0,W,H);g.addColorStop(0,'rgba(255,236,190,0)');g.addColorStop(1,`rgba(242,171,116,${a.toFixed(3)})`);ctx.fillStyle=g;ctx.fillRect(0,0,W,GROUND);
  }
  if(speed>470){
  ...
  if(combo>=5){const flow=Math.min(1,(combo-4)/10),shift=(worldTime*(340+speed*.25))%210;ctx.strokeStyle=`rgba(80,107,44,${(.10+.10*flow).toFixed(3)})`;ctx.lineWidth=2+flow;for(let y=330;y<GROUND+45;y+=46){ctx.beginPath();ctx.moveTo(W-((shift+y*1.7)%210),y);ctx.lineTo(W-70-((shift+y*1.7)%210),y);ctx.stroke()}}
  ctx.restore();
}
...
function drawRunnerSilhouetteHalo(drawX,footY,altitude){
  if(state!=='playing'&&state!=='ending')return;const cx=drawX+player.w*.5,cy=footY-58,r=72+Math.min(22,altitude*.08);
  ctx.save();const g=ctx.createRadialGradient(cx,cy,12,cx,cy,r);g.addColorStop(0,'rgba(255,253,248,.16)');g.addColorStop(.48,'rgba(255,253,248,.075)');g.addColorStop(1,'rgba(255,253,248,0)');ctx.fillStyle=g;ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fill();ctx.restore();
}
function drawPlayerVector(){
  const grounded=player.y>=GROUND-player.h-1,airborne=!grounded;
  const cadence=10.5+Math.min(6.5,(speed-350)/92),phase=worldTime*cadence,run=Math.sin(phase),step=Math.cos(phase);
  ...
  const rise=airborne?Math.max(0,Math.min(1,-player.vy/720)):0,fall=airborne?Math.max(0,Math.min(1,player.vy/820)):0,double=airborne&&player.jumps>=2?1:0;
  const landing=grounded?Math.max(0,Math.min(1,player.squash/.105)):0,bob=grounded?-(1-Math.abs(step))*1.5:0;
  ...
  if(grounded&&speed>520){ctx.save();ctx.globalAlpha=Math.min(.09,(speed-500)/1900);ctx.strokeStyle='#171717';ctx.lineWidth=2.5;for(let i=0;i<3;i++){const yy=footY-15-i*12;ctx.beginPath();ctx.moveTo(drawX-24-i*11,yy);ctx.lineTo(drawX-5,yy);ctx.stroke()}ctx.restore()}

  ctx.save();ctx.translate(drawX+player.w/2,footY+bob);
  const lean=grounded?(.045+run*.012):(rise?-.025*rise:(.035*fall));
  const sx=1.10*(1+landing*.035),sy=1.10*(1-landing*.075);ctx.rotate(lean);ctx.scale(sx,sy);
  function limb(a,b,c,d,e,f,w=5,color='#2c2c2c',shoe=false){ctx.strokeStyle=color;ctx.lineWidth=w;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();ctx.moveTo(a,b);ctx.lineTo(c,d);ctx.lineTo(e,f);ctx.stroke();if(shoe){ctx.fillStyle='#252525';ctx.save();ctx.translate(e,f);ctx.rotate(-.03);rr(-5,-3,13,5,1.5);ctx.fill();ctx.restore()}}

  let backLeg,frontLeg,backArm,frontArm;
  if(grounded){
    const stride=run*13,heel=Math.max(0,step)*2.5;
    backLeg={hip:[-4,-20],knee:[-5-stride*.32,-10-Math.max(0,-step)*2],foot:[-8-stride,-heel]};
    frontLeg={hip:[5,-20],knee:[5+stride*.32,-10-Math.max(0,step)*2],foot:[9+stride,Math.max(0,-step)*1.2]};
    backArm={shoulder:[-3,-40],elbow:[-10+run*6,-31],hand:[-13+run*10,-23]};
    frontArm={shoulder:[6,-40],elbow:[13-run*6,-31],hand:[16-run*10,-24]};
  }else{
    const tuck=Math.max(rise,double*.82),brace=fall;
    backLeg={hip:[-4,-20],knee:[-10-5*tuck,-12-7*tuck],foot:[-14-4*tuck,-3-9*tuck+5*brace]};
    frontLeg={hip:[5,-20],knee:[9+7*tuck,-11-8*tuck],foot:[15+6*tuck,-2-8*tuck+6*brace]};
    backArm={shoulder:[-3,-40],elbow:[-12-6*tuck,-32-5*tuck+3*brace],hand:[-16-8*tuck,-24-9*tuck+7*brace]};
    frontArm={shoulder:[6,-40],elbow:[15+6*tuck,-31-4*tuck+3*brace],hand:[20+8*tuck,-24-7*tuck+7*brace]};
  }

  // Rear limbs first: the silhouette now reads as a runner moving to the right instead of a front-facing puppet.
  limb(...backLeg.hip,...backLeg.knee,...backLeg.foot,6,'#404040',true);
  limb(...backArm.shoulder,...backArm.elbow,...backArm.hand,5,'#303030');

  // Messenger bag stays behind the torso and follows the gait with a much smaller swing.
  const bagSwing=grounded?-run*2.2:(2.5+fall*2);ctx.save();ctx.translate(-11+bagSwing,-31);ctx.rotate(-.10-run*.025);ctx.fillStyle='#8b6545';ctx.strokeStyle='#171717';ctx.lineWidth=2;rr(-7,-2,18,20,3);ctx.fill();ctx.stroke();ctx.restore();

  // Slightly side-on torso with forward shoulder; keep the visual body inside the original collision box footprint.
  ctx.fillStyle='#fffdf8';ctx.strokeStyle='#171717';ctx.lineWidth=2.2;rr(-13,-46,28,28,5);ctx.fill();ctx.stroke();
  ctx.fillStyle='#d7ecfb';ctx.beginPath();ctx.moveTo(-8,-44);ctx.lineTo(-1,-37);ctx.lineTo(-6,-33);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(10,-44);ctx.lineTo(2,-37);ctx.lineTo(7,-33);ctx.closePath();ctx.fill();
  ctx.strokeStyle='#6f6a61';ctx.lineWidth=1.3;ctx.beginPath();ctx.moveTo(-6,-43);ctx.lineTo(1,-29);ctx.lineTo(8,-43);ctx.stroke();
  const tieKick=Math.min(9,(speed-350)/62)+(airborne?5:Math.abs(run)*2);ctx.fillStyle='#171717';ctx.beginPath();ctx.moveTo(-2,-39);ctx.lineTo(3,-39);ctx.lineTo(7+tieKick,-25);ctx.lineTo(1,-20);ctx.lineTo(-3,-26);ctx.closePath();ctx.fill();
  ctx.fillStyle='#f3df9d';ctx.strokeStyle='#171717';ctx.lineWidth=1;rr(-4,-31,9,8,1);ctx.fill();ctx.stroke();

  // Front limbs overlay the torso, which gives the run cycle depth instead of four limbs radiating from the body.
  limb(...frontLeg.hip,...frontLeg.knee,...frontLeg.foot,6,'#454545',true);
  limb(...frontArm.shoulder,...frontArm.elbow,...frontArm.hand,5,'#272727');

  // 3/4 profile head facing right: nose, single dominant eye and asymmetric hair remove the uncanny front-mask look.
  ctx.fillStyle='#e9d1b0';ctx.beginPath();ctx.arc(2,-55,14,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#171717';ctx.lineWidth=2;ctx.stroke();
  ctx.fillStyle='#171717';ctx.beginPath();ctx.arc(0,-59,14,Math.PI*1.02,Math.PI*1.93);ctx.lineTo(11,-60);ctx.quadraticCurveTo(7,-69,-6,-68);ctx.quadraticCurveTo(-14,-64,-13,-56);ctx.closePath();ctx.fill();
  ctx.fillStyle='#e9d1b0';ctx.beginPath();ctx.arc(-11,-54,3,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle='#171717';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(15,-54);ctx.lineTo(19,-52);ctx.lineTo(15,-50);ctx.stroke();
  const blink=(worldTime%4.7)>4.58;ctx.fillStyle='#171717';if(blink)ctx.fillRect(6,-56,5,1);else ctx.fillRect(8,-57,3,3);
  ctx.strokeStyle='#8a5f4c';ctx.lineWidth=1.2;ctx.beginPath();ctx.arc(7,-49,5,.15*Math.PI,.62*Math.PI);ctx.stroke();
  if(speed>610||stageIndex>=4){ctx.strokeStyle='#171717';ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(5,-61);ctx.lineTo(11,-60);ctx.stroke()}
  ctx.restore();
}
...
const OFFICE_HAZARD_ATLAS={
  src:'assets/sprites/office-hazards-v124.webp?v=1240',width:480,height:288,
  // v1.24: compact cuts from the user's original high-resolution office-runner art board.
  frames:{
    bossPatrol:[0,0,96,96],bossRush:[96,0,96,96],bugNormal:[192,0,96,96],bugTall:[288,0,96,96],bugLong:[384,0,96,96],
    request:[0,96,96,96],coffeeSpill:[96,96,96,96],dumbbell:[192,96,96,96],meetingOpen:[288,96,96,96],coffeePickup:[384,96,96,96],
    mail:[0,192,96,96],riskPickup:[96,192,96,96],leavePickup:[192,192,96,96]
  }
};
...
function drawPlayerSprite(){
  ...
  drawRunnerSilhouetteHalo(drawX,footY,altitude);
  ctx.save();ctx.globalAlpha=.20*shadowScale;ctx.fillStyle='#171717';ctx.beginPath();ctx.ellipse(drawX+player.w/2,GROUND+3,26*shadowScale,5.3*shadowScale,0,0,Math.PI*2);ctx.fill();ctx.restore();
  const size=HERO_SPRITE.display,scale=size/cell,dx=drawX+player.w*.5-size*.5,dy=footY-HERO_SPRITE.baselineY*scale;
  ctx.save();ctx.imageSmoothingEnabled=true;if(state==='playing'&&hitInvulnTimer>0&&hitFlashTimer<=0)ctx.globalAlpha=(Math.floor(hitInvulnTimer*7)%2)?0.82:0.98;ctx.drawImage(heroSpriteImage,col*cell,row*cell,cell,cell,dx,dy,size,size);ctx.restore()
}
...
function drawBossArt(o){
  ...
  if(rushing){ctx.save();ctx.globalAlpha=.10;ctx.fillStyle='#9c3f2f';for(let i=0;i<3;i++){ctx.beginPath();ctx.moveTo(-30-i*13,-58+i*7);ctx.lineTo(-8-i*9,-58+i*7);ctx.lineTo(-8-i*9,-54+i*7);ctx.lineTo(-30-i*13,-54+i*7);ctx.closePath();ctx.fill()}ctx.restore()}
  // Thick, compact silhouette: visually fills the same 60×92 logical obstacle instead of looking like a thin stick figure.
  const leg=rushing?stride*5:stride*3.3;
  ctx.strokeStyle='#171717';ctx.lineCap='round';ctx.lineJoin='round';ctx.lineWidth=7;
  ctx.beginPath();ctx.moveTo(-9,-24);ctx.lineTo(-11-leg,-9);ctx.lineTo(-14-leg*1.35,-1);ctx.moveTo(9,-24);ctx.lineTo(12+leg,-10);ctx.lineTo(16+leg*1.35,-1);ctx.stroke();
  ctx.strokeStyle='#242629';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(-18-leg*1.35,-1);ctx.lineTo(-8-leg*1.35,-1);ctx.moveTo(12+leg*1.35,-1);ctx.lineTo(22+leg*1.35,-1);ctx.stroke();
  // Broad suit body and lapels.
  ctx.fillStyle='#25303a';ctx.strokeStyle='#171717';ctx.lineWidth=2.8;ctx.beginPath();ctx.moveTo(-19,-57);ctx.quadraticCurveTo(-23,-44,-20,-22);ctx.lineTo(20,-22);ctx.quadraticCurveTo(23,-44,18,-57);ctx.lineTo(7,-62);ctx.lineTo(-8,-62);ctx.closePath();ctx.fill();ctx.stroke();
  ctx.fillStyle='#fffdf8';ctx.beginPath();ctx.moveTo(-9,-59);ctx.lineTo(0,-43);ctx.lineTo(9,-59);ctx.lineTo(5,-62);ctx.lineTo(-5,-62);ctx.closePath();ctx.fill();
  ctx.fillStyle='#b7483c';ctx.beginPath();ctx.moveTo(-2,-55);ctx.lineTo(3,-55);ctx.lineTo(6,-33);ctx.lineTo(0,-27);ctx.lineTo(-5,-34);ctx.closePath();ctx.fill();
  // Arms stay mostly inside the logical width; briefcase slightly overlaps the silhouette but not the collision read.
  ctx.strokeStyle='#25303a';ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(-16,-51);ctx.lineTo(-24-stride*2,-36);ctx.moveTo(16,-51);ctx.lineTo(23+stride*2,-37);ctx.stroke();
  ctx.fillStyle='#e6c2a4';ctx.strokeStyle='#171717';ctx.lineWidth=1.8;ctx.beginPath();ctx.arc(24+stride*2,-36,5,0,Math.PI*2);ctx.fill();ctx.stroke();
  ctx.fillStyle='#725a42';ctx.strokeStyle='#171717';ctx.lineWidth=2.2;rr(-29-stride*2,-35,18,22,3);ctx.fill();ctx.stroke();ctx.strokeRect(-24-stride*2,-39,9,5);
  // Large face, hair and sunglasses keep the boss readable at play speed.
  ctx.fillStyle='#e8c5a7';ctx.strokeStyle='#171717';ctx.lineWidth=2.4;ctx.beginPath();ctx.ellipse(0,-72,15,16,0,0,Math.PI*2);ctx.fill();ctx.stroke();
  ctx.fillStyle='#24211f';ctx.beginPath();ctx.arc(-1,-77,14,Math.PI*1.02,Math.PI*1.94);ctx.lineTo(12,-75);ctx.quadraticCurveTo(5,-89,-9,-85);ctx.quadraticCurveTo(-15,-81,-14,-73);ctx.closePath();ctx.fill();
  ctx.fillStyle='#171717';ctx.fillRect(-11,-76,9,5);ctx.fillRect(3,-76,9,5);ctx.fillRect(-2,-75,5,2);ctx.strokeStyle='#171717';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(-12,-77);ctx.lineTo(-16,-79);ctx.moveTo(12,-77);ctx.lineTo(16,-79);ctx.stroke();
  ctx.strokeStyle='#8f5a45';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(-5,-65);ctx.quadraticCurveTo(2,-61,8,-66);ctx.stroke();
  if(rushing||warning){const wp=warning?.58+.42*Math.abs(Math.sin(worldTime*19)):1;ctx.globalAlpha=wp;ctx.fillStyle='#d95a49';ctx.font='950 24px ui-monospace,monospace';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('!',-23,-78);ctx.globalAlpha=1}
  ctx.restore()
}
function drawBugArt(o){
  ...
  if(officeHazardReady){let key='bugNormal',dw=64,dh=58;if(o.mutationState==='grow'||o.mutationState==='done'){if(o.mutation==='long'){key='bugLong';dw=120;dh=62}else if(o.mutation==='tall'){key='bugTall';dw=68;dh=96}}const dx=o.w*.5-dw*.5,dy=o.h-dh+4;if(warn){ctx.save();ctx.globalAlpha=.16+.10*pulse;ctx.fillStyle='#d95a49';ctx.beginPath();ctx.ellipse(o.w*.5,o.h*.55,Math.max(30,o.w*.62),Math.max(23,o.h*.84),0,0,Math.PI*2);ctx.fill();ctx.restore()}drawOfficeHazardFrame(key,dx,dy,dw,dh,warn?.72+.28*pulse:1);if(warn){ctx.save();ctx.globalAlpha=pulse;ctx.fillStyle='#d95a49';ctx.font='950 18px ui-monospace,monospace';ctx.textAlign='center';ctx.fillText('!',o.w*.5,dy+5);ctx.restore()}return}
  ...
  ctx.fillStyle=mut?'#51466f':'#655a86';ctx.beginPath();ctx.ellipse(cx,cy+5,Math.max(18,o.w*.39),Math.max(10,o.h*.31),0,0,Math.PI*2);ctx.fill();ctx.stroke();
  ctx.fillStyle=mut?'#70649a':'#8176a8';ctx.beginPath();ctx.ellipse(cx,Math.min(14,o.h*.34),Math.max(14,o.w*.29),Math.max(9,o.h*.24),0,0,Math.PI*2);ctx.fill();ctx.stroke();
  ctx.strokeStyle='#171717';ctx.lineWidth=1.8;ctx.beginPath();ctx.moveTo(cx-8,7);ctx.lineTo(cx-15,-3);ctx.moveTo(cx+8,7);ctx.lineTo(cx+15,-3);ctx.stroke();
  ctx.fillStyle='#ff5b4f';ctx.beginPath();ctx.ellipse(cx-6,Math.min(13,o.h*.32),4.4,2.5,-.16,0,Math.PI*2);ctx.ellipse(cx+6,Math.min(13,o.h*.32),4.4,2.5,.16,0,Math.PI*2);ctx.fill();ctx.fillStyle='#171717';ctx.beginPath();ctx.arc(cx-5,Math.min(13,o.h*.32),1.3,0,Math.PI*2);ctx.arc(cx+7,Math.min(13,o.h*.32),1.3,0,Math.PI*2);ctx.fill();
...
function drawRequestArt(o){
  if(officeHazardReady){if(o.dropState==='warning'){const pulse=.45+.55*Math.abs(Math.sin(o.warningPulse)),target=o.targetY-o.y;ctx.save();ctx.globalAlpha=.24+.22*pulse;ctx.strokeStyle='#d95a49';ctx.lineWidth=3;ctx.setLineDash([8,7]);ctx.strokeRect(0,target,o.w,o.h);ctx.setLineDash([]);ctx.restore();drawOfficeHazardFrame('request',o.w*.5-42,target-68,84,104,.28+.18*pulse);return}const dw=86,dh=108;drawOfficeHazardFrame('request',o.w*.5-dw*.5,o.h-dh+8,dw,dh);return}
  ...
  ctx.save();ctx.translate(56,13);ctx.rotate(-.15);ctx.strokeStyle='#b7483c';ctx.lineWidth=2.4;ctx.strokeRect(-10,-7,20,14);ctx.fillStyle='#b7483c';ctx.font='950 7px ui-monospace,monospace';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('ASAP',0,0);ctx.restore();
  ctx.fillStyle='#6a5848';ctx.strokeStyle='#171717';ctx.lineWidth=2;rr(62,27,18,22,3);ctx.fill();ctx.stroke();ctx.fillStyle='#c7a66b';ctx.fillRect(67,23,8,5);
...
function drawSmallHazardArt(o){
  ...
  if(o.label==='咖啡渍'){
    ...
    ctx.fillStyle='#3b3d40';ctx.strokeStyle='#171717';ctx.lineWidth=2;rr(13,12,o.w-26,8,3);ctx.fill();ctx.stroke();for(const x of [2,o.w-18]){ctx.fillStyle='#252629';rr(x,4,16,24,3);ctx.fill();ctx.stroke();ctx.fillStyle='#55585c';rr(x+4,7,8,18,2);ctx.fill()}ctx.fillStyle='rgba(255,255,255,.24)';ctx.fillRect(19,14,o.w-38,2)
  }else if(o.label==='邮件'){
    ctx.translate(0,Math.sin(o.wave||0)*1.5);ctx.fillStyle='#fffefb';ctx.strokeStyle='#171717';ctx.lineWidth=2.6;rr(4,8,44,25,4);ctx.fill();ctx.stroke();ctx.strokeStyle='#6489a6';ctx.beginPath();ctx.moveTo(7,11);ctx.lineTo(26,24);ctx.lineTo(45,11);ctx.stroke();ctx.strokeStyle='#171717';ctx.beginPath();ctx.moveTo(7,31);ctx.lineTo(19,21);ctx.moveTo(45,31);ctx.lineTo(33,21);ctx.stroke();ctx.fillStyle='#d95a49';ctx.beginPath();ctx.arc(49,9,10,0,Math.PI*2);ctx.fill();ctx.fillStyle='#fff';ctx.font='900 12px ui-monospace,monospace';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('!',49,9);ctx.strokeStyle='rgba(23,23,23,.32)';ctx.beginPath();ctx.moveTo(-12,14);ctx.lineTo(-1,14);ctx.moveTo(-8,22);ctx.lineTo(-1,22);ctx.stroke()
  }
}
function drawObstacle(o){
  ...
  if(o.label==='老板'){
    ...
    // v1.26: the meeting gate reads as actual conference-room furniture: a lowered projector screen over a solid AV console.
    ctx.fillStyle='#343e45';rr(3,1,o.w-6,10,4);ctx.fill();
    ctx.fillStyle='#f6f3ea';ctx.strokeStyle='#20272c';ctx.lineWidth=2.5;rr(7,9,o.w-14,Math.max(18,panelH-16),5);ctx.fill();ctx.stroke();
    const innerH=Math.max(12,panelH-34);ctx.fillStyle='#edf5f6';rr(14,16,o.w-28,innerH,3);ctx.fill();
    ctx.strokeStyle='#6d8996';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(22,23+innerH*.66);ctx.lineTo(43,23+innerH*.42);ctx.lineTo(62,23+innerH*.53);ctx.lineTo(82,23+innerH*.24);ctx.lineTo(o.w-24,23+innerH*.38);ctx.stroke();
    ctx.fillStyle='#d99955';ctx.beginPath();ctx.arc(43,23+innerH*.42,3.5,0,Math.PI*2);ctx.fill();ctx.fillStyle='#6e9d7b';ctx.beginPath();ctx.arc(82,23+innerH*.24,3.5,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='#454e54';ctx.fillRect(6,panelH-9,o.w-12,7);ctx.fillStyle='#8b969c';ctx.fillRect(o.w*.5-12,panelH-2,24,4);
    ctx.fillStyle='#ad7448';ctx.strokeStyle='#20272c';ctx.lineWidth=2.5;rr(2,tableY,o.w-4,12,4);ctx.fill();ctx.stroke();
    ctx.fillStyle='#3d474e';rr(5,tableY+11,o.w-10,Math.max(8,o.tableH-11),4);ctx.fill();
    ctx.fillStyle='#56636b';ctx.fillRect(13,tableY+20,o.w-26,4);
    // Chunky gap markers replace the old hanging guide wires; the obstacle reads as two solid masses with a clean pass-through opening.
    ctx.globalAlpha=o.firstGate?.78:(.58+.16*pulse);ctx.fillStyle='#78934d';
    for(const x of [10,o.w-18]){rr(x,panelH+3,8,13,3);ctx.fill();rr(x,tableY-16,8,13,3);ctx.fill()}
    for(const x of [15,o.w-15]){ctx.beginPath();ctx.moveTo(x-5,panelH+18);ctx.lineTo(x+5,panelH+18);ctx.lineTo(x,panelH+25);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(x-5,tableY-18);ctx.lineTo(x+5,tableY-18);ctx.lineTo(x,tableY-25);ctx.closePath();ctx.fill()}ctx.globalAlpha=1;
  } else if(o.label==='BUG'){
    drawBugArt(o);
  } else if(o.label==='临时需求'){
    drawRequestArt(o);
  } else if(o.label==='咖啡渍'||o.label==='哑铃'||o.label==='邮件'){
    drawSmallHazardArt(o);
  }
  ctx.restore();
}
function drawPickup(p){
  const yy=p.y+Math.sin(p.spin)*5;ctx.save();ctx.translate(p.x,yy);
  if(officeHazardReady){const key=p.kind==='leave'?'leavePickup':(p.kind==='risk'?'riskPickup':'coffeePickup');const dims=p.kind==='leave'?[62,52]:(p.kind==='risk'?[44,68]:[50,72]),dw=dims[0],dh=dims[1];ctx.globalAlpha=.11+.04*Math.sin(p.spin*2);ctx.fillStyle=p.kind==='leave'?'#d8ef9f':'#f0d487';ctx.beginPath();ctx.ellipse(15,20,dw*.38,dh*.34,0,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;drawOfficeHazardFrame(key,15-dw*.5,20-dh*.5,dw,dh);ctx.restore();return}
...
function drawRunPickupEffects(){
  ...
  const cx=player.x+player.w/2,foot=player.y+player.h;
  if(leaveSlipTimer>0){const pulse=.45+.15*Math.sin(worldTime*7);ctx.globalAlpha=.26+pulse*.12;ctx.strokeStyle='#506b2c';ctx.lineWidth=2.2;ctx.beginPath();ctx.arc(cx,foot-22,27,-Math.PI*.82,Math.PI*.12);ctx.stroke()}
  if(riskBoostTimer>0){const pulse=.42+.14*Math.sin(worldTime*9);ctx.globalAlpha=.24+pulse*.12;ctx.strokeStyle='#b1862c';ctx.lineWidth=2.2;ctx.beginPath();ctx.arc(cx,foot-22,31,Math.PI*.16,Math.PI*.94);ctx.stroke()}
  ctx.restore()
}
function drawGameFeelFx(){
  if(state!=='playing'&&state!=='ending')return;const cx=player.x+player.w*.5,cy=player.y+player.h*.52;ctx.save();
...
if(DEBUG_MODE)window.__GAME_TEST__={
  ...
  debugHeroSprite:()=>({ready:heroSpriteReady,failed:heroSpriteFailed,frame:heroSpriteFrame(),src:HERO_SPRITE.src,display:HERO_SPRITE.display,runStep:+heroRunFrameStep().toFixed(2)}),debugHazardAtlas:()=>({ready:officeHazardReady,failed:officeHazardFailed,src:OFFICE_HAZARD_ATLAS.src,frames:{...OFFICE_HAZARD_ATLAS.frames}}),debugSceneBackdrop:()=>({ready:[...sceneBackdropReady],failed:[...sceneBackdropFailed],src:[...SCENE_BACKDROPS]}),debugHeroPose:(patch={},time=null)=>{cancelAnimationFrame(raf);tutorialActive=false;tutorialToast.classList.add('hidden');state='playing';if(Number.isFinite(Number(time)))worldTime=Number(time);Object.assign(player,patch);draw();return {sprite:window.__GAME_TEST__.debugHeroSprite(),player:{...player},ground:GROUND,worldTime}},
