...
function heroSpriteFrame(){
  ...
  if(state==='gameover')return HERO_SPRITE.hurt;
...
function drawPlayerSprite(){
  ...
  ctx.save();ctx.imageSmoothingEnabled=true;ctx.drawImage(heroSpriteImage,col*cell,row*cell,cell,cell,dx,dy,size,size);ctx.restore()
}
...
function drawGameFeelFx(){
  ...
  if(feelFlash>0){const a=Math.min(.16,feelFlash*(feelKind==='perfect'?1.15:.68));ctx.globalAlpha=a;ctx.fillStyle=feelKind==='perfect'?'#d8ef9f':(feelKind==='boss'?'#d95a49':'#fffdf8');ctx.fillRect(0,0,W,H)}
...
if(DEBUG_MODE)window.__GAME_TEST__={
  initialized:true,canvas:Boolean(ctx),version:'1.16.1',dayEndDistance:DAY_END_DISTANCE,
...
};document.documentElement.dataset.gameReady='true';document.documentElement.dataset.messageEnabled=MESSAGE_ENABLED?'true':'false';document.documentElement.dataset.gameVersion='1.16.1';
})();
