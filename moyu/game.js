...
function takeObstacleHit(o){
  ...
  const left=MAX_MISTAKES-mistakes;addFloater(player.x+48,player.y-22,currentLang==='en'?`Mistake ${mistakes}/${MAX_MISTAKES} · ${left} left`:`失误 ${mistakes}/${MAX_MISTAKES} · 还剩 ${left} 次`,'#9c3f2f');tickerEl.textContent=currentLang==='en'?`${tr(o.label)} clipped you. ${left} buffer${left===1?'':'s'} left.`:`撞上「${o.label}」，但本局还剩 ${left} 次容错。`;tickerTimer=Math.max(tickerTimer,3.4);if(navigator.vibrate)navigator.vibrate(24);beep(170,.08,'sawtooth',.04);setTimeout(()=>beep(260,.045,'square',.024),55);return true
}
...
if(DEBUG_MODE)window.__GAME_TEST__={
  initialized:true,canvas:Boolean(ctx),version:'1.24.0',dayEndDistance:DAY_END_DISTANCE,
...
};document.documentElement.dataset.gameReady='true';document.documentElement.dataset.messageEnabled=MESSAGE_ENABLED?'true':'false';document.documentElement.dataset.gameVersion='1.24.0';
})();
