...
function queueAI(delay=220){
  ...
  aiTimer=setTimeout(()=>{
    ...
    try{
      aiWorker=aiWorker||new Worker('ai-worker.js?v=070');
...
function placeNotice(){if(noticeEl.hidden)return;const toolbarEl=document.querySelector('.toolbar'),setupEl=document.querySelector('.ai-setup'),matchEl=document.querySelector('.matchbar'),shown=e=>e&&getComputedStyle(e).display!=='none';noticeEl.style.top=((toolbarEl?.offsetHeight||0)+(shown(setupEl)?setupEl.offsetHeight:0)+(shown(matchEl)?matchEl.offsetHeight:0)+8)+'px'}
function showNotice(text,tone='info',ms=1800){clearTimeout(noticeTimer);noticeEl.hidden=false;noticeEl.dataset.tone=tone;noticeEl.textContent=text;requestAnimationFrame(placeNotice);noticeTimer=setTimeout(clearNotice,ms)}
...
function sync(){
  ...
  gameName.textContent=nameOf(mode);moveCount.textContent=String(Math.max(0,history.length-1));goCaptureStats.hidden=mode!=='go';goBlackCaptures.textContent=T(`黑提 ${captures.b}`,`Black captured ${captures.b}`);goWhiteCaptures.textContent=T(`白提 ${captures.w}`,`White captured ${captures.w}`);
  ...
  undoBtn.disabled=inReview||scoring||!!winner||(!aiBusy&&!pending&&history.length<=1);restartBtn.disabled=inReview||(history.length<=1&&!pending&&!scoring&&!(aiEnabled()&&matchStarted));resignBtn.disabled=waitingStart||inReview||aiTurn||aiBusy||!!winner||scoring;
  ...
  aiState.dataset.busy=String(aiBusy);playcard.dataset.aiTurn=String(aiTurn||aiBusy);playcard.dataset.waitingStart=String(waitingStart);playcard.dataset.started=String(matchStarted);playcard.dataset.opponent=opponent;
...
function resize(){const wrap=canvas.parentElement,ratio=mode==='xiangqi'?10/9:1,compactLandscape=!document.fullscreenElement&&innerWidth<=1024&&innerWidth>innerHeight&&innerHeight<720,minBoard=compactLandscape?150:210,widthCap=Math.max(minBoard,Math.min(wrap.clientWidth-2,mode==='xiangqi'?760:860));let heightCap=widthCap;if(document.fullscreenElement)heightCap=Math.max(180,(wrap.clientHeight-2)/ratio);else if(compactLandscape){const available=Math.max(150,innerHeight-wrap.getBoundingClientRect().top-8);heightCap=available/ratio}const w=Math.max(minBoard,Math.min(widthCap,heightCap)),cssH=Math.round(w*ratio);canvas.style.width=w+'px';canvas.style.height=cssH+'px';const dpr=Math.min(2,window.devicePixelRatio||1);canvas.width=Math.round(w*dpr);canvas.height=Math.round(cssH*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);draw()}
...
window.addEventListener('resize',()=>{resize();placeNotice()},{passive:true});window.addEventListener('board:layout',()=>{resize();placeNotice()});window.addEventListener('board:language',()=>{cancelClearData();sync();draw()});window.addEventListener('board:notice',ev=>showNotice(ev.detail&&ev.detail.text||T('操作不可用','Action unavailable'),'warn',2400));
...
canvas.tabIndex=0;reset();resize();setInterval(tickClocks,250);window.addEventListener('beforeunload',cancelAI);document.documentElement.dataset.gameVersion='0.7.0';
})();
