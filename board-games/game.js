...
function saveSession(key=activeKey){if(!key||!board)return;sessions[key]={board,turn,winner,resultNote,selected,targets,history,last,passes,captures,cursor,scoring,dead:new Set(dead),scoreApprover}}
function loadSession(key){const s=sessions[key];if(!s)return false;board=s.board;turn=s.turn;winner=s.winner;resultNote=s.resultNote||'';selected=s.selected;targets=s.targets;history=s.history;last=s.last;passes=s.passes;captures=s.captures;cursor=s.cursor;pending=null;scoring=!!s.scoring;dead=new Set(s.dead||[]);scoreApprover=s.scoreApprover||'b';return true}
function freshBoard(){winner=null;resultNote='';selected=null;targets=[];history=[];last=null;passes=0;captures={b:0,w:0};cursor=null;pending=null;scoring=false;dead=new Set();scoreApprover='b';turn=mode==='xiangqi'?'r':'b';board=mode==='gomoku'?R.gomokuBoard():mode==='xiangqi'?R.xiangqiBoard():R.goBoard(+sizeSelect.value);snapshot()}
function reset(){freshBoard();sessions[activeKey]={board,turn,winner,resultNote,selected,targets,history,last,passes,captures,cursor,scoring,dead:new Set(dead),scoreApprover};cancelRestart();cancelResign();clearNotice();sync();draw()}
function stateSnapshot(){return{board:R.clone(board),turn,winner,resultNote,selected:selected&&{...selected},last:last&&{...last},passes,captures:{...captures},scoring,dead:[...dead],scoreApprover}}
...
function restore(s){board=R.clone(s.board);turn=s.turn;winner=s.winner;resultNote=s.resultNote||'';selected=s.selected&&{...s.selected};last=s.last&&{...s.last};passes=s.passes;captures={...s.captures};scoring=!!s.scoring;dead=new Set(s.dead||[]);scoreApprover=s.scoreApprover||'b';pending=null;targets=selected&&mode==='xiangqi'?R.xiangqiTargets(board,selected.x,selected.y):[];cursor=last?{...last}:cursor;sync();draw()}
...
function armResign(){if(winner)return;if(resignArmed){const loser=currentSideName();winner=opponentWinner();resultNote=loser+'认输';scoring=false;dead.clear();pending=null;cancelResign();snapshot();showNotice(resultNote,'ok',2600);sync();draw();return}resignArmed=true;resignBtn.classList.add('armed');resignBtn.textContent='确认认输';showNotice('再次点击“认输”才会结束本局','warn',2200);resignTimer=setTimeout(cancelResign,2200)}
...
function sync(){
  gameName.textContent=names[mode];
  moveCount.textContent=Math.max(0,history.length-1)+' 手';
  sizeWrap.hidden=mode!=='go';
  passBtn.hidden=mode!=='go'||scoring||!!winner;
  resumeBtn.hidden=mode!=='go'||!scoring||!!winner;
  confirmBtn.hidden=mode==='xiangqi'||!!winner;
  confirmBtn.textContent=scoring?(scoreApprover==='b'?'黑方确认':'白方确认'):'落子';
  confirmBtn.disabled=!!winner||(mode==='xiangqi')||(!scoring&&!pending);
  undoBtn.disabled=scoring||(!pending&&history.length<=1);
  restartBtn.disabled=history.length<=1&&!pending&&!scoring;
  resignBtn.disabled=!!winner||scoring;
  if(mode==='gomoku'){
    hint.textContent='自由规则：先连成五子者胜。';
    statusEl.textContent=winner?(winner==='draw'?'和棋':(winner==='b'?'黑方胜':'白方胜')+(resultNote?' · '+resultNote:'')):(turn==='b'?'黑方落子':'白方落子');
    sessionHint.textContent='先选落点，再点“落子”确认';
  }
  if(mode==='xiangqi'){
    hint.textContent='基础完整规则：不能送将；支持将帅照面、将死与困毙。';
    if(winner)statusEl.textContent=(winner==='r'?'红方胜':'黑方胜')+(resultNote?' · '+resultNote:'');
    else statusEl.textContent=(turn==='r'?'红方':'黑方')+(R.xiangqiInCheck(board,turn)?' · 将军':'走棋');
    sessionHint.textContent='选棋子，再点目标位置';
  }
  if(mode==='go'){
    if(scoring&&!winner){
      const s=scorePreview();
      hint.textContent=`计分阶段：点棋子可标记/取消整组死子 · 当前估算 黑 ${s.black.toFixed(1)} / 白 ${s.white.toFixed(1)}`;
      statusEl.textContent=(scoreApprover==='b'?'黑方确认计分':'白方确认计分');
      sessionHint.textContent='双方确认前可继续调整死子，或恢复下棋';
    }else{
      hint.textContent=`中国面积计分 · 贴目 7.5 · 黑提 ${captures.b} · 白提 ${captures.w} · 禁止重复整盘局面`;
      statusEl.textContent=winner?goWinnerText():(turn==='b'?'黑方落子':'白方落子');
      sessionHint.textContent='先选落点，再点“落子”确认';
    }
  }
  ...
  canvas.setAttribute('aria-label',scoring?'围棋计分阶段。点击棋子组标记死子，双方依次确认计分。':mode==='xiangqi'?`${names[mode]}棋盘。${statusEl.textContent}。选择己方棋子后选择目标位置。`:`${names[mode]}棋盘。${statusEl.textContent}。先选择落点，再确认落子。`);
}
...
function onGo(x,y){if(winner||scoring||!insidePoint(x,y))return false;cursor={x,y};const koKey=history.length>=2?R.boardKey(history[history.length-2].board):null,res=R.goPlay(board,x,y,turn,koKey,goRepeatKeys());if(!res.ok){showNotice(goIssueText(res.reason),'warn');clickSound(100,.07);draw();return false}board=res.board;captures[turn]+=res.captured;last={x,y};passes=0;clickSound(res.captured?155:245);if(res.captured)showNotice(`提子 ${res.captured} 枚`,'ok');turn=turn==='b'?'w':'b';snapshot();sync();draw();return true}
...
function selectPlacement(x,y){if(winner||scoring||mode==='xiangqi')return false;cursor={x,y};const issue=placementIssue(x,y);if(issue){pending=null;sync();showNotice(goIssueText(issue),'warn');clickSound(100,.07);draw();return false}pending={x,y};sync();showNotice('已选择落点，点击“落子”确认','info',2400);draw();return true}
function confirmPlacement(){if(!pending||winner||scoring||mode==='xiangqi')return false;const{x,y}=pending;pending=null;return mode==='gomoku'?onGomoku(x,y):onGo(x,y)}
function toggleDeadGroup(x,y){if(mode!=='go'||!scoring||winner||!insidePoint(x,y)||!board[y][x])return false;const stones=R.group(board,x,y).stones,allDead=stones.every(([sx,sy])=>dead.has(sx+','+sy));for(const[sx,sy]of stones){const key=sx+','+sy;if(allDead)dead.delete(key);else dead.add(key)}scoreApprover='b';replaceTopSnapshot();sync();draw();showNotice(allDead?'已恢复该棋组为活棋':'已标记整组死子','info',1800);return true}
function confirmScore(){if(mode!=='go'||!scoring||winner)return false;if(scoreApprover==='b'){scoreApprover='w';replaceTopSnapshot();sync();showNotice('黑方已确认，请白方确认','ok',2200);return true}const s=scorePreview();winner=s.winner;resultNote=`终局 ${s.margin.toFixed(1)} 目（黑 ${s.black.toFixed(1)} / 白 ${s.white.toFixed(1)}）`;scoring=false;snapshot();sync();draw();showNotice(goWinnerText(),'ok',3000);return true}
function resumeGo(){if(mode!=='go'||!scoring||winner)return;scoring=false;dead.clear();scoreApprover='b';passes=0;pending=null;replaceTopSnapshot();sync();draw();showNotice('已退出计分阶段，继续下棋','ok',2200)}
...
function switchGame(next){if(next===mode)return;saveSession();cancelRestart();cancelResign();clearNotice();pending=null;mode=next;activeKey=sessionKey();const restored=loadSession(activeKey);if(!restored)freshBoard();syncUrl();sync();resize();showNotice(restored&&history.length>1?'已恢复这类棋的当前对局':'已切换至'+names[mode],'ok')}
canvas.addEventListener('pointerup',ev=>{cancelResign();const{x,y}=point(ev);cursor={x,y};if(scoring)toggleDeadGroup(x,y);else if(mode==='xiangqi')onXiangqi(x,y);else selectPlacement(x,y)});
...
undoBtn.addEventListener('click',()=>{cancelRestart();cancelResign();if(scoring)return;if(pending){pending=null;sync();draw();showNotice('已取消待确认落点','ok');return}if(history.length<=1)return;history.pop();restore(history[history.length-1]);showNotice('已悔棋','ok')});
sizeSelect.addEventListener('change',()=>{if(mode!=='go')return;saveSession();cancelRestart();cancelResign();clearNotice();pending=null;activeKey=sessionKey();const restored=loadSession(activeKey);if(!restored)freshBoard();syncUrl();sync();resize();showNotice(restored&&history.length>1?`已恢复 ${sizeSelect.value}×${sizeSelect.value} 对局`:`已切换为 ${sizeSelect.value}×${sizeSelect.value} 棋盘`,'ok')});
passBtn.addEventListener('click',()=>{if(mode!=='go'||winner||scoring)return;cancelRestart();cancelResign();pending=null;passes++;last=null;cursor=cursor||defaultCursor();clickSound(330);turn=turn==='b'?'w':'b';if(passes>=2){scoring=true;dead.clear();scoreApprover='b';showNotice('双方连续停一手，进入死子确认与计分阶段','ok',2800)}else showNotice('已停一手；对方再停一手将进入计分阶段','info',2200);snapshot();sync();draw()});
...
canvas.tabIndex=0;reset();resize();document.documentElement.dataset.gameVersion='0.3.0';
})();
