(()=>{
'use strict';
const R=window.BoardRules,AI=window.BoardAI,U=window.BoardUI||{lang:()=>'zh',t:(zh)=>zh,musicEnabled:()=>true},T=(zh,en)=>U.t(zh,en),canvas=document.getElementById('board'),ctx=canvas.getContext('2d'),playcard=document.querySelector('.playcard');
...
function displayNote(note){if(!note)return'';const fixed={'电脑无合法着法':'AI has no legal move','将帅被吃':'general captured','困毙':'stalemate','将死':'checkmate'};if(U.lang()==='en'&&fixed[note])return fixed[note];if(U.lang()==='en')return note.replace('认输',' resigned').replace('超时',' timed out').replace('终局 ','Final · ').replace(' 目（黑 ',' pts (Black ').replace(' / 白 ',' / White ').replace('）',')');return note}
...
function queueAI(delay=220){
  ...
  aiTimer=setTimeout(()=>{
    ...
    try{
      aiWorker=aiWorker||new Worker('ai-worker.js?v=060');
...
function sync(){
  ...
  if(mode==='xiangqi'){hint.textContent=T('基础完整规则：不能送将；支持将帅照面、将死与困毙。','Core Xiangqi rules: self-check is illegal; flying generals, checkmate and stalemate are enforced.');statusEl.textContent=winner?sideName(winner)+T('胜',' wins')+(resultNote?' · '+displayNote(resultNote):''):sideName(turn)+(R.xiangqiInCheck(board,turn)?T(' · 将军',' · check'):T('走棋',' to move'));sessionHint.textContent=T('选棋子，再点目标位置','Select a piece, then its destination.')}
...
function onXiangqi(x,y,fromAI=false){if((!fromAI&&humanBlocked())||winner||reviewing!==null||!insidePoint(x,y))return false;cursor={x,y};const p=board[y][x];if(!selected){if(p&&p.c===turn){selected={x,y};targets=R.xiangqiTargets(board,x,y);draw();return true}showNotice(T('请选择己方棋子','Select one of your own pieces.'),'warn');draw();return false}if(p&&p.c===turn){selected={x,y};targets=R.xiangqiTargets(board,x,y);draw();return true}const fx=selected.x,fy=selected.y,moving=board[fy][fx],moved=R.xiangqiMove(board,fx,fy,x,y);if(!moved){showNotice(T('此走法不合法','Illegal move.'),'warn');draw();return false}if(!settleClock())return false;const captured=moved.captured,movedColor=turn,enemy=turn==='r'?'b':'r';board=moved.board;last={x,y};recordLabel=`${sideShort(movedColor)} ${glyph[moving.t]} ${fx+1},${fy+1}→${x+1},${y+1}${captured?' ×'+glyph[captured.t]:''}`;clickSound(captured?150:230);selected=null;targets=[];if(captured&&captured.t.toUpperCase()==='K'){winner=movedColor;resultNote='将帅被吃';showNotice(sideName(movedColor)+T('获胜',' wins'),'ok',2600)}else{const terminal=R.xiangqiTerminal(board,enemy);if(terminal){winner=movedColor;resultNote=terminal==='stalemate'?'困毙':'将死';showNotice(displayNote(resultNote)+T('，',', ')+sideName(movedColor)+T('胜',' wins'),'ok',2600)}else turn=enemy}snapshot();sync();draw();queueAI();return true}
...
canvas.tabIndex=0;reset();resize();setInterval(tickClocks,250);window.addEventListener('beforeunload',cancelAI);document.documentElement.dataset.gameVersion='0.6.0';
})();
