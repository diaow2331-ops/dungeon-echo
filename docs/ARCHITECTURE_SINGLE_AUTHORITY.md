...
| Critical-damage multiplier + ordinary-monster pressure tuning | `game/domain/combat/combat-rules-v130.js` | calculate the canonical critical damage multiplier from caller-supplied crit power; quote ordinary-monster threat/HP/DEF pressure curves from caller-supplied depth/elite flags | roll critical hits, roll elites, consume RNG, mutate actors, sequence attacks, compose spawns, or own other combat/defense/healing arithmetic |
...
- town growth policy owns fixed project definitions, tier/relic/Gold requirements, bounded project-effect values, qualitative service-upgrade eligibility, deterministic project-gated safe-return town-event selection, state-aware NPC/resident/ambient-rumor copy and resident-roster eligibility; economy consumes smithy/market modifiers as explicit inputs and owns retemper/restock pricing, while core remains the sole owner of Gold/supply mutation, forge execution, tavern reward application, resident movement/interaction, event/project/chronicle persistence, market invalidation, UI and Canvas state;
- combat owns the canonical critical-damage multiplier and (since v1.9.2) the pure ordinary-monster threat/HP/DEF pressure curves; elite rolls, spawn composition, engagement strikes, defense, damage application and turn consequences remain core-owned. Guardian/final-boss authored pressure is intentionally frozen while ordinary-monster tactical AI is refined.
