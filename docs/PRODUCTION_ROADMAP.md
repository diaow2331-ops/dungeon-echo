# 地牢回响：v1.2.9 发布收尾与后续路线图

> 当前仓库语义版本为 **Dungeon Echo v1.2.9**，静态资源 cache generation 为 **153**。固定中英文路线、共享存档、输入/运行时治理与最终 locale 收口均已进入发布边界。最近一次统一三包发布边界仍是 **Dungeon Echo v1.2.7 + 91hwl site v1.3.3 + Moyu v1.11.3**。
...
**v1.2.9 发布/部署 → 真实浏览器验收 → 仅根据真人证据做定点修复。**
...
- ✅ Return Scroll 已收敛到 Commerce 两阶段语义状态机；键盘连发被抑制，手柄不再合成键盘 T。
- ✅ 城镇、装备、成长、战斗压力、Guardian、视觉层、新手引导等大部分常驻 polling / 无意义 RAF / DOM follower 已移除或改为生命周期驱动。
- ✅ 游戏手柄采样只在真实连接且页面可见时运行。
- ✅ 当前生产静态资源 cache generation 为 153；v1.2.9 语义版本与 cache generation 已明确分离。

这些结构性修改的目标是减少冻结、后台无效工作、输入 owner 冲突和中英文混杂，而不是改变战斗数值或经济平衡。

## 当前 P0：部署与真实浏览器验收

源码和仓库契约不能替代真实浏览器。v1.2.9 发布后需要人工确认的高价值场景为：
...
本月 GitHub Actions 额度已用完，因此当前治理周期不把 Actions 作为前置条件，也不重复查询 Actions 状态。任何 PASS 都必须对应实际执行过的源码检查、部署检查或人工浏览器验证。

## 当前 P1：发布后仓库维护

根目录文档/历史收敛已经完成，后续维持以下原则：

- `README.md`、`LICENSE`、`SECURITY.md`、`CONTRIBUTING.md`、`VERSION` 保持根目录可见。
- 活跃生产入口和运行时 JavaScript 暂时保持根目录，避免收尾阶段做高风险纯路径迁移。
- 工程/维护/平衡/AI 协作文档统一放在 `docs/`。
- 历史 release notes 统一放在 `docs/releases/`。
- 被替代的松散文档进入 `docs/archive/`。
- README 首页保持 source map，让游客直接找到 `game.js`、主要 system owner、中英文入口和开发文档。
- 只有路径迁移带来明确维护收益时才考虑把活跃 JS 移到 `src/`；不为了目录美观承担生产 URL、release manifest 和大量测试引用同时变更的风险。
...
v1.2.9 使用 `ops/release/build-site-bundle.sh` 构建 Dungeon Echo 单包，并只覆盖 `/dungeon-echo/`，保留当前 site 与 Moyu 内容。`build-web-toys-release.sh` 继续作为 v1.2.7 的历史统一三包边界，不为当前游戏治理改标。

`VERSION=1.2.9` 是语义发布身份；`runtime-bootstrap.js` 的 `assetVersion=153` 是静态缓存 generation。二者从本版开始不再要求数值相同。

发布完成必须真实观察到构建、校验、服务器切换、origin/public health check 和双端浏览器验收结果。GitHub 合并、tag 或 Release 页面本身不等于已经上线。

## 本轮停止条件

满足以下条件后，本轮 Dungeon Echo 升级治理可以关闭：

- 固定中英文架构与运行时治理已合并；
- Return Scroll 两阶段 owner、共享存档与 stable item identity 已进入 v1.2.9 release manifest；
