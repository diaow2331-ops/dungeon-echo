...
## Dungeon Echo v1.8.1 town viewport polish
Post-release visual acceptance on v1.8.0 found a presentation mismatch rather than an art deficit: the town shell occupied a large modal, but Market/Tavern/Fortune remained constrained to an 820px service card and Fortune itself rendered at only 230px. v1.8.1 reuses the existing v1.8 town/NPC/growth/relic art authorities, expands the desktop town workspace and service scale, enlarges Fortune to 380px, strengthens relic/service visual weight, and exposes fullscreen directly inside the town action bar. The old optional-details CSS is restricted to actual `details` elements so it no longer compresses Fortune, and the mobile town padding cascade is corrected. No gameplay, save schema, economy, combat or art authority moved. Release authority advances to cache generation 183.
