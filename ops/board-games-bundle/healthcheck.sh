...
curl -fsSL --noproxy '*' --resolve "$ORIGIN" "$URL" -o "$work/index.html" || fail 'HTML missing'; grep -Fq '<meta name="version" content="0.4.0"' "$work/index.html" || fail 'version marker missing'
for f in rules.js game.js style.css VERSION; do curl -fsSL --noproxy '*' --resolve "$ORIGIN" "${URL}$f" -o "$work/$f" || fail "$f missing"; done
test "$(tr -d '
[:space:]' < "$work/VERSION")" = "$version" || fail 'VERSION mismatch'; grep -Fq "dataset.gameVersion='0.4.0'" "$work/game.js" || fail 'runtime marker missing'
