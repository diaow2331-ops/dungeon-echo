#!/usr/bin/env bash
set -euo pipefail
BUNDLE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; SITE_ROOT=/srv/91hwl-play; RELEASES_DIR=$SITE_ROOT/releases; CURRENT_LINK=$SITE_ROOT/current; SOURCE=$BUNDLE_ROOT/public/board-games; HEALTHCHECK=$BUNDLE_ROOT/ops/healthcheck.sh; ROOT_POLICY=$BUNDLE_ROOT/ops/play-release-root-policy.sh
test -r "$ROOT_POLICY" || { echo "BOARD_GAMES_DEPLOY_ERROR: release-root policy missing" >&2; exit 1; }; source "$ROOT_POLICY"
...
version="$(tr -d '\n ' < "$BUNDLE_ROOT/VERSION")"; revision="$(tr -d '\n ' < "$BUNDLE_ROOT/REVISION")"; test "$version" = '0.7.0' || fail "unexpected version $version"
...
play_copy_release_root "$previous" "$tmp"; rm -rf "$tmp/board-games"; mkdir -p "$tmp/board-games"; cp -a "$SOURCE/." "$tmp/board-games/"; play_assert_release_root "$tmp" || fail 'release root contains unapproved entries'; find "$tmp" -type d -exec chmod 0755 {} +; find "$tmp" -type f -exec chmod 0644 {} +
