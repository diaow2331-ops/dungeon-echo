...
test "$site_version" = '1.11.5' || { echo "unexpected site version: $site_version" >&2; exit 2; }
...
test "$board_version" = '0.3.0' || { echo "unexpected Board Trio version: $board_version" >&2; exit 2; }
...
grep -Fq 'data-site-version="1.11.5"' "$home"
...
grep -Fq '03 / v0.3.0' "$home"
grep -Fq '围棋加入死子确认、双方计分确认与整盘重复局面校验' "$home"
...
grep -Fq 'data-site-version="1.11.5"' "$de_detail"
...
grep -Fq 'data-site-version="1.11.5"' "$moyu_detail"
...
grep -Fq "test \"\$version\" = '1.11.5'" "$source_root/deploy.sh"
...
grep -Fq 'web-toys-v1115' "$source_root/deploy.sh"
...
grep -Fq 'public site v1.11.5 check failed' "$source_root/healthcheck.sh"
