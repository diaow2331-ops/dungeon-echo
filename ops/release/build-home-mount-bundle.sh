...
test "$site_version" = '1.11.4' || { echo "unexpected site version: $site_version" >&2; exit 2; }
...
test "$board_version" = '0.2.1' || { echo "unexpected Board Trio version: $board_version" >&2; exit 2; }
...
grep -Fq 'data-site-version="1.11.4"' "$home"
...
grep -Fq '03 / v0.2.1' "$home"
grep -Fq '先选落点、再确认落子' "$home"
...
grep -Fq 'data-site-version="1.11.4"' "$de_detail"
...
grep -Fq 'data-site-version="1.11.4"' "$moyu_detail"
...
grep -Fq "test \"\$version\" = '1.11.4'" "$source_root/deploy.sh"
...
grep -Fq 'web-toys-v1114' "$source_root/deploy.sh"
...
grep -Fq 'public site v1.11.4 check failed' "$source_root/healthcheck.sh"
