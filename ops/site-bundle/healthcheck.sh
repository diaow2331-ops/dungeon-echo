...
probe_game(){
  ...
  grep -Fq '地牢回响' "$body" || return 1
  grep -Fq "v$version" "$body" || return 1
...
for ((attempt=1; attempt<=PUBLIC_ATTEMPTS; attempt++)); do
  if probe_game public "$public_url"; then
    public_ok=true
    break
  fi
  if (( attempt < PUBLIC_ATTEMPTS )); then sleep "$PUBLIC_DELAY"; fi
done
test "$public_ok" = true || fail "public game check failed after $PUBLIC_ATTEMPTS attempts"
