...
check_home(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.11.3"' 'homepage site version' || return 1
  ...
  require_fixed "$file" 'v0.2.0' 'homepage Board Trio version' || return 1
  ...
  require_fixed "$file" 'site-v1110/style.css' 'homepage v1.11.3 shared design' || return 1
...
check_de_detail(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.11.3"' 'Dungeon Echo detail site version' || return 1
...
check_moyu_detail(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.11.3"' 'Moyu detail site version' || return 1
...
check_trust_page(){
  ...
  require_fixed "$file" 'data-site-version="1.11.3"' "$label site version" || return 1
...
test "$board_origin" = '0.2.0' || fail "origin Board Trio VERSION mismatch: $board_origin"
...
test "$public_ok" = true || fail "public site v1.11.3 check failed after $ATTEMPTS attempts"
