...
check_home(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.11.2"' 'homepage site version' || return 1
  ...
  require_fixed "$file" 'site-v1110/style.css' 'homepage v1.11.2 shared design' || return 1
...
check_de_detail(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.11.2"' 'Dungeon Echo detail site version' || return 1
...
check_moyu_detail(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.11.2"' 'Moyu detail site version' || return 1
...
check_trust_page(){
  ...
  require_fixed "$file" 'data-site-version="1.11.2"' "$label site version" || return 1
...
test "$public_ok" = true || fail "public site v1.11.2 check failed after $ATTEMPTS attempts"
