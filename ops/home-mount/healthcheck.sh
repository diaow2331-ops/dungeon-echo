...
STYLE_URL=https://91hwl.cn/assets/site-v170/style.css
SCRIPT_URL=https://91hwl.cn/assets/site-v170/site.js
ART_URL=https://91hwl.cn/assets/site-v170/wang-jian-landscape-1668.jpg
...
version="$(tr -d '\r\n' < "$BUNDLE_ROOT/VERSION")"
work_dir="$(mktemp -d /tmp/web-toys-home-v170-health.XXXXXX)"
...
check_pref_contract(){
  ...
  require_fixed "$file" 'site-v170/site.js' 'shared interaction runtime' || return 1
...
check_home(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.7.0"' 'homepage site version' || return 1
...
check_de_detail(){
  file="$1"
  require_fixed "$file" 'data-site-version="1.7.0"' 'Dungeon Echo detail site version' || return 1
...
check_trust_page(){
  ...
  require_fixed "$file" 'data-site-version="1.7.0"' "$label site version" || return 1
  require_fixed "$file" 'site-v170/style.css' "$label shared design" || return 1
  require_fixed "$file" 'site-v170/site.js' "$label shared interactions" || return 1
  require_fixed "$file" "$ADSENSE_CLIENT" "$label AdSense client" || return 1
...
test "$public_ok" = true || fail "public site v1.7.0 check failed after $ATTEMPTS attempts"
