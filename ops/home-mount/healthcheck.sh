...
version="$(tr -d '\r\n' < "$BUNDLE_ROOT/VERSION")"
work_dir="$(mktemp -d /tmp/web-toys-home-health.XXXXXX)"
...
check_moyu_detail(){
  ...
  grep -Fq 'Clock Out Alive' "$file" || return 1
  grep -Fq 'data-site-version="1.3.0"' "$file" || return 1
  grep -Fq 'softwareVersion":"1.11.0"' "$file" || return 1
  grep -Fq '14:00 → 18:00' "$file" || return 1
  grep -Fq 'PC + Mobile' "$file" || return 1
  grep -Fq 'https://play.91hwl.cn/moyu/' "$file" || return 1
  grep -Fq 'data-lang-choice="en"' "$file" || return 1
}
...
fetch "$work_dir/de.html" --resolve "$MAIN_RESOLVE" "$DE_DETAIL_URL" || fail 'origin Dungeon Echo detail check failed'
check_de_detail "$work_dir/de.html" || fail 'origin Dungeon Echo detail contract failed'
fetch "$work_dir/moyu.html" --resolve "$MAIN_RESOLVE" "$MOYU_DETAIL_URL" || fail 'origin Moyu detail check failed'
check_moyu_detail "$work_dir/moyu.html" || fail 'origin Moyu detail contract failed'
fetch /dev/null --resolve "$PLAY_RESOLVE" "$DE_PLAY_URL" || fail 'live Dungeon Echo check failed'
fetch /dev/null --resolve "$PLAY_RESOLVE" "$MOYU_PLAY_URL" || fail 'live Moyu check failed'
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  if fetch "$work_dir/public-home.html" "${HOME_URL}?release=$revision" && check_home "$work_dir/public-home.html" \
      ...
      && fetch "$work_dir/public-moyu.html" "${MOYU_DETAIL_URL}?release=$revision" && check_moyu_detail "$work_dir/public-moyu.html"; then
    public_ok=true
    break
  fi
  if (( attempt < ATTEMPTS )); then sleep "$DELAY"; fi
done
test "$public_ok" = true || fail "public unified site check failed after $ATTEMPTS attempts"
