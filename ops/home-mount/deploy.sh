...
test "$version" = '1.11.5' || fail "unexpected site version: $version"
grep -Fq 'data-site-version="1.11.5"' "$PUBLIC_ROOT/index.html" || fail 'homepage site version marker missing'
...
grep -Fq 'site-v1110/style.css' "$PUBLIC_ROOT/index.html" || fail 'homepage v1.11.5 shared design missing'
...
grep -Fq '03 / v0.3.0' "$PUBLIC_ROOT/index.html" || fail 'homepage Board Trio v0.3.0 marker missing'
...
mkdir -p "$BACKUP_ROOT"
backup_dir="$(mktemp -d "$BACKUP_ROOT/web-toys-v1115.XXXXXX")"
...
index_tmp="$SITE_ROOT/.index.web-toys-v1115.tmp"
...
ads_tmp="$SITE_ROOT/.ads.txt.web-toys-v1115.tmp"
...
grep -Fq 'site-v1110/style.css' "$SITE_ROOT/index.html" || fail 'homepage v1.11.5 design write verification failed'
...
grep -Fq '03 / v0.3.0' "$SITE_ROOT/index.html" || fail 'homepage Board Trio v0.3.0 write verification failed'
