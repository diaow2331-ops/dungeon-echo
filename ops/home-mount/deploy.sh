...
test "$version" = '1.11.4' || fail "unexpected site version: $version"
grep -Fq 'data-site-version="1.11.4"' "$PUBLIC_ROOT/index.html" || fail 'homepage site version marker missing'
...
grep -Fq 'site-v1110/style.css' "$PUBLIC_ROOT/index.html" || fail 'homepage v1.11.4 shared design missing'
...
grep -Fq '03 / v0.2.1' "$PUBLIC_ROOT/index.html" || fail 'homepage Board Trio v0.2.1 marker missing'
...
mkdir -p "$BACKUP_ROOT"
backup_dir="$(mktemp -d "$BACKUP_ROOT/web-toys-v1114.XXXXXX")"
...
index_tmp="$SITE_ROOT/.index.web-toys-v1114.tmp"
...
ads_tmp="$SITE_ROOT/.ads.txt.web-toys-v1114.tmp"
...
grep -Fq 'site-v1110/style.css' "$SITE_ROOT/index.html" || fail 'homepage v1.11.4 design write verification failed'
...
grep -Fq '03 / v0.2.1' "$SITE_ROOT/index.html" || fail 'homepage Board Trio v0.2.1 write verification failed'
