...
- final `responsive-v1115.css` with phone safe-area padding and narrow-header compaction
...
- final HTML adapted at build time with v1.11.5 release fingerprints, responsive stylesheet, prepaint language resolution and browser-translation suppression
- VERSION 1.11.5
...
The final responsive pass does not touch runner geometry. On narrow phones it respects display safe areas, compacts Home/Settings controls into icon-width targets, and wraps the action header below 380px instead of squeezing the brand. Desktop layout, route length, obstacle weights, rewards, hitbox, endings and local saves remain unchanged.

All v1.11.4 quality guards remain: one-shot keyboard repeat suppression, primary mouse-button filtering, event-driven Canvas layout invalidation, memoized presentation-state DOM synchronization, and final-width spacing reserve for long-mutating BUGs.

The deployer verifies checksums, release/cache fingerprints, responsive asset, prepaint/notranslate markers, the explicit language-precedence contract, readable result/control scale, all earlier quality guards, and the absence of active player-halo and drifting-coworker draw calls. It then runs origin/public health checks. Failures after switching current trigger rollback.
