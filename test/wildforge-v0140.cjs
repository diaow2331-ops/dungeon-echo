...
assert.equal(read('wildforge/VERSION').trim(),'0.14.0');
assert(html.includes('<meta name="version" content="0.14.0"'));
assert(data.includes("VERSION = '0.14.0'"));
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0140'"));
assert(html.includes('<meta name="version" content="0.14.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(data.includes("VERSION = '0.14.0'"),'data module version must match incubation build');
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0140'"),'v0.9 save key missing');
...
console.log(`wildforge_v0140=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
...
console.log('wildforge_v0140=PASS beacon_network=persistent');
...
console.log('wildforge_v0140=PASS route_network=supply-route');
...
console.log('wildforge_v0140=PASS route_visibility=cross-biome-cargo');
...
console.log('wildforge_v0140=PASS frontier_market=regional-arbitrage');
...
console.log('wildforge_v0140=PASS trade_onboarding=early-game');
...
console.log('wildforge_v0140=PASS depot_logistics=production-storage-capacity');
...
assert(game.includes('max=5.75*(route.active?ROUTE_SPEED_MULT:1)*freightMove'),'cargo load must affect movement through canonical player speed');
...
console.log('wildforge_v0140=PASS haul_risk=weight-night-spill');
...
console.log('wildforge_v0140=PASS armed_haul=cargo-setdown-raiders-escort');
...
console.log('wildforge_v0140=PASS hotfix-save-migration');
...
console.log('wildforge_v0140=PASS visual_foundation=pixel-atlas');
