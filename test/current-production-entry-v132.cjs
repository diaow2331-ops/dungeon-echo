...
ok([...zh.matchAll(/<script\s+src="([^"]+)"/g)].every(m => /\?v=170$/.test(m[1])), 'Chinese synchronous scripts use cache generation 170');
ok([...en.matchAll(/<script\s+src="([^"]+)"/g)].every(m => /\?v=170$/.test(m[1])), 'English synchronous scripts use cache generation 170');
ok(/classic-100/.test(productionBootstrap), 'production bootstrap forces the classic-100 public profile');
ok(/assetVersion = '170'/.test(runtime) && /version:'v22'/.test(runtime), 'late presentation graph is generation 170 / runtime v22');
