...
assert.equal(read('wildforge/VERSION').trim(),'0.21.0');
assert(html.includes('<meta name="version" content="0.21.0"'));
assert(data.includes("VERSION = '0.21.0'"));
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0200'"));
assert(html.includes('<meta name="version" content="0.21.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(html.includes('id="showcaseBtn"')&&html.includes('id="showcaseExit"'),'showcase controls missing');
assert(game.includes("const SHOWCASE_KEY = 'wildforge.showcase'")&&game.includes('function toggleShowcase()')&&game.includes('function setShowcase(on)'),'showcase runtime missing');
assert(game.includes('function drawSurfaceAtmosphere()')&&game.includes('function drawAtmosphericGrade()'),'showcase atmosphere render layers missing');
assert(game.includes('game.showcase?.88:1')&&game.includes('game.showcase?8.5:18'),'showcase camera framing/follow tuning missing');
assert(css.includes('html.showcase-mode #hud')&&css.includes('html.showcase-mode #hotbar')&&css.includes('html.showcase-mode #mobileControls'),'showcase UI-clean capture CSS missing');
assert(css.includes('html.showcase-mode #game{cursor:none}')&&css.includes('html.showcase-mode #showcaseExit'),'showcase cursor/exit contract missing');
assert(game.includes('if(!game.showcase)drawTarget()')&&game.includes('if(!game.showcase&&(e.elite||e.hp<e.maxHp))'),'world-space capture clutter must be suppressed');
...
assert(data.includes("VERSION = '0.21.0'"),'data module version must match incubation build');
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0200'"),'v0.9 save key missing');
...
console.log(`wildforge_v0210=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
...
console.log('wildforge_v0210=PASS beacon_network=persistent');
...
console.log('wildforge_v0210=PASS route_network=supply-route');
...
console.log('wildforge_v0210=PASS route_visibility=cross-biome-cargo');
...
console.log('wildforge_v0210=PASS frontier_market=regional-arbitrage');
...
console.log('wildforge_v0210=PASS trade_onboarding=early-game');
...
console.log('wildforge_v0210=PASS depot_logistics=production-storage-capacity');
...
console.log('wildforge_v0210=PASS haul_risk=weight-night-spill');
...
console.log('wildforge_v0210=PASS armed_haul=cargo-setdown-raiders-escort');
...
console.log('wildforge_v0210=PASS hotfix-save-migration');
...
console.log('wildforge_v0210=PASS visual_foundation=pixel-atlas');
...
assert(game.includes('game.showcase?8.5:18')&&game.includes('Math.round(game.camera.x*s)/s'),'responsive pixel-snapped camera missing');
...
const settlementRuntime=new Function(settlementEconomy.replace(/^export\s+/gm,'')+'\nreturn {settlementName,settlementDailyDemand,settlementDemandRemaining,settlementSellPrice};')();
...
assert(html.includes('宣传模式')&&html.includes('地貌氛围升级')&&html.includes('聚落/贸易闭环'),'v0.21 showcase/atmosphere changes are not surfaced on start screen');
console.log('wildforge_v0210=PASS settlement-demand-destinations');
console.log('wildforge_v0210=PASS surface-inventory-knockback-expansion');
console.log('wildforge_v0210=PASS terraria_like_controls=fixed-step-context-use-camera');


// v0.20 foundation-feel regression gates.
...
console.log('wildforge_v0210=PASS foundation_feel=large-settlement-real-pack-pass-through-trees');
