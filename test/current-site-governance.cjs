...
assert.equal(siteVersion,'1.11.8');
...
const finalStage=read('ops/home-mount/build-site-v1118.cjs');
const deploy=read('ops/home-mount/deploy.sh'),health=read('ops/home-mount/healthcheck.sh');
assert(builder.includes('build-site-v1118.cjs'),'current site build must end at v1118');
...
assert(builder.includes('public/toys/board-games/index.html'),'current site bundle must include Board Trio detail source');
assert(finalStage.includes('/toys/board-games/')&&finalStage.includes('dungeon-guardians.webp')&&finalStage.includes('moyu-scenes.webp'),'v1118 detail-art transformations missing');
for(const rel of ['ops/home-mount/public/index.html','ops/home-mount/public/toys/board-games/index.html','ops/home-mount/public/contact/index.html','ops/home-mount/build-trust-v135.cjs','ops/home-mount/build-home-v150.cjs','ops/home-mount/build-site-v170.cjs','ops/home-mount/build-site-v180.cjs','ops/home-mount/build-site-v190.cjs']){
  const body=read(rel);
