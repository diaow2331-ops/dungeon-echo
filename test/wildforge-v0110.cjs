...
assert.equal(read('wildforge/VERSION').trim(),'0.11.0');
assert(html.includes('<meta name="version" content="0.11.0"'));
assert(data.includes("VERSION = '0.11.0'"));
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0110'"));
assert(html.includes('<meta name="version" content="0.11.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(data.includes("VERSION = '0.11.0'"),'data module version must match incubation build');
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0110'"),'v0.9 save key missing');
...
console.log(`wildforge_v0110=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
...
console.log('wildforge_v0110=PASS beacon_network=persistent');
...
console.log('wildforge_v0110=PASS route_network=supply-route');
...
console.log('wildforge_v0110=PASS route_visibility=cross-biome-cargo');
...
console.log('wildforge_v0110=PASS frontier_market=regional-arbitrage');
...
console.log('wildforge_v0110=PASS trade_onboarding=early-game');
...
assert(html.includes('生产 <i>→</i> 装货')&&html.includes('据点物流'),'v0.11 logistics core loop is not surfaced');
assert(css.includes('.logistics-strip'),'logistics HUD/panel styling missing');
console.log('wildforge_v0110=PASS depot_logistics=production-storage-capacity');
