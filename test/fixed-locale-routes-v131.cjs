...
ok(JSON.stringify(scripts(zh))===JSON.stringify(expectedScripts),'Chinese route boots canonical v179 synchronous graph');
ok(JSON.stringify(scripts(en))===JSON.stringify(expectedScripts),'English route boots the identical canonical v179 synchronous graph');
...
ok(/const assetVersion = '179'/.test(runtime)&&/version:'v31'/.test(runtime),'runtime bootstrap aligns with generation 179 / v31');
