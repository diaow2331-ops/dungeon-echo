...
ok(JSON.stringify(scripts(zh))===JSON.stringify(expectedScripts),'Chinese route boots canonical v176 synchronous graph');
ok(JSON.stringify(scripts(en))===JSON.stringify(expectedScripts),'English route boots the identical canonical v176 synchronous graph');
...
ok(/const assetVersion = '176'/.test(runtime)&&/version:'v28'/.test(runtime),'runtime bootstrap aligns with generation 176 / v28');
