...
ok(JSON.stringify(scripts(zh))===JSON.stringify(expectedScripts),'Chinese route boots canonical v180 synchronous graph');
ok(JSON.stringify(scripts(en))===JSON.stringify(expectedScripts),'English route boots the identical canonical v180 synchronous graph');
...
const expectedFollowers=[
  "fresh('game/core/release-stamp-v181.js')",
...
ok(/const assetVersion = '183'/.test(runtime)&&/version:'v34'/.test(runtime),'runtime bootstrap aligns with generation 183 / v34');
