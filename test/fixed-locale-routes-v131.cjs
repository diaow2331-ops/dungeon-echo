...
ok(JSON.stringify(scripts(zh))===JSON.stringify(expectedScripts),'Chinese route boots canonical v172 synchronous graph');
ok(JSON.stringify(scripts(en))===JSON.stringify(expectedScripts),'English route boots the identical canonical v172 synchronous graph');
...
ok(/const assetVersion = '172'/.test(runtime)&&/version:'v24'/.test(runtime),'runtime bootstrap aligns with generation 172 / v24');
