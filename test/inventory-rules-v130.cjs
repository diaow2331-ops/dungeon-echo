...
for (const entry of ['index.html', 'en/index.html']) {
  ...
  assert(html.includes(`${rel}?v=${generation}`), `${entry}: production inventory rules must be loaded`);
  assert(html.indexOf(`${rel}?v=${generation}`) < html.indexOf(`game/core/game.js?v=${generation}`), `${entry}: inventory authority must load before core`);
}
