...
assert.equal(read('wildforge/VERSION').trim(),'0.42.0');
assert(html.includes('<meta name="version" content="0.42.0"'));
assert(data.includes("VERSION = '0.42.0'"));
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0420'"));
...
assert(html.includes('<meta name="version" content="0.42.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(data.includes("VERSION = '0.42.0'"),'data module version must match incubation build');
assert(game.includes("const SAVE_KEY = 'wildforge.save.v0420'"),'v0.9 save key missing');
...
console.log(`wildforge_v0420=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
...
console.log('wildforge_v0420=PASS beacon_network=persistent');
...
console.log('wildforge_v0420=PASS route_network=supply-route');
...
console.log('wildforge_v0420=PASS route_visibility=cross-biome-cargo');
...
console.log('wildforge_v0420=PASS frontier_market=regional-arbitrage');
...
console.log('wildforge_v0420=PASS trade_onboarding=early-game');
...
console.log('wildforge_v0420=PASS depot_logistics=production-storage-capacity');
...
console.log('wildforge_v0420=PASS haul_risk=weight-night-spill');
...
console.log('wildforge_v0420=PASS armed_haul=cargo-setdown-raiders-escort');
...
console.log('wildforge_v0420=PASS hotfix-save-migration');
...
console.log('wildforge_v0420=PASS visual_foundation=pixel-atlas');
...
console.log('wildforge_v0420=PASS settlement-demand-destinations');
console.log('wildforge_v0420=PASS surface-inventory-knockback-expansion');
console.log('wildforge_v0420=PASS terraria_like_controls=fixed-step-context-use-camera');
...
assert(html.includes('WILDFORGE · v0.42.0')&&html.includes('v0.42.0 · 新手生存/贸易闭环'),'v0.41 build identity missing');
...
console.log('wildforge_v0420=PASS foundation_feel=large-settlement-real-pack-pass-through-trees');
...
console.log('wildforge_v0420=PASS visual_spiral=biomes-settlements-depots');
...
console.log('wildforge_v0420=PASS settlement_memory=preference-reliability-causality');
...
assert((html.includes('商誉可视化')&&html.includes('聚落繁荣层')&&html.includes('动态灯火'))||html.includes('WILDFORGE · v0.42.0'),'settlement visual spiral / current build identity is not surfaced on start screen');
console.log('wildforge_v0420=PASS settlement_standing_visuals=reliability-prosperity-world-feedback');
