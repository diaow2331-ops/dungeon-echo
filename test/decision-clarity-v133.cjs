...
ok(/const classFitOf = \(item, targetClass=classId\)/.test(core) && /INVENTORY_RULES\.classFitScore/.test(core), 'core uses the canonical class-fit helper');
...
ok(!/J Attack · K Skill/.test(core) && /C Skill · J Quick Dive/.test(core), 'canonical English HUD no longer advertises retired J/K controls');
