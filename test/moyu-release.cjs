...
assert.equal(version,'1.26.2'); assert.match(index,/content="1\.26\.2"/); assert.match(index,/game\.js\?v=1262/);
assert.match(game,/dataset\.gameVersion='1\.26\.2'/); assert.match(game,/DAY_END_DISTANCE=2200/); assert.match(game,/RUN_PROGRESS_SCALE=\.020/); assert.match(game,/MAX_MISTAKES=3/);
assert.match(game,/assets\/sprites\/hero-v125\.webp\?v=1262/); assert.match(game,/display:136/); assert.match(game,/assets\/sprites\/office-hazards-v124\.webp\?v=1240/);
assert.match(game,/const SCENE_BACKDROPS=\[/); assert.match(game,/assets\/scenes-v126\/workstation\.svg\?v=1262/); assert.match(game,/function drawSceneBackdrop\(idx\)/); assert.match(game,/function drawRunnerBackdropClearance\(i\)/); assert.match(game,/function drawRunnerSilhouetteHalo\(drawX,footY,altitude\)/); assert.match(game,/imageSmoothingQuality='high'/);
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'moyu1262-')),archive=path.join(tmp,'moyu.zip');r=spawnSync('bash',[path.join(root,'ops/release/build-moyu-bundle.sh'),archive],{cwd:root,encoding:'utf8'});assert.equal(r.status,0,r.stderr);assert.match(r.stdout,/version=1\.26\.2/);assert.match(r.stdout,/moyu_bundle_build=PASS/);
...
assert.equal(unzip('public/moyu/VERSION').trim(),'1.26.2'); fs.rmSync(tmp,{recursive:true,force:true}); console.log('RESULT Clock Out Alive v1.26.2 HD vector scenes PASS');
