...
assert.equal(read('moyu/VERSION').trim(),'1.11.3');
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'moyu1113-')),js=path.join(tmp,'game.js'),indexPath=path.join(tmp,'index.html');fs.writeFileSync(js,base);fs.writeFileSync(indexPath,sourceIndex);
...
assert.match(index,/content="1\.11\.3"/);assert.match(index,/style\.css\?v=1113/);assert.match(index,/visual-v1113\.css\?v=1113/);assert.match(index,/game\.js\?v=1113/);assert.match(index,/translate="no"/);assert.match(index,/name="google" content="notranslate"/);assert.match(index,/q\.get\('lang'\)/);
assert.match(game,/dataset\.gameVersion='1\.11\.3'/);assert.match(game,/DAY_END_DISTANCE=2200/);assert.match(game,/PLAYER_HIT=\{left:10,right:10,top:7,bottom:6\}/);assert.match(game,/groundTakeoff=before===0/);assert.match(game,/writeSharedLangCookie\(currentLang\)/);
...
const archive=path.join(tmp,'moyu.zip');r=spawnSync('bash',[path.join(root,'ops/release/build-moyu-bundle.sh'),archive],{cwd:root,encoding:'utf8'});assert.equal(r.status,0,r.stderr);assert.match(r.stdout,/version=1\.11\.3/);assert.match(r.stdout,/moyu_bundle_build=PASS/);
r=spawnSync('unzip',['-p',archive,'public/moyu/index.html'],{encoding:'utf8'});assert.equal(r.status,0,r.stderr);assert.match(r.stdout,/visual-v1113\.css\?v=1113/);assert.match(r.stdout,/translate="no"/);
...
fs.rmSync(tmp,{recursive:true,force:true});console.log('RESULT  Clock Out Alive v1.11.3 prepaint/typography contract PASS');
