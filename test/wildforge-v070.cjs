...
assert.equal(read('wildforge/VERSION').trim(),'0.7.0');
assert(html.includes('<meta name="version" content="0.7.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(data.includes("VERSION = '0.7.0'"),'data module version must match incubation build');
...
assert(game.includes('LEGACY_SAVE_KEY_OLDEST')&&game.includes('for(const key of [SAVE_KEY,LEGACY_SAVE_KEY,LEGACY_SAVE_KEY_OLD,LEGACY_SAVE_KEY_OLDER,LEGACY_SAVE_KEY_OLDEST])'),'new-world reset must clear all legacy saves');
...
console.log(`wildforge_v070=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
