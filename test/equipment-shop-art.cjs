...
ok(/__DE_EQUIPMENT_SHOP_ART/.test(src) && /version:\s*'v2'/.test(src), 'shop equipment preview declares event-driven v2');
...
ok(!/MutationObserver/.test(src) && /addEventListener\('keydown',\s*schedule,\s*true\)/.test(src), 'shop preview follows real input transitions without DOM observation');
