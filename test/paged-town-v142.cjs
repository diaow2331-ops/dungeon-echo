...
for(const [name,html] of [['zh',zh],['en',en]]){
  ...
  ok((town.match(/data-town-page="/g)||[]).length===6, `${name} exposes six town tabs`);
  ok((town.match(/data-town-page-panel="/g)||[]).length===6, `${name} exposes six town panels`);
...
ok(/bag:'gear', stash:'gear', market:'market', tavern:'tavern', wheel:'wheel', portal:'depart'/.test(core),'services map to focused pages');
...
ok(/\.town-page \{[\s\S]*overflow: auto/.test(css)&&/\.town-tabs[\s\S]*repeat\(6/.test(css),'town pages own overflow beneath six desktop tabs');
