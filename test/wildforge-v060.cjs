...
assert.equal(read('wildforge/VERSION').trim(),'0.6.0');
assert(html.includes('<meta name="version" content="0.6.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(tileEntries>=24,'expected at least 24 tile/material definitions');assert(recipeEntries>=22,'expected at least 19 craft recipes');assert(enemyEntries>=6,'expected six enemy families including the finale boss');
...
assert(data.includes("VERSION = '0.6.0'"),'data module version must match incubation build');
assert(game.includes("const SAVE_KEY = 'wildforge.save.v060'"),'v0.6 save key missing');
...
assert(game.includes('LEGACY_SAVE_KEY_OLDER')&&game.includes('localStorage.removeItem(LEGACY_SAVE_KEY_OLDER)'),'new-world reset must clear all legacy saves');
...
console.log(`wildforge_v060=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
