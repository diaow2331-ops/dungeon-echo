...
assert.equal(read('wildforge/VERSION').trim(),'0.9.0');
assert(html.includes('<meta name="version" content="0.9.0"'));
assert(data.includes("VERSION = '0.9.0'"));
assert(game.includes("const SAVE_KEY = 'wildforge.save.v090'"));
assert(html.includes('<meta name="version" content="0.9.0"'));assert(html.includes('id="victoryScreen"')&&html.includes('id="continueAfterVictory"'),'v0.3 finale UI missing');
...
assert(data.includes("VERSION = '0.9.0'"),'data module version must match incubation build');
assert(game.includes("const SAVE_KEY = 'wildforge.save.v090'"),'v0.9 save key missing');
...
console.log(`wildforge_v090=PASS tiles=${tileEntries} recipes=${recipeEntries} enemies=${enemyEntries}`);
...
assert(game.includes("raw.v===VERSION")&&game.includes("raw.v==='0.8.0'")&&game.includes("LEGACY_SAVE_KEY = 'wildforge.save.v080'"),'v0.8 migration chain missing');
console.log('wildforge_v090=PASS beacon_network=persistent');
