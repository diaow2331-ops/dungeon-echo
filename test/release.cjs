...
assert.equal(version, '1.9.7', 'release test must lock the v1.9.7 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '197', 'release test must lock cache generation 197');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v197-release-'));
...
console.log('release_v1_9_7=PASS');
