...
assert.equal(version, '1.9.5', 'release test must lock the v1.9.5 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '195', 'release test must lock cache generation 195');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v195-release-'));
...
console.log('release_v1_9_5=PASS');
