...
assert.equal(version, '1.9.8', 'release test must lock the v1.9.8 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '198', 'release test must lock cache generation 198');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v198-release-'));
...
console.log('release_v1_9_8=PASS');
