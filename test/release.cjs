...
assert.equal(version, '1.4.2', 'release test must lock the v1.4.2 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '178', 'release test must lock cache generation 178');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v142-release-'));
...
console.log('release_v1_4_2=PASS');
