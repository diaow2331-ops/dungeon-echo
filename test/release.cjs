...
assert.equal(version, '1.5.0', 'release test must lock the v1.5.0 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '179', 'release test must lock cache generation 179');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v150-release-'));
...
console.log('release_v1_5_0=PASS');
