...
assert.equal(version, '1.4.1', 'release test must lock the v1.4.1 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '177', 'release test must lock cache generation 177');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v141-release-'));
...
console.log('release_v1_4_1=PASS');
