...
assert.equal(version, '1.9.6', 'release test must lock the v1.9.6 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '196', 'release test must lock cache generation 196');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v196-release-'));
...
console.log('release_v1_9_6=PASS');
