...
assert.equal(version, '1.9.4', 'release test must lock the v1.9.4 boundary');
assert.equal(authority.version, version, 'authority map version drifted');
assert.equal(generation, '194', 'release test must lock cache generation 194');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'de-v194-release-'));
...
console.log('release_v1_9_4=PASS');
