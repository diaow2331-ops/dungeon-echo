...
const assetRoot='ops/home-mount/public/assets/site-v1100';
assert.equal(read('VERSION').trim(),'1.4.2');
assert.equal(read('moyu/VERSION').trim(),'1.26.5');
...
for(const f of [assetRoot+'/style.css',assetRoot+'/site.js',assetRoot+'/wang-jian-landscape-1668.jpg',assetRoot+'/moyu-run-v1265.jpg',assetRoot+'/dungeon-roster.webp'])assert(fs.statSync(path.join(root,f)).size>100,'missing asset '+f);
...
for(const f of ['ops/home-mount/build-v134.cjs','ops/home-mount/build-social-v134.cjs','ops/home-mount/build-trust-v135.cjs','ops/home-mount/build-home-v140.cjs','ops/home-mount/build-home-v150.cjs','ops/home-mount/build-home-v160.cjs','ops/home-mount/build-site-v170.cjs','ops/home-mount/build-site-v180.cjs','ops/home-mount/build-site-v190.cjs','ops/home-mount/build-site-v1100.cjs',assetRoot+'/site.js']){
 const x=run('node',['--check',path.join(root,f)]);assert.equal(x.status,0,x.stderr);
}
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'web-toys-home-v1100-'));
...
x=run('node',[path.join(root,'ops/home-mount/build-site-v1100.cjs'),...six]);
assert.equal(x.status,0,x.stderr);assert.match(x.stdout,/site_v1100_build=PASS/);
const built=six.map(f=>fs.readFileSync(f,'utf8'));
for(const html of built){
 assert.match(html,/data-site-version="1\.10\.0"/);
 assert.match(html,/site-v1100\/style\.css/);
 assert.match(html,/site-v1100\/site\.js/);
...
assert.match(home,/data-game-choice="random"/);assert.match(home,/site v1\.10\.0/);assert.match(home,/id="navToggle"/);assert.match(home,/game-media-moyu/);assert.match(home,/dungeon-roster\.webp/);assert.match(home,/浏览器游戏/);
assert.match(de,/softwareVersion":"1\.4\.2"/);assert.match(de,/1120×460 可步行广场/);
assert.match(moyu,/softwareVersion":"1\.26\.5"/);assert.match(moyu,/画面与信息都更清楚/);
...
assert.equal(x.status,0,x.stderr);assert.match(x.stdout,/site_version=1\.10\.0/);assert.match(x.stdout,/site_bundle_build=PASS/);
x=run('unzip',['-Z1',archive]);assert.equal(x.status,0,x.stderr);
const files=x.stdout.trim().split(/\r?\n/);
for(const f of ['README.txt','REVISION','VERSION','SHA256SUMS','ops/deploy.sh','ops/healthcheck.sh','public/index.html','public/toys/dungeon-echo/index.html','public/toys/moyu/index.html','public/about/index.html','public/privacy/index.html','public/contact/index.html','public/ads.txt','public/assets/site-v1100/style.css','public/assets/site-v1100/site.js','public/assets/site-v1100/wang-jian-landscape-1668.jpg','public/assets/site-v1100/moyu-run-v1265.jpg','public/assets/site-v1100/dungeon-roster.webp'])assert(files.includes(f),'bundle missing '+f);
...
assert.equal(txt('VERSION').trim(),'1.10.0');
...
assert.match(txt('public/assets/site-v1100/style.css'),/\.folio-art/);
assert.match(txt('public/assets/site-v1100/site.js'),/data-reset-prefs/);assert.match(txt('public/assets/site-v1100/site.js'),/navToggle/);assert(unzip('public/assets/site-v1100/moyu-run-v1265.jpg').length>50000);
assert(unzip('public/assets/site-v1100/wang-jian-landscape-1668.jpg').length>200000);assert(unzip('public/assets/site-v1100/dungeon-roster.webp').length>100000);
...
assert.match(txt('ops/deploy.sh'),/web-toys-v1100/);
assert.match(txt('ops/deploy.sh'),/ASSET_REL=assets\/site-v1100/);
assert.match(txt('ops/healthcheck.sh'),/public site v1\.10\.0 check failed/);
assert.match(txt('ops/healthcheck.sh'),/ART_URL=https:\/\/91hwl\.cn\/assets\/site-v1100/);
fs.rmSync(tmp,{recursive:true,force:true});
console.log('RESULT  91hwl site v1.10.0 modern Chinese game-site + denser project imagery contract PASS');
