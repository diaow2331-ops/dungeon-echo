...
const sourcePolish=read('ops/home-mount/build-home-v140.cjs');

assert.equal(read('VERSION').trim(),'1.4.2');
assert.equal(read('moyu/VERSION').trim(),'1.17.0');
assert.equal(siteVersion,'1.4.0');
...
// and the v1.4.0 clarity/version postprocessor.
...
for(const script of ['ops/home-mount/build-v134.cjs','ops/home-mount/build-social-v134.cjs','ops/home-mount/build-trust-v135.cjs','ops/home-mount/build-home-v140.cjs']){
...
assert.match(sourcePolish,/site-home-v140/);assert.match(sourcePolish,/长线构筑，还是四分钟逃班？/);assert.match(sourcePolish,/softwareVersion/);
assert.match(sourceDeploy,/test "\$version" = '1\.4\.0'/);
assert.match(sourceDeploy,/Dungeon Echo v1\.4\.2 detail marker missing/);
assert.match(sourceDeploy,/Clock Out Alive v1\.17\.0 detail marker missing/);
assert.match(sourceDeploy,/site-trust-hub-v135/);
assert.match(sourceDeploy,/site-home-v140/);
assert.match(sourceDeploy,/mailto:diaow2331@gmail\.com/);
assert.match(sourceDeploy,/pub-2648680835467283/);
assert.match(sourceDeploy,/web-toys-v140/);
...
assert.match(sourceHealth,/public site v1\.4\.0 check failed/);
assert.match(sourceHealth,/site-trust-hub-v135/);
assert.match(sourceHealth,/site-home-v140/);
assert.match(sourceHealth,/关于、隐私与联系，一眼就能找到。/);
assert.match(sourceHealth,/de_origin.*1\.4\.2/s);
assert.match(sourceHealth,/moyu_origin.*1\.17\.0/s);
...
assert.match(bundledHome,/data-site-version="1\.4\.0"/);
assert.match(bundledHome,/v1\.4\.2/);
assert.match(bundledHome,/v1\.17\.0/);
assert.match(bundledHome,/GitHub \/ Source/);
assert.match(bundledHome,/公开仓库/);
assert.match(bundledHome,/site-trust-hub-v135/);
assert.match(bundledHome,/site-home-v140/);
assert.match(bundledHome,/长线构筑，还是四分钟逃班？/);
assert.match(bundledHome,/Two games\.<br>Zero waiting\./);
assert.match(bundledHome,/class="index-version"/);
assert.doesNotMatch(bundledHome,/class="site-story"/);
assert.match(bundledHome,/关于、隐私与联系，一眼就能找到。/);
assert.match(bundledHome,/About, privacy and contact — easy to find\./);
...
assert.match(bundledDe,/data-site-version="1\.4\.0"/);
...
assert.match(bundledMoyu,/data-site-version="1\.4\.0"/);
assert.match(bundledMoyu,/softwareVersion":"1\.17\.0"/);
assert.match(bundledMoyu,/四分钟更完整/);
assert.match(bundledMoyu,/A fuller four-minute run/);
...
console.log('RESULT  91hwl site v1.4.0 clarity + current-game presentation contract PASS');
