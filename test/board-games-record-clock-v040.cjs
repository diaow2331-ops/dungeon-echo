...
assert(game.includes("timeSelect.disabled=inReview||history.length>1||scoring||!!winner"),'time control must lock after play begins');
...
assert(game.includes('if(reviewing!==null)return;cancelResign()'),'board input must be inert during review');
...
console.log('board_games_record_clock_v040=PASS');
