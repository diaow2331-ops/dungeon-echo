...
assert.equal(growth.smithyRefinementUnlocked({smithy:0}),true,'legacy +3 refinement remains a baseline service');
assert.equal(growth.smithyMasterworkUnlocked({smithy:0}),true,'legacy +5 masterwork remains a baseline service');
assert.equal(growth.smithyRetemperUnlocked({smithy:1}),false);
assert.equal(growth.smithyRetemperUnlocked({smithy:2}),true,'quench trough unlocks paid route retempering');
assert.equal(growth.smithyMasterRetemperUnlocked({smithy:2}),false);
assert.equal(growth.smithyMasterRetemperUnlocked({smithy:3}),true,'main furnace unlocks safe retemper for masterworked gear');
assert.equal(growth.marketReadinessUnlocked({market:0}),true,'legacy one-click readiness remains a baseline service');
assert.equal(growth.marketReadinessKeyTarget({market:0}),0);
assert.equal(growth.marketReadinessKeyTarget({market:1}),1,'road repair upgrades readiness to include one Key');
assert.equal(growth.marketRestockUnlocked({market:1}),false);
assert.equal(growth.marketRestockUnlocked({market:2}),true,'caravan guards unlock one-cycle restock');
assert.equal(growth.marketPriceDiscount({market:2}),0);
assert.equal(growth.marketPriceDiscount({market:3}),.08,'night market adds bounded supply discount');
assert.deepEqual([0,1,2,3].map(x=>growth.tavernChoiceCount({tavern:x})),[1,2,3,4],'tavern construction converts random toast into bounded choices');
...
for(const [name,html] of [['zh',zh],['en',en]]){
  assert(html.includes('game/domain/town/town-growth-rules-v180.js?v=183'),name+' boots town-growth policy');
  assert(html.indexOf('town-growth-rules-v180.js?v=183')<html.indexOf('game/core/game.js?v=183'),name+' boots town-growth policy before core');
}
