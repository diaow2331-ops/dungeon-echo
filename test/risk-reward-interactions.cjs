...
ok(R&&R.version==='p0-v4','risk-reward system boots current contract');
...
ok(!riskSrc.includes('armCaskWatch'),'risk-reward no longer snapshots casks from every input event');
ok(!riskSrc.includes("document.addEventListener('keydown'")&&!riskSrc.includes("document.addEventListener('click'"),'risk-reward owns no broad document input followers');
ok(riskSrc.includes("window.addEventListener('click', interceptShrine, true)"),'only shrine acceptance keeps a global click interceptor');
ok(riskSrc.includes('defer(syncShrineCopy)'),'shrine presentation follows the movement transition that can actually open it');
