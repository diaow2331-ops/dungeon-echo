/* Dungeon Echo desktop visual polish v4.
 * Presentation-only overlay for the public classic-100 route.
 * Owns atmosphere, v13 equipment art, tier identity and character gear presentation.
...
(() => {
  ...
  for (const [id,sheet] of Object.entries(EQUIPMENT_SHEETS)) {
    const img = new Image(); img.src = sheet.url; sheetImages[id] = img;
  }
  ...
  function dims() {
    ...
    const width = Number(game.width) || 1280, height = Number(game.height) || 896;
  ...
  function radial(x,y,inner,outer,rgb,alpha) {
    ...
    g.addColorStop(0,rgba(rgb,alpha)); g.addColorStop(.46,rgba(rgb,alpha*.42)); g.addColorStop(1,rgba(rgb,0));
    ctx.fillStyle = g; ctx.fillRect(x-outer,y-outer,outer*2,outer*2);
  }
  function drawSource(src,x,y,w,h,alpha=1,rotation=0) {
    if (!src) return false;
    const [sheetId,sx,sy] = src, sheet = EQUIPMENT_SHEETS[sheetId], img = sheetImages[sheetId];
    if (!sheet || !imageReady(img)) return false;
    const sw = img.naturalWidth/sheet.cols, sh = img.naturalHeight/sheet.rows;
    ctx.save(); ctx.globalAlpha=alpha; ctx.imageSmoothingEnabled=false; ctx.translate(x,y); if(rotation)ctx.rotate(rotation);
    ctx.shadowColor='rgba(0,0,0,.72)'; ctx.shadowBlur=2; ctx.shadowOffsetY=2;
    ctx.drawImage(img,sx*sw,sy*sh,sw,sh,-w/2,-h/2,w,h); ctx.restore(); return true;
  }
  ...
  function applyDomItemArt(el,item) {
    ...
    const [sheetId,sx,sy] = src, sheet = EQUIPMENT_SHEETS[sheetId];
    const px = sheet.cols > 1 ? sx/(sheet.cols-1)*100 : 0;
    const py = sheet.rows > 1 ? sy/(sheet.rows-1)*100 : 0;
  ...
  function syncEquipmentUi() {
    ...
    for (const slot of ['weapon','armor','helmet','boots','ring','amulet']) {
      ...
      if (el && p.equip[slot]) applyDomItemArt(el,p.equip[slot]);
    }
    ...
    for (let i=0;i<cells.length;i++) {
      ...
      if (icon) applyDomItemArt(icon,item);
    }
  }

  function drawCharacterGear(now,d) {
    const p=api.player, pos=position(p,d); if(!p||!pos||!p.equip)return;
    const eq=p.equip, scale=clamp(Math.min(d.tw,d.th)/32,.78,1.35);
    ...
    if(eq.ring){const r=rarityOf(eq.ring);ctx.save();ctx.strokeStyle=RARITY_GLOW[r];ctx.globalAlpha=.24+r*.055+t*.05;ctx.lineWidth=1+r*.25;ctx.beginPath();ctx.arc(pos.x-12*scale,pos.y+4*scale,(3.5+r*.45)*scale,0,Math.PI*2);ctx.stroke();ctx.restore();}
    if(eq.amulet){const r=rarityOf(eq.amulet);ctx.save();ctx.fillStyle=RARITY_GLOW[r];ctx.globalAlpha=.34+r*.06+t*.08;ctx.translate(pos.x,pos.y-2*scale);ctx.rotate(Math.PI/4);const s=(3.2+r*.5)*scale;ctx.fillRect(-s/2,-s/2,s,s);ctx.restore();}
  }

  function drawDust(now,d,depth){const count=depth>=70?18:depth>=30?14:10,t=reduceMotion?0:now/1000,pulseSpeed=depth>=80?1:.65;ctx.save();for(let i=0;i<count;i++){const px=((i*97.3+t*(6+(i%4)*1.7))%(d.width+80))-40,py=((i*61.7+t*(2.2+(i%3)*1.2))%(d.height+60))-30,pulse=reduceMotion?.5:.5+.5*Math.sin(t*pulseSpeed+i*1.93);ctx.globalAlpha=.045+pulse*.055;ctx.fillStyle=depth>=80?'#c9b7ff':depth>=50?'#d8b9aa':'#ead8b2';ctx.beginPath();ctx.arc(px,py,.8+(i%3)*.55,0,Math.PI*2);ctx.fill();}ctx.restore();}
  function drawPlayerPresence(now,d){const p=api.player,pos=position(p,d);if(!p||!pos)return;const rgb=CLASS_GLOW[classId()]||CLASS_GLOW.warrior,pulse=reduceMotion?.5:.5+.5*Math.sin(now*.0042);radial(pos.x,pos.y+d.th*.18,d.tw*.12,d.tw*(1.05+pulse*.10),rgb,.12+pulse*.035);if((Number(p.skillCd)||0)<=0){ctx.save();ctx.globalAlpha=.16+pulse*.08;ctx.strokeStyle=rgba(rgb,1);ctx.lineWidth=1.25;ctx.beginPath();ctx.ellipse(pos.x,pos.y+d.th*.26,d.tw*(.48+pulse*.025),d.th*.17,0,0,Math.PI*2);ctx.stroke();ctx.restore();}}
  function drawEnemyPresence(now,d){const list=Array.isArray(api.monsters)?api.monsters:[];let boss=null,guardian=null;for(const m of list){if(!m||Number(m.hp)<=0)continue;const pos=position(m,d);if(!pos)continue;if(m.boss)boss=m;else if(m.midBoss)guardian=m;else if(m.elite){const pulse=reduceMotion?.5:.5+.5*Math.sin(now*.0048+(Number(m.x)||0));radial(pos.x,pos.y+d.th*.20,d.tw*.08,d.tw*.78,[214,153,65],.075+pulse*.025);}}const focal=boss||guardian;if(!focal)return;const pos=position(focal,d);if(!pos)return;const pulse=reduceMotion?.5:.5+.5*Math.sin(now*(boss?.0054:.0046)),rgb=boss?[178,72,116]:[220,157,73];radial(pos.x,pos.y+d.th*.10,d.tw*.16,d.tw*(boss?2.4:1.75),rgb,(boss?.17:.12)+pulse*.04);const v=ctx.createRadialGradient(d.width*.5,d.height*.48,Math.min(d.width,d.height)*.20,d.width*.5,d.height*.48,Math.max(d.width,d.height)*.72);v.addColorStop(0,'rgba(0,0,0,0)');v.addColorStop(1,boss?`rgba(30,4,19,${.20+pulse*.04})`:`rgba(18,10,4,${.11+pulse*.025})`);ctx.fillStyle=v;ctx.fillRect(0,0,d.width,d.height);}
  function drawDepthAtmosphere(now,d){const depth=clamp(Number(api.depth)||1,1,100),band=clamp(Math.floor((depth-1)/10),0,DEPTH_TINTS.length-1),tint=DEPTH_TINTS[band];ctx.save();ctx.globalAlpha=.018+depth/100*.035;ctx.fillStyle=rgba(tint,1);ctx.fillRect(0,0,d.width,d.height);ctx.restore();drawDust(now,d,depth);const v=ctx.createRadialGradient(d.width*.5,d.height*.48,Math.min(d.width,d.height)*.28,d.width*.5,d.height*.48,Math.max(d.width,d.height)*.72);v.addColorStop(0,'rgba(0,0,0,0)');v.addColorStop(1,`rgba(3,2,5,${.18+depth/100*.10})`);ctx.fillStyle=v;ctx.fillRect(0,0,d.width,d.height);}

  let lastUiSync=-Infinity;
  function draw(now=0){const d=dims();if(overlay.width!==d.width)overlay.width=d.width;if(overlay.height!==d.height)overlay.height=d.height;ctx.clearRect(0,0,overlay.width,overlay.height);if(api.state!=='playing')return;drawDepthAtmosphere(now,d);drawPlayerPresence(now,d);drawCharacterGear(now,d);drawEnemyPresence(now,d);if(now-lastUiSync>=120){syncEquipmentUi();lastUiSync=now;}}
  let lastPaint=-Infinity;
  function frame(now){const t=Number(now)||0,interval=reduceMotion?120:33;if(t-lastPaint>=interval){draw(t);lastPaint=t;}requestAnimationFrame(frame);}

  window.__DE_EQUIPMENT_TIER_ART = { version:'v1', sourceForItem, tierIndex, weaponThresholds:[...WEAPON_THRESHOLDS], armorThresholds:[...ARMOR_THRESHOLDS], ringThresholds:[...RING_THRESHOLDS] };
  window.__DE_CHARACTER_GEAR_OVERLAY = { version:'v2', mappedIcons:Object.keys(EQUIPMENT_SOURCE_BY_ICON).length, sourceByIcon:{...EQUIPMENT_SOURCE_BY_ICON}, sourceForItem, drawCharacterGear };
  window.__DE_VISUAL_POLISH = { version:'v4', draw, dims, classGlow:{...CLASS_GLOW} };
