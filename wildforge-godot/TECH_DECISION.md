...
## v0.46 environment art maps to existing world identities

Terrain, ore and vegetation art must remain projections of existing authoritative identities. DIRT/STONE/COAL/COPPER and `wild_tree` are not duplicated into visual variants with their own state. Coal and copper use a stone base plus ore overlay; the block registry, mining power, drops, lighting, ownership and save data remain unchanged. Vegetation sprites likewise cannot alter harvest hits, drops, territorial ownership or deterministic baseline/delta persistence.

Art coverage is intentionally truthful rather than exhaustive. A material with no suitable source Atlas asset keeps its procedural fallback. Recoloring or reusing unrelated production art merely to increase coverage is forbidden because it reduces visual identity and hides missing-art work.
