...
func _run() -> void:
	...
	var before_dwell := progression.simulate_hour_end(100 + SliceWorldProgressionAuthority.FRACTURE_MIN_DWELL_HOURS - 1, [])
	_check(before_dwell.is_empty(), "peaceful maturity still respects the Fracture dwell time")
	var warfront_hour := 100 + SliceWorldProgressionAuthority.FRACTURE_MIN_DWELL_HOURS
	var war_capable := progression.simulate_hour_end(warfront_hour, [])
	...
	var too_soon := progression.simulate_hour_end(warfront_hour + SliceWorldProgressionAuthority.WARFRONT_BALANCE_MIN_DWELL_HOURS - 1, [])
	_check(too_soon.is_empty(), "peaceful late game cannot skip the Warfront maturity window")
	var reforging := progression.simulate_hour_end(warfront_hour + SliceWorldProgressionAuthority.WARFRONT_BALANCE_MIN_DWELL_HOURS, [])
