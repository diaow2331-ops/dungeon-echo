...
func _run() -> void:
	...
	check(SliceArtCatalog.terrain_texture("dirt") != null and SliceArtCatalog.terrain_texture("stone") != null, "core dirt and stone tiles resolve production textures")
	check(SliceArtCatalog.terrain_texture("dirt").get_width() >= 150 and SliceArtCatalog.terrain_texture("stone").get_height() >= 150, "terrain crops retain master pixel density")
	check(SliceArtCatalog.terrain_texture("coal_overlay") != null and SliceArtCatalog.terrain_texture("copper_overlay") != null, "coal and copper resolve dedicated ore overlays")
	check(SliceArtCatalog.environment_texture("wild_tree") != null and SliceArtCatalog.environment_texture("wild_tree").get_height() >= 290, "wild tree resolves a high-resolution production crop")
	var chunk_view := SliceBlockChunkView.new()
	check((chunk_view._production_tile_assets(SliceWorld.DIRT).get("base") as Texture2D) != null, "dirt presentation maps onto canonical DIRT state")
	check((chunk_view._production_tile_assets(SliceWorld.COAL).get("overlay") as Texture2D) != null, "coal art remains a projection of canonical COAL state")
	check(chunk_view._production_tile_assets(SliceWorld.SNOW).is_empty(), "unmatched snow art safely retains existing fallback instead of reusing wrong atlas art")
	chunk_view.free()
	var tree := SliceTreeResource.new()
	root.add_child(tree)
	await process_frame
	check(tree.texture_filter == CanvasItem.TEXTURE_FILTER_NEAREST, "world tree production art keeps nearest filtering")
	check(tree.hp == 3 and tree.drop_item_id == "wood" and tree.drop_count == 2, "tree art does not alter harvest authority")
	tree.free()
	player.free()
