...
const SAVE_PATH := "user://wildforge-godot-v020.json"
...
static func apply_snapshot(main: Node, data: Dictionary) -> bool:
	...
	if version not in [SAVE_VERSION, LEGACY_TRAVELER_SAVE_VERSION, LEGACY_SEEDED_SAVE_VERSION, LEGACY_VEGETATION_SAVE_VERSION, LEGACY_FLUID_SAVE_VERSION, LEGACY_OWNERSHIP_SAVE_VERSION, LEGACY_DELTA_SAVE_VERSION, LEGACY_FULL_SAVE_VERSION]:
		return false
	...
	if version != LEGACY_FULL_SAVE_VERSION:
		...
		if not overrides is Array:
			return false
		var restored_overrides := world.restore_cell_overrides(overrides) if version == SAVE_VERSION else (world.restore_generation2_overrides(overrides) if version in [LEGACY_TRAVELER_SAVE_VERSION, LEGACY_SEEDED_SAVE_VERSION] else world.restore_cell_overrides(overrides))
		...
		if not legacy_rows is Array or not world.restore_legacy_v13_cells(legacy_rows):
			return false
	if version in [SAVE_VERSION, LEGACY_TRAVELER_SAVE_VERSION, LEGACY_SEEDED_SAVE_VERSION, LEGACY_VEGETATION_SAVE_VERSION, LEGACY_FLUID_SAVE_VERSION, LEGACY_OWNERSHIP_SAVE_VERSION]:
		...
		world.apply_baseline_ownership()
	if version in [SAVE_VERSION, LEGACY_TRAVELER_SAVE_VERSION, LEGACY_SEEDED_SAVE_VERSION, LEGACY_VEGETATION_SAVE_VERSION, LEGACY_FLUID_SAVE_VERSION]:
	...
	player.forge_marks = maxi(0, int(p.get("marks", 0))) if version == SAVE_VERSION else 0
	...
	if actor_authority == null:
		return false
	if version in [SAVE_VERSION, LEGACY_TRAVELER_SAVE_VERSION, LEGACY_SEEDED_SAVE_VERSION]:
	...
	if version == SAVE_VERSION:
		if not world.settlement_authority.restore_state(data.get("settlements", [])):
			return false
	if version in [SAVE_VERSION, LEGACY_TRAVELER_SAVE_VERSION]:
