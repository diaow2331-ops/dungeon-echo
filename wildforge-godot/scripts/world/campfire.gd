...
func _draw() -> void:
	...
	var texture := ArtCatalog.item_icon("campfire")
	if texture != null:
		draw_circle(Vector2(0, -4), 13.0 + flicker, Color(0.95, 0.43, 0.18, 0.18))
		var height := 54.0
		var width := height * float(texture.get_width()) / maxf(1.0, float(texture.get_height()))
		draw_texture_rect(texture, Rect2(-width * 0.5, 14.0 - height, width, height), false)
		return
