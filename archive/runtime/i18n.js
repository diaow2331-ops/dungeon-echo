...
(() => {
  ...
  function chooseInitial(){
    try{
      const url=new URL(location.href),q=String(url.searchParams.get('lang')||'').toLowerCase();
      if(q==='en')return'en';if(q==='zh'||q==='zh-cn')return'zh-CN';
      const saved=localStorage.getItem(STORAGE_KEY);if(LANGS.has(saved))return saved;
    }catch(e){}
    const browser=String(navigator.language||'').toLowerCase();
    return browser.startsWith('zh') ? 'zh-CN' : 'en';
  }

  let lang=chooseInitial(),applying=false;
  ...
  function t(key,vars){
    const table=D[lang]||D['zh-CN'];let out=table[key]==null?(D['zh-CN'][key]==null?key:D['zh-CN'][key]):table[key];
    if(vars)for(const[k,v]of Object.entries(vars))out=String(out).replaceAll(`{${k}}`,String(v));return out;
  }
  const write=(el,value)=>{if(el&&el.textContent!==value)el.textContent=value};
  function setText(selector,key){write(document.querySelector(selector),t(key))}
  function setKnownText(selector,key){const el=document.querySelector(selector);if(!el)return;const raw=String(el.textContent||'').trim();if(raw===D['zh-CN'][key]||raw===D.en[key])write(el,t(key))}
  function setPrefix(childId,key){const child=document.getElementById(childId),parent=child&&child.parentElement;if(!parent)return;let node=Array.from(parent.childNodes).find(n=>n.nodeType===3&&String(n.nodeValue||'').trim());const wanted=t(key)+' ';if(node){if(node.nodeValue!==wanted)node.nodeValue=wanted}else parent.insertBefore(document.createTextNode(wanted),child)}
  function setAction(selector,key,hotkey){const el=document.querySelector(selector);if(!el)return;let hk=el.querySelector('span');if(!hk){hk=document.createElement('span');el.appendChild(hk)}if(hk.textContent!==hotkey)hk.textContent=hotkey;let n=Array.from(el.childNodes).find(x=>x.nodeType===3);if(!n){n=document.createTextNode('');el.insertBefore(n,el.firstChild)}const wanted=t(key)+' ';if(n.nodeValue!==wanted)n.nodeValue=wanted}
  function setFullscreen(){const el=document.getElementById('fullscreen-toggle');if(!el)return;const kbd=el.querySelector('kbd');if(kbd&&kbd.textContent!=='F')kbd.textContent='F';let n=Array.from(el.childNodes).find(x=>x.nodeType===3);if(!n){n=document.createTextNode('');el.insertBefore(n,kbd||null)}const wanted=' '+t('action.fullscreen')+' ';if(n.nodeValue!==wanted)n.nodeValue=wanted}
  ...
  function applyClasses(){
    ...
    for(const[id,c]of Object.entries(api.CLASSES)){
      ...
      const src=lang==='en'?CLASS_EN[id]:originalClasses.get(id);if(!src)continue;c.name=src.name;c.blurb=src.blurb;if(c.skill&&src.skill){c.skill.name=src.skill.name;c.skill.desc=src.skill.desc}
    }
  }

  function syncLateUi(){
    setText('#de-audio-settings-pop .de-audio-head b','audio.title');
    const labels=document.querySelectorAll('#de-audio-settings-pop label');if(labels[0])write(labels[0].querySelector('span'),t('audio.music'));if(labels[1])write(labels[1].querySelector('span'),t('audio.sfx'));
    ...
    const master=document.getElementById('de-audio-master');if(master)write(master,window.__DE_AUDIO_DIRECTOR&&window.__DE_AUDIO_DIRECTOR.muted?t('audio.off'):t('audio.on'));
  }
  ...
  function installToggle(){
    ...
    write(btn,t('lang.switch'));if(btn.title!==t('lang.title'))btn.title=t('lang.title');if(btn.getAttribute('aria-label')!==t('lang.title'))btn.setAttribute('aria-label',t('lang.title'));
  }

  function applyShell(){
    applying=true;
    try{
      document.documentElement.lang=lang;document.title=lang==='en'?'Dungeon Echo · 100-Floor Browser Roguelike':'地牢回响 · Dungeon Echo';
      const meta=document.querySelector('meta[name="description"]');if(meta&&meta.content!==t('meta.description'))meta.content=t('meta.description');
      const h1=document.querySelector('header h1'),h1html=`${t('game.name')} <span class="sub">${t('game.name.sub')}</span>`;if(h1&&h1.innerHTML!==h1html)h1.innerHTML=h1html;
      ...
      applyClasses();const api=window.DE_TEST,id=api&&(api.classId||(api.meta&&api.meta.classId)),cls=id&&api.CLASSES&&api.CLASSES[id];write(document.getElementById('st-class'),cls?cls.name:t('slot.class'));
      ...
      setText('#title-screen h2','game.name');setText('#title-screen .title-card > .lede','title.lede');const depth=document.querySelector('#title-screen .depth-picker');if(depth){write(depth.querySelector('.kicker'),t('title.kicker'));write(depth.querySelector('.lede'),t('title.route'))}
      setText('#btn-continue','title.continue');setText('#btn-new','title.new');const greedy=document.getElementById('btn-greedy');if(greedy)write(greedy,t(greedy.getAttribute('aria-pressed')==='true'?'title.greedy.on':'title.greedy.off'));setText('#btn-help','title.help');setText('#btn-achv','title.achv');
      ...
      setText('#shop-screen .kicker','shop.kicker');setText('#shop-screen h2','shop.title');setText('#btn-shop-leave','shop.leave');setText('#talent-screen .kicker','talent.kicker');setText('#talent-screen h2','talent.title');setText('#talent-screen .lede','talent.lede');
      ...
      setText('#town-screen > .title-card > .kicker','town.kicker');setText('#town-screen > .title-card > h2','town.title');setKnownText('#town-head','town.head');syncLateUi();installToggle();
    }finally{applying=false}
  }

  function setLang(next,opts={}){next=String(next||'').toLowerCase()==='en'?'en':'zh-CN';if(!LANGS.has(next))return false;lang=next;if(opts.persist!==false)try{localStorage.setItem(STORAGE_KEY,lang)}catch(e){}if(opts.url)try{const u=new URL(location.href);u.searchParams.set('lang',lang==='en'?'en':'zh');history.replaceState(null,'',u.href)}catch(e){}applyShell();window.dispatchEvent(new CustomEvent('de:languagechange',{detail:{lang}}));return true}
