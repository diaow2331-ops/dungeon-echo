...
(() => {
  ...
  function translateDynamic(value) {
    ...
    if ((m = src.match(/^木桶裂开，滚出\s*(\d+)\s*枚金币。$/))) return `The cask split open and spilled ${m[1]} Gold.`;
    if (src === '木桶里藏着一眼治疗药水！') return 'A Healing Potion was hidden in the cask!';
    if ((m = src.match(/^木桶里藏着【(.+)】，但它掉落在了地上。$/))) return `The cask held [${name(m[1])}], but it fell to the ground.`;
    if ((m = src.match(/^木桶里藏着【(.+)】！$/))) return `The cask held [${name(m[1])}]!`;
    if (src === '木桶碎片下空空如也。') return 'Nothing but splinters inside the cask.';
    if (src === '木桶里积满了陈年灰尘。') return 'The cask was filled with ancient dust.';

    if (src === '你捡起了一瓶治疗药水。') return 'Picked up a Healing Potion.';
    if (src === '你捡起了一张传送卷轴。') return 'Picked up a Teleport Scroll.';
    if (src === '你捡起了一张回城卷轴——按 T 即可带着战利品平安回镇！') return 'Picked up a Return Scroll — press T to bring your loot safely back to town!';
    if (src === '你捡起了一把锈蚀钥匙。') return 'Picked up a Rusty Key.';
    if ((m = src.match(/^你捡起了\s*(\d+)\s*枚金币。$/))) return `Picked up ${m[1]} Gold.`;
    ...
    if ((m = src.match(/^本层有\s*(\d+)\s*个敌人、\s*(\d+)\s*处物资。$/))) return `This floor has ${m[1]} enemies and ${m[2]} loot spots.`;
    if ((m = src.match(/^本层有\s*(\d+)\s*个敌人、\s*(\d+)\s*处物资。站上楼梯按 Enter 下潜。$/))) return `This floor has ${m[1]} enemies and ${m[2]} loot spots. Stand on the stairs and press Enter to descend.`;
    if ((m = src.match(/^第\s*(\d+)\s*次下潜：搜刮战利品，用回城卷轴（T）把一切平安带回小镇——死在这里就会失去背包和金币！$/))) return `Descent ${m[1]}: loot what you can, then use Return Scroll (T) to bring it safely back to town — dying here loses your backpack and carried Gold!`;
    ...
    if ((m = src.match(/^金币不够——向回响支付\s*(\d+)\s*G\s*才能直坠\s*(\d+)\s*层。$/))) return `Not enough Gold — the Echo demands ${m[1]} G to dive ${m[2]} floors.`;
    ...
    if ((m = src.match(/^(.+)被消灭了！（\+(\d+)\s*经验）$/))) return `${name(m[1])} was slain! (+${m[2]} XP)`;
    ...
    out = out
      .replace(/一瓶Healing Potion/g, 'a Healing Potion')
      .replace(/一张Teleport Scroll/g, 'a Teleport Scroll')
      .replace(/一张Return Scroll/g, 'a Return Scroll')
      .replace(/一把Rusty Key/g, 'a Rusty Key')
      .replace(/(\d+)\s*枚Gold/g, '$1 Gold')
      .replace(/本层有\s*(\d+)\s*个敌人、\s*(\d+)\s*处物资。?/g, 'This floor has $1 enemies and $2 loot spots.')
      .replace(/站上楼梯按 Enter 下潜。?/g, ' Stand on the stairs and press Enter to descend.')
      .replace(/木桶裂开，滚出\s*(\d+)\s*Gold。?/g, 'The cask split open and spilled $1 Gold.')
      .replace(/^Picked up\s+一瓶Healing Potion[。.]?$/, 'Picked up a Healing Potion.');
